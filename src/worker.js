...
export class RewardLedger {
  ...
  async fetch(request) {
    ...
    if (path === '/exsign/submit') {
      ...
      const arow = this.rows('SELECT banned FROM accounts WHERE address=?', acct)[0];
      if (arow && arow.banned) return this.j({ error: 'banned' }, 403);
      const EXCH = ['bybit', 'binance', 'okx', 'bitget', 'kucoin', 'gate', 'mexc', 'kraken', 'cryptocom']; // Coinbase removed from the $3 UID bonus — its "UID" is an account name / wallet address we can't verify as a referral
