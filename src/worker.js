...
async function collectorHealth(env) {
  ...
  const exHtml = st.exchanges.map((e) => `<div class="hp-ex"><span class="hp-edot ${e.connected ? 'on' : 'off'}"></span><b>${e.name}</b><small>${e.connected ? 'live' : 'OFFLINE'} · ${ago(e.lastEventAt)} · ${e.eventsPerMin || 0}/min</small></div>`).join('');
...
async function handleStats(url, env) {
  ...
  try {
    ...
    if (!all.length) return new Response('Stats temporarily unavailable — daily KV list quota reached (resets at UTC midnight). Your data is still being recorded.', { status: 200, headers: { 'content-type': 'text/plain; charset=utf-8' } });
  }
