...
function loadPromo(){var el=document.getElementById('rwdPromo');if(!el)return;fetch('/api/reward/promo/list?key='+encodeURIComponent(key)).then(function(r){return r.json();}).then(function(d){var pend=(d&&d.pending)||[],dec=(d&&d.decided)||[];var amt=(+((d&&d.promoUsd)||1)).toFixed(2);if(!pend.length&&!dec.length){el.innerHTML='<div class="empty">no promo submissions yet</div>';return;}var nowT=Date.now();function who(p){return p.username?('@'+esc(p.username)):(p.email?esc(p.email):esc(String(p.address||'').replace(/^u:/,'').slice(0,10)));}function plat(p){return p.platform==='x'?'<span style="font-weight:700;color:#e9e7df;flex:0 0 46px">X</span>':'<span style="font-weight:700;color:#5ad8e6;flex:0 0 46px">TikTok</span>';}function lnk(p){return '<a href="'+esc(p.url)+'" target="_blank" rel="noopener" style="color:#9fe0ff;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;flex:1;min-width:0">'+esc(String(p.url||'').replace(/^https?:\\/\\//,''))+'</a>';}var html=pend.map(function(p){var age=nowT-p.ts,h=Math.floor(age/3600000),ready=age>=86400000;return '<div class="wd-row">'+plat(p)+'<span class="addr mono" style="color:'+addrColor(p.address)+'">'+who(p)+'</span>'+lnk(p)+'<span class="meta">'+(h<1?'now':h+'h ago')+'</span>'+(ready?'<button class="pay" data-pra="'+esc(p.id)+'">approve $'+amt+'</button>':'<span class="meta" style="color:#7f8893">approvable in '+Math.ceil((86400000-age)/3600000)+'h</span>')+'<button class="pay" data-prr="'+esc(p.id)+'" style="color:#ff8a80;border-color:rgba(255,98,88,.4)">reject</button></div>';}).join('');html+=dec.slice(0,8).map(function(p){var st=p.status==='approved'?'<span class="meta" style="color:#2ebd85">approved +$'+(+p.amount||0).toFixed(2)+'</span>':'<span class="meta" style="color:#ff8a80">rejected'+(p.note?' · '+esc(p.note):'')+'</span>';return '<div class="wd-row" style="opacity:.55">'+plat(p)+'<span class="addr mono">'+who(p)+'</span>'+lnk(p)+st+'</div>';}).join('');el.innerHTML=html;Array.prototype.forEach.call(el.querySelectorAll('[data-pra]'),function(b){b.addEventListener('click',function(){if(!confirm('Approve? $'+amt+' lands on their balance. Did you open the post and check it mentions MarginPad?'))return;b.textContent='…';fetch('/api/reward/promo/review?key='+encodeURIComponent(key),{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({id:b.getAttribute('data-pra'),action:'approve'})}).then(function(r){return r.json();}).then(function(j){if(j&&j.error)alert(j.error==='too_early'?'Less than 24h old — wait.':j.error);loadPromo();});});});Array.prototype.forEach.call(el.querySelectorAll('[data-prr]'),function(b){b.addEventListener('click',function(){var note=prompt('Reason (the user sees this):','');if(note===null)return;b.textContent='…';fetch('/api/reward/promo/review?key='+encodeURIComponent(key),{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({id:b.getAttribute('data-prr'),action:'reject',note:note})}).then(function(){loadPromo();});});});}).catch(function(){});}
...
export class RewardLedger {
  ...
  async fetch(request) {
    ...
    if (path === '/promo/review') { // admin: approve (credits the balance) or reject (with a note the user sees)
      ...
      if (now - p.ts < 86400000) return this.j({ error: 'too_early', waitMs: 86400000 - (now - p.ts) }, 400); // the post must have been live 24h before payout
