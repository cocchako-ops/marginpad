...
async function opsCfg(env) { let c = {}; try { c = JSON.parse(await env.STATS.get('ops:cfg') || '{}'); } catch (e) {} return { brief: c.brief !== false, briefHour: (Number.isFinite(+c.briefHour) && +c.briefHour >= 0 && +c.briefHour <= 23) ? +c.briefHour : 8, alerts: c.alerts !== false }; }
...
function renderOpsViz(){
  ...
  host.innerHTML='<div class="ovz-grid">'+left+'<div class="ovz-p" id="ovzMid"><div class="ovz-h">Coins → direction → traders · ribbon = margin</div><div id="ovzSvg"></div></div>'+right+'</div>'
    ...
    +'<div class="ovz-p"><div class="ovz-h">Trade tape <span style="color:#31405c;text-transform:none;letter-spacing:0">&middot; live events while this tab is open</span></div><div id="ovzTape"></div></div>'
    +'<div class="ovz-p"><div class="ovz-h">Risk radar <span id="ovzRiskN" style="color:#ffb347"></span></div><div id="ovzRisk"></div></div>'
    +'<div class="ovz-p"><div class="ovz-h">Largest positions'+(OPS.coinF?' <span class="ovz-cf" data-ovc="'+esc(OPS.coinF)+'" title="Clear filter">'+esc(OPS.coinF)+' &#10005;</span>':'')+'</div><div id="ovzBig"></div></div></div>';
  // ---- sankey ----
  ...
  document.getElementById('ovzTape').innerHTML=OPS.tape.slice(0,12).map(function(x){var t=x.t,lg=t.side!=='short';var lbl,col;
  ...
  async fetch(request, env, ctx) {
   try {
    ...
    if (url.pathname === '/api/admin/opscfg' && (await adminCookieOk(request, env))) { // ops automation settings (morning brief + alerts)
      ...
      if (request.method === 'POST') {
        let b = {}; try { b = await request.json(); } catch (e) {}
        const next = { brief: b.brief !== false, briefHour: (Number.isFinite(+b.briefHour) && +b.briefHour >= 0 && +b.briefHour <= 23) ? +b.briefHour : 8, alerts: b.alerts !== false };
...
export class UserStore {
  constructor(state, env) {
    ...
    s.exec('CREATE TABLE IF NOT EXISTS missions(user_id TEXT, day TEXT, mid TEXT, ts INTEGER, PRIMARY KEY(user_id,day,mid))'); // claimed daily missions (verification runs against uevents) // per-user per-endpoint daily API usage (the ops API tab reads this)
