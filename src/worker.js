...
async function handleTrack(url, request, env, ctx) {
  ...
  if (type === 'pageview') {
    ...
    if (type === 'exchange' || type === 'paper' || type === 'hotpair' || type === 'tool' || type === 'tab' || type === 'el' || type === 'nav' || type === 'prod' || type === 'close' || type === 'chat' || type === 'signin' || type === 'search' || type === 'watch') { // live activity ring buffer — now includes every meaningful CLICK (calculator tabs, product opens, element clicks) + key actions (trade close, chat, sign-in, search, watchlist) with a visitor id so the journeys view can show WHAT each person does, not just where they go
      try {
        const cc = (request.cf && request.cf.country) || '';
        let log = []; try { log = JSON.parse(await env.STATS.get('evlog') || '[]'); } catch (e) {}
...
<div class="ovv-p" style="margin-top:12px"><div class="ovv-ph">Community <span>latest posts &middot; you also get a Telegram ping per post</span></div><div class="list">${commList}</div></div>
  ...
  function evKindOf(t){if(t==='exchange')return 'money';if(t==='paper'||t==='hotpair'||t==='close')return 'trade';if(t==='chat'||t==='like'||t==='comment'||t==='follow'||t==='post')return 'social';if(t==='signup'||t==='login'||t==='claim'||t==='withdraw'||t==='mission'||t==='academy'||t==='signin')return 'acct';if(t==='tool')return 'tool';if(t==='pv')return 'page';return 'other';}
...
function handleHappyHour() {
  ...
  return new Response(JSON.stringify({ enabled: HH.enabled, hourUTC: HH.hourUTC, durMin: HH.durMin, roeMin: HH.roeMin, levMax: HH.levMax, xp: HH.xp, active, nextStartMs, endMs: active ? todayStart + dur : null, now }), { headers: { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'public, max-age=30', ...CORS } });
}
    ...
    if (path === '/trades') { // signed-in user's paper-trade journal sync — MERGE with the stored set (union by trade id) so a stale device/browser can't wipe trades synced from another. A blind full-replace caused leaderboard entries to flicker on/off as two devices took turns overwriting each other.
      ...
      try {
        ...
        for (const ev9 of evs) {
          ...
          nIns++; try { if (kind === 'close') { this._grantXp(uid, 'trade', 3, { dayCap: 15, note: (e.sym||'') + ' closed' }); if (pv != null && pv > 0) { this._grantXp(uid, 'trade_win', 15, { dayCap: 60, note: (e.sym||'') + ' +$' + pv.toFixed(2) }); if (roe != null && roe >= HH.roeMin && (+e.lev || 1) <= HH.levMax && hhActiveAt(ts9)) this._grantXp(uid, 'trade_hh', HH.xp, { dayCap: 750, note: 'XP Happy Hour · ' + Math.round(roe) + '% ROE' }); } } } catch (xe) {}
        }
...
async function handleComm(url, request, env, ctx) {
  ...
  // surface community actions in the live activity feed (comment always; follow only when it becomes a follow, not an unfollow)
  if (request.method === 'POST' && r && r.status === 200 && (path === '/comment' || path === '/follow')) { try { const _cd = JSON.parse(txt); if (_cd && !_cd.error) {
    if (path === '/comment') { const pr3 = evPush(env, request, 'comment', '', '/community/'); if (ctx && ctx.waitUntil) ctx.waitUntil(pr3); else await pr3; }
    else if (path === '/follow' && _cd.following) { let tn = ''; try { tn = String((JSON.parse(rawBody || '{}').tname) || '').slice(0, 24); } catch (e) {} const pr4 = evPush(env, request, 'follow', tn, '/community/'); if (ctx && ctx.waitUntil) ctx.waitUntil(pr4); else await pr4; }
  } } catch (e) {} }
