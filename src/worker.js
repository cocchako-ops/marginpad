...
function renderWd(){var el=document.getElementById('wdList');if(!el)return;
  ...
  el.innerHTML=(rows.slice(0,120).map(function(w){
    ...
    return '<div style="display:flex;gap:10px;align-items:center;font-family:Consolas,monospace;font-size:11px;padding:7px 0;border-bottom:1px solid #10151f">'
      +'<span style="width:96px;color:#8fa3c4" title="requested">'+wdFmtD(w.ts)+'</span>'
      +'<span style="width:130px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:#dbe4f5"'+(w.email?' title="'+esc(w.email)+'"':'')+'>'+whoTxt+(w.banned?' <b style="color:#ff6258">BAN</b>':'')+'</span>'
      +'<b style="width:64px;text-align:right;color:#c2f64a">$'+(+w.usd).toFixed(2)+'</b>'
      +'<span style="width:118px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:'+addrColor(w.wallet)+'" title="'+esc(w.wallet)+'">'+esc(String(w.wallet||'').slice(0,10))+'&hellip;</span>'
      +'<span style="width:40px">'+flag(w.cc)+'</span>'
      +'<span style="width:104px;color:#8fa3c4" title="account IP">'+esc(String(w.ip||'&#8212;'))+'</span>'
      +'<span style="width:52px;color:#8fa3c4" title="claims">'+(w.claims||0)+'cl</span>'
      +'<span style="width:60px">'+st+'</span>'
      +'<span style="width:96px;color:#8fa3c4" title="paid at">'+(w.paidTs?wdFmtD(w.paidTs):'&#8212;')+'</span>'
      +'<span style="flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:#5c6b84" title="'+esc(w.txid||'')+'">'+(w.txid?esc(String(w.txid).slice(0,14))+'&hellip;':'')+'</span>'
