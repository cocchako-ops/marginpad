...
function renderWd(){var el=document.getElementById('wdList');if(!el)return;
  ...
  el.innerHTML=(rows.slice(0,120).map(function(w){
    ...
    return '<div style="padding:8px 0;border-bottom:1px solid #10151f">'
      ...
      +'</div>'
      +'<div style="font-family:Consolas,monospace;font-size:11px;margin-top:3px;color:'+addrColor(w.wallet)+';word-break:break-all;user-select:all" title="payout wallet — tap to select">'+esc(String(w.wallet||''))+'</div>'
...
function secBFlags(r,minWd){var f=[],tot=+r.dwellTotal||0,share=tot>0?(+r.dwellRewards||0)/tot:0;
  ...
  if(r.vpn&&r.claims>0)f.push({k:'vpn',t:'VPN/DC',c:'#b48cff'});
  ...
  return f;}
function secBScore(f){var W={only:40,major:25,notrade:15,run:20,payout:10,vpn:10,hungry:20,dev:30},s=0;f.forEach(function(x){s+=W[x.k]||0;});return Math.min(100,s);}
function renderSecB(){var el=document.getElementById('secBList');if(!el||!secBRows)return;var minWd=window._secMinWd||5;
  ...
  var cc=document.getElementById('secBCards');if(cc)cc.innerHTML=card('<span style="color:#ff6258">'+(CNT.only||0)+'</span>','Rewards-ONLY users')+card('<span style="color:#ff8c42">'+((CNT.major||0)+(CNT.run||0))+'</span>','Rewards-majority / farm-&-leave')+card('<span style="color:#ffb347">'+(CNT.notrade||0)+'</span>','Claim, never trade')+card('<span style="color:#ffd75a">'+(CNT.payout||0)+'</span>','Close to payout')+card('<span style="color:#b48cff">'+(CNT.vpn||0)+'</span>','VPN / datacenter')+card('$'+atRisk.toFixed(2),'Unpaid $ on flagged');
  ...
  bindAccts(el);}
function loadSecB(){fetch('/api/admin/security?_='+Date.now()).then(function(r){return r.json();}).then(function(d){window._secMinWd=+d.minWdUsd||5;secBRows=(d.rows||[]).filter(function(r){return (r.claims||0)>0||(+r.balanceUsd||0)>0||(+r.dwellRewards||0)>0;});renderSecB();}).catch(function(){var el=document.getElementById('secBList');if(el)el.innerHTML='<div class="empty">could not load</div>';});}
...
export class RewardLedger {
  ...
  async fetch(request) {
    ...
    if (path === '/exsign/submit') {
      ...
      if (!/^[A-Za-z0-9_-]{4,32}$/.test(uid)) return this.j({ error: 'bad_uid' }, 400);
...
export class UserStore {
  ...
  async fetch(request) {
    ...
    if (path === '/security') { // internal: behavioral profile per user for the ops Security tab — how much of their time is spent on /rewards, do they trade at all, VPN org. Joined with RewardLedger accounts in the worker.
      ...
      const users = this.rows("SELECT id,email,username,created,last_seen,pv,cc,dev,org,status FROM users ORDER BY created DESC LIMIT 3000").map(u => { const d = dw[u.id] || {}, t = tr[u.id] || {}; return { uid: u.id, email: u.email || '', username: u.username || '', created: u.created || 0, lastSeen: u.last_seen || 0, pv: u.pv || 0, cc: u.cc || '', dev: u.dev || '', org: u.org || '', status: u.status || 'active', dwellTotal: +d.tot || 0, dwellRewards: +d.rw || 0, paths: +d.np || 0, trades: +t.n || 0, opens: +t.opens || 0 }; });
      return this.j({ users });
    }
