...
OPS.srv=[];
function loadTradeev(){fetch('/api/auth/tradeev?key='+encodeURIComponent(key)+'&since='+(Date.now()-172800000)).then(function(r){return r.json();}).then(function(d){OPS.srv=(d&&d.events)||[];try{renderOpsViz();}catch(e7){}}).catch(function(){});}
function loadOps(){fetch('/api/auth/opentrades?key='+encodeURIComponent(key)).then(function(r){return r.json();}).then(function(d){var np9=(d&&d.positions)||[];try{opsDiff(OPS.pos,np9);}catch(e8){}OPS.pos=np9;OPS.traders=(d&&d.traders)||0;renderOps();}).catch(function(){var el=document.getElementById('opsBody');if(el&&!OPS.pos.length)el.innerHTML='<div class="empty">could not load</div>';});}
...
function renderOpsViz(){
  ...
  var tOpen=0,tClose=0,tPnl=0,tWin=0,tLiq=0;
  (OPS.srv||[]).forEach(function(ev){if(+ev.ts<utc0)return;
    if(ev.kind==='open')tOpen++;
    else if(ev.kind==='close'){tClose++;if(ev.pnl!=null){tPnl+=+ev.pnl;if(+ev.pnl>0)tWin++;}if(ev.liq)tLiq++;}});
  ...
  if(tdEl)upd9('ovzToday','<span class="tk">TODAY (UTC)</span>'
    ...
    +'<span>win rate <b>'+(tClose?Math.round(tWin/tClose*100)+'%':'&#8212;')+'</b></span>'
