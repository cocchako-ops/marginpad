...
function loadXpPromos(){var list=document.getElementById('xpPromoList');if(!list||list._init)return;list._init=1;var PR=[],xoff=0;/* xoff = server clock offset so windows are correct regardless of this machine's clock */
  ...
  function draw(){list.innerHTML=PR.length?PR.map(function(p,i){return '<div class="xp-promo" data-i="'+i+'" style="border:1px solid #242b34;border-radius:11px;padding:12px;margin-bottom:9px;background:#0e1116">'
    ...
    +'<label style="font-size:10.5px;color:#9aa3ad;display:block;grid-column:1/-1">Coins <span style="color:#5c656f">(space-separated &middot; blank = any coin)</span><input data-f="coins" value="'+xesc((p.coins||[]).join(' '))+'" placeholder="BTC ETH SOL XRP HYPE" style="'+IN+'"></label>'
  ...
  list.addEventListener('input',function(e){var card=e.target.closest('.xp-promo');if(!card)return;var i=+card.getAttribute('data-i'),f=e.target.getAttribute('data-f'),p=PR[i];if(!p||!f)return;
    if(f==='enabled')p.enabled=e.target.checked;else if(f==='winOnly')p.winOnly=e.target.checked;else if(f==='title')p.title=e.target.value;else if(f==='coins')p.coins=e.target.value.toUpperCase().split(/[^A-Z0-9]+/).filter(Boolean);else if(f==='xp')p.xp=Math.max(1,+e.target.value||0);else if(f==='levMax')p.levMax=Math.max(1,+e.target.value||20);else if(f==='roeMin')p.roeMin=Math.max(0,+e.target.value||0);else if(f==='hours'){p._hours=Math.max(1,+e.target.value||24);var sb=card.querySelector('.xp-start');if(sb)sb.innerHTML='&#9654; Start / Restart for '+p._hours+'h';}});
  list.addEventListener('click',function(e){var b=e.target.closest('[data-act]');if(!b)return;var card=b.closest('.xp-promo'),i=+card.getAttribute('data-i'),act=b.getAttribute('data-act'),p=PR[i];
