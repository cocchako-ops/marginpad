  ...
  if (type === 'pageview') {
    ...
    if (type === 'exchange' || type === 'paper' || type === 'hotpair' || type === 'tool') { // live activity ring buffer
      try {
        ...
        log.unshift({ t: type, e: label, cc: cc, u: (getCookie(request, 'mp_un') || '').slice(0, 24), p: (p.get('p') || '').slice(0, 48), ts: Date.now() });
...
async function handleStats(url, env, request) {
  ...
  if (url.searchParams.get('format') === 'json') {
    const gc = async k => { try { const r = await env.STATS.getWithMetadata(k); return (r && r.metadata && r.metadata.c) || (r && r.value ? parseInt(r.value, 10) : 0) || 0; } catch (e) { return 0; } };
    const day = new Date().toISOString().slice(0, 10);
    const [uvTod, uvTot, pvTot, affTot, affTod, botTod] = await Promise.all([gc('uv:day:' + day), gc('uv:total'), gc('pv:total'), gc('aff:total'), gc('aff:day:' + day), gc('botf:day:' + day)]);
    ...
    return new Response(JSON.stringify({ online: recent.size, uvToday: uvTod, uvTotal: uvTot, pv: pvTot, aff: affTot, revToday: Math.round(affTod * 0.45), botToday: botTod, visitors: pvl.slice(0, 120), feed: evl.slice(0, 30), ts: now }), { headers: { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' } });
  }
...
@media(max-width:620px){body{padding:20px 13px 70px}h1{font-size:22px}}
  ...
  function renderJourneys(arr){var de=document.getElementById('vjDesk'),mo=document.getElementById('vjMob');if(!de&&!mo)return;arr=arr||[];var by={},order=[];for(var i=0;i<arr.length;i++){var v=arr[i],id=v.v||'?';if(!by[id]){by[id]={v:id,cc:v.cc,u:v.u,d:(v.d||''),s:v.s,f:v.f,last:v.ts,pages:[]};order.push(id);}var g=by[id];if(v.ts>g.last)g.last=v.ts;g.pages.unshift(v.p||'/');g.s=v.s;g.f=v.f;if(v.u)g.u=v.u;}
    function jcard(g){var path=g.pages.map(function(p){return '<span class="vj-pg">'+esc(pageShort(p))+'</span>';}).join('<span class="vj-arr">›</span>');var src=g.f?('from '+esc(pageShort(g.f))):('via '+esc(srcName(g.s)||'direct'));return '<div class="vj"><div class="vj-top"><span class="vj-dot" style="background:'+vidColor(g.v)+'"></span><span class="vj-cc">'+(g.u?('👤 @'+esc(g.u)):((flag(g.cc)||'🌐')+' '+(g.cc?esc(CC[g.cc]||g.cc):'Unknown')))+'</span><span class="vj-src">'+src+'</span><span class="vj-a">'+ago(g.last)+' ago</span></div><div class="vj-path">'+path+'</div></div>';}
  ...
  function poll(){fetch('/api/stats?key='+encodeURIComponent(key)+'&format=json',{cache:'no-store'}).then(function(r){return r.json();}).then(function(d){
    ...
    cv('#topCards .cv',0,N(d.uvToday));cv('#topCards .cv',1,N(d.uvTotal));cv('#topCards .cv',2,N(d.pv));cv('#topCards .cv',3,N(d.aff));
    cv('#revCards .cv',1,'$'+N(d.revToday));
    var bf=document.getElementById('botf');if(bf)bf.textContent=N(d.botToday);
    renderJourneys(d.visitors);
...
async function handleReward(url, request, env) {
  ...
  if (path === '/config') {
    ...
    if (request.method === 'POST') {
      ...
      const next = { ...cur };
      for (const k of ['enabled', 'wdEnabled', 'requireOnchain']) if (k in b) next[k] = !!b[k];
      for (const k of ['amountUsd', 'perDayUsd', 'minWdUsd', 'capUsd', 'cooldownS', 'ipCap', 'minClaimsToWd', 'welcomeUsd', 'prize1', 'prize2', 'prize3']) if (k in b) next[k] = +b[k];
  ...
  const cfg = { amountC: full.amountC, cooldown: full.cooldown, perDayC: full.perDayC, minWdC: full.minWdC, capC: full.capC, ipCap: full.ipCap, minClaimsToWd: full.minClaimsToWd, welcomeC: full.welcomeC, pauseMsg: full.pauseMsg, prize1: full.prize1, prize2: full.prize2, prize3: full.prize3 };
  ...
  if ((path === '/admin' || path === '/admin/paid' || path === '/accounts' || path === '/log' || path === '/unlock' || path === '/remove' || path === '/detail' || path === '/note' || path === '/ban' || path === '/unban' || path === '/adjust' || path === '/lbban' || path === '/lbtop' || path === '/lbhistory' || path === '/message' || path === '/support/close' || path === '/support/new' || (path === '/support' && request.method === 'GET')) && !adminOk) return jr({ error: 'forbidden' }, 403);
  ...
  const fwd = new Request('https://do' + path + (request.method === 'GET' ? url.search : ''), {
    method: request.method, headers: { 'content-type': 'application/json', 'x-cfg': JSON.stringify(cfg), 'x-ip': ip, 'x-cc': cc, 'x-dev': deviceOf(ua), 'x-vid': vid, 'x-acct': acct || '' },
  ...
  if (r.status === 200 && (path === '/log' || path === '/accounts' || path === '/detail')) {
    try {
      const data = JSON.parse(txt);
      const arr = path === '/log' ? data.log : path === '/accounts' ? data.accounts : (data.address ? [data] : []);
...
export class RewardLedger {
  constructor(state, env) {
    ...
    for (const col of ['cc TEXT', 'dev TEXT', 'claims INTEGER DEFAULT 0', 'banned INTEGER DEFAULT 0', 'payout_addr TEXT', 'welcome INTEGER DEFAULT 0']) { try { s.exec('ALTER TABLE accounts ADD COLUMN ' + col); } catch (e) {} } // idempotent migration. accounts keyed by account id ('u:<uid>'); payout_addr = withdrawal wallet; welcome = one-time sign-up bonus granted
  ...
  async fetch(request) {
    ...
    const cc = request.headers.get('x-cc') || '', dev = request.headers.get('x-dev') || '', vid = request.headers.get('x-vid') || '';
    ...
    const meta = { amount: cfg.amountC / 100, perDay: cfg.perDayC / 100, minWd: cfg.minWdC / 100, minClaimsToWd: cfg.minClaimsToWd || 0, welcomeAmt: (cfg.welcomeC || 0) / 100, pauseMsg: cfg.pauseMsg || '', prize1: cfg.prize1, prize2: cfg.prize2, prize3: cfg.prize3 };
    ...
    if (path === '/claim') {
      ...
      if (!r) {
        ...
        sql.exec('INSERT INTO accounts(address,day,created,ip,cc,dev) VALUES(?,?,?,?,?,?)', acct, day, now, ip, cc, dev);
    ...
    if (path === '/accounts') { // mini-dashboard: who signed up (address + where-from + multi-account flag) — newest first
      ...
      const list = this.rows('SELECT address,balance,earned,claims,cc,dev,ip,created,last_claim,banned FROM accounts ORDER BY created DESC LIMIT 500')
        .map(a => ({ address: a.address, balanceUsd: a.balance / 100, earnedUsd: a.earned / 100, claims: a.claims || 0, cc: a.cc || '', dev: a.dev || '', ip: a.ip || '', created: a.created, lastClaim: a.last_claim, sameIp: (a.ip ? (ipCount[a.ip] || 1) : 1), banned: !!a.banned }));
    ...
    if (path === '/detail') { // admin: everything about one address, for the click-through popup
      ...
      const fraud = { riskLevel, flags, sameIp, ipWallets, ageMs, claimsPerHour: +claimsPerHour.toFixed(2) };
