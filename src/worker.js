...
function renderOpsViz(){
  ...
  topT.forEach(function(k,ix){var g=tr[k];var ny=ty0+ix*(TH+TG);var yc=ny+TH/2;
    ...
    sv+='<rect x="'+(W-NW+6)+'" y="'+ny+'" width="'+(NW-8)+'" height="'+TH+'" rx="9" fill="#0e1322" stroke="#22304e"/>'
      +'<text x="'+(W-NW+16)+'" y="'+(yc-2.5)+'" font-size="10.5" font-weight="700" fill="#e9edf7" font-family="Consolas,monospace">'+esc(('@'+k).slice(0,15))+'</text>'
      +'<text x="'+(W-NW+16)+'" y="'+(yc+9.5)+'" font-size="9" fill="#8fa3c4" font-family="Consolas,monospace">'+ovzMoney(g.m)+' · '+g.n+' pos</text>';});
...
(function(){
  ...
  if(hv)hv.addEventListener('click',function(e){var c2=e.target.closest&&e.target.closest('[data-ovc]');if(c2){var sy2=c2.getAttribute('data-ovc');OPS.coinF=OPS.coinF===sy2?null:sy2;renderOpsViz();return;}var c=e.target.closest&&e.target.closest('[data-ovf]');if(!c)return;OPS.feedF=c.getAttribute('data-ovf');renderOpsViz();});
