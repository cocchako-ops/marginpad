...
window.mpWorldMap=function(cv,hubLabel){
  ...
  var items=[],base=null,rafId=null,W=0,H=0,dpr=Math.min(2,window.devicePixelRatio||1);
  function proj(lat,lon){return [(lon+180)/360*W,(LG.top-lat)/(LG.top-LG.bot)*H];}
  function size(){var w=cv.clientWidth||800;var h=Math.round(w*(LG.r/LG.c));cv.width=Math.round(w*dpr);cv.height=Math.round(h*dpr);cv.style.height=h+'px';W=w;H=h;base=null;}
  ...
  size();
  return {update:function(list){items=list||[];if(!rafId)rafId=requestAnimationFrame(frame);}};
};
OPS.srv=[];
...
function ovWorldCc(d){var cv=document.getElementById('ovWorld');if(!cv||!d||!d.cc7||!window.mpWorldMap)return;
  ...
  window.__ovWm.update(d.cc7.slice(0,24).map(function(x,i){return {cc:String(x.cc||'').toUpperCase(),v:(+x.n)*3,col:'139,123,255',hot:0,label:i<4?String(x.cc).toUpperCase():null};}));
