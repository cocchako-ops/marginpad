...
<div class="ovv-g ovv-g21">
  <div class="ovv-p"><div class="ovv-ph">Real-time activity <span>tools opened &middot; paper trades &middot; &#128176; money clicks &middot; 12s</span><a class="ovv-more" data-goto="jmap">view all &rarr;</a></div><div class="feed" id="evFeed">${feed}</div></div>
...
var PROMO_REJ_KEY='mp_promo_reject';
  ...
  var EXNAMES={bybit:'Bybit',binance:'Binance',okx:'OKX',bitget:'Bitget',kucoin:'KuCoin',gate:'Gate',mexc:'MEXC',kraken:'Kraken',cryptocom:'Crypto.com'};
...
export class RewardLedger {
  ...
  async fetch(request) {
    ...
    if (path === '/exsign/submit') {
      ...
      const arow = this.rows('SELECT banned FROM accounts WHERE address=?', acct)[0];
      if (arow && arow.banned) return this.j({ error: 'banned' }, 403);
      const EXCH = ['bybit', 'binance', 'okx', 'bitget', 'kucoin', 'gate', 'mexc', 'kraken', 'cryptocom'];
