...
async function handleReward(url, request, env) {
  ...
  if (path === '/config') {
    ...
    if (request.method === 'POST') {
      ...
      const next = { ...cur };
      for (const k of ['enabled', 'wdEnabled', 'requireOnchain', 'promoEnabled', 'exsignEnabled']) if (k in b) next[k] = !!b[k];
...
export class UserStore {
  constructor(state, env) {
    ...
    s.exec('CREATE TABLE IF NOT EXISTS botuse(uid TEXT, day TEXT, ep TEXT, n INTEGER DEFAULT 0, last INTEGER, PRIMARY KEY(uid,day,ep))'); // per-user per-endpoint daily API usage (the ops API tab reads this)
