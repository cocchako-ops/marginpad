...
async function handleKlines(url) {
  ...
  for (const b of ['https://data-api.binance.vision', 'https://api.binance.com']) {
    try {
      const r = await fetch(b + '/api/v3/klines?symbol=' + pair + '&interval=' + (biMap[iv] || '1h') + '&limit=300', { cf: { cacheTtl: 30 } });
  ...
  try {
    ...
    const r = await fetch('https://api.bybit.com/v5/market/kline?category=spot&symbol=' + pair + '&interval=' + (byMap[iv] || '60') + '&limit=300', { cf: { cacheTtl: 30 } });
