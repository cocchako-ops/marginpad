...
(async () => {
  ...
  await withBrowser(async (browser) => {
    ...
    const cards = await page.evaluate(() => [...document.querySelectorAll('#bsw .bs')].map(b => ({ k: b.getAttribute('data-board'), logo: !!b.querySelector('svg.bybit-logo'), pb: (b.querySelector('.pb') || {}).textContent || '', pz: (b.querySelector('.pz') || {}).textContent || '' })));
    ok(cards.length === 6, 'six board cards (' + cards.map(c => c.k).join(',') + ')');
    ...
    const sw = await page.evaluate(async () => {
      ...
      const on = document.querySelector('#bsw .bs.on');
    ...
    ok(/Powered by Bybit/.test(sw.note) && /2026-09-14/.test(sw.note) && /\$100/.test(sw.note) && !/margin/i.test(sw.note), 'note: powered by Bybit, report-based, prizes from 2026-09-14');
