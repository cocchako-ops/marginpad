...
(async () => {
  ...
  await withBrowser(async (browser) => {
    ...
    if (!only.length) {
      ...
      const pm = await page.evaluate(() => ({ folds: document.querySelectorAll('#view details.fold').length, height: document.getElementById('view').scrollHeight, lapses: /lapses|expiring/i.test(document.getElementById('view').innerText), book: /paid for Premium, all time/i.test(document.getElementById('view').innerText), payRows: document.querySelectorAll('#view .pmk').length, months: document.querySelectorAll('#view .pmc').length, paidCol: !!Array.from(document.querySelectorAll('#view .tbl th')).find(th => th.innerText.trim().toLowerCase() === 'paid') }));
      chk('premium desk: payment book first (tiles, 12-month strip, who/when/how-much rows, paid column on members), positions and closes folded, page under 2800px', pm.folds === 2 && pm.height < 2800 && pm.book && pm.months === 12 && pm.paidCol, pm);
