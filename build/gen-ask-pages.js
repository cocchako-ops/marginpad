...
const SIBS = [
  ...
  ['/mcp-server-for-crypto-trading/', 'Is there an MCP server for crypto trading?'],
  ['/practice-for-a-funded-account/', 'Where can I practice for a funded account challenge?'],
  ['/what-leverage-should-a-beginner-use/', 'What leverage should a beginner use?'],
];
...
const PAGES = [
  {
    ...
    // money. The live figures arrive server-side from /api/leverage; nothing here restates them, so they cannot drift.
    slug: 'what-leverage-should-a-beginner-use',
    kind: 'leverage',
    h1: 'What leverage should a beginner use in crypto futures?',
    title: 'What Leverage Should a Beginner Use? - Measured on Real Closed Trades | MarginPad',
    desc: 'Answered with a count, not an opinion: how often trades at each leverage band actually end in liquidation, measured on 30 days of closed trades settled on live prices with real fees, funding and wick-checked liquidation.',
    lead: 'Answered from our own book: how often trades at each leverage band end in a forced close, and what the middle trade in each band actually returns.',
    dataset: ['MarginPad leverage scan', 'Every paper trade closed on MarginPad in the last 30 days, grouped by the leverage it was opened at, with the share of each band that ended in liquidation, the share that closed in profit, and the median return on margin. Settled on real live prices with taker fees on both legs, funding on held positions, and liquidation checked against one-minute candle extremes.', 'https://marginpad.io/api/leverage'],
    body: [
      ['The one piece of arithmetic that decides it', '<p>Leverage does not change how much you can make. It changes <strong>how far price has to move against you before the position is gone</strong>, and that distance is roughly <code>1 / leverage</code>.</p><p>At 10x, a 10% move against you wipes the margin. At 25x it takes 4%. At 100x it takes 1%. At 125x, 0.8% - which Bitcoin covers in an ordinary hour and an altcoin covers between two sips of coffee. The maintenance margin the venue holds back takes a little more off each of those numbers, so the real distance is always slightly shorter than the headline suggests.</p><p>This is why the question is not "how much leverage can I get" but "how wrong can I afford to be". A beginner is going to be wrong often, and being wrong is only educational if you are still in the market afterwards.</p>'],
      ['The three numbers a venue quotes, and what they hide', '<p><strong>Maximum leverage</strong> is a marketing number. A venue advertising 125x will quietly cut the ceiling as your position grows, because maintenance margin rises with size - so the 125x on the banner is available on a position too small to matter to you.</p><p><strong>Cross vs isolated</strong> decides what gets taken when it goes wrong. Isolated margin risks only what you put on that trade; cross margin lets one bad position reach into everything else in the account. A beginner wants isolated, every time, and wants to know which one is selected before the first order, not after.</p><p><strong>The fee</strong> is charged on the full position, not on your margin. A taker fee of 0.055% on a $10,000 position is about $11 to open and close - trivial against $10,000, and 11% of your money if your margin was $100. High leverage quietly multiplies your fee bill against your own capital, and that alone turns a break-even strategy into a losing one.</p>'],
      ['Why almost everybody lands in the same band, and what it costs them', '<p>Look at the sample column in the table above: the middle bands hold most of the trades. That is not a coincidence. Exchanges default there, the sizing feels significant without feeling insane, and it is where a modest account can still open a position big enough to be interesting.</p><p>It is also where the liquidation rate stops being small. The step from the low bands to the middle is where the share of trades that end in a forced close first becomes a routine outcome rather than an accident - and the step above it is where the median trade stops making money at all.</p><p>The honest reading is not "leverage is bad". It is that leverage is a <em>time limit</em>. The more you take, the less room the market has to be temporarily wrong before you are permanently out, and beginners are not usually wrong about direction so much as about timing.</p>'],
      ['A number you can actually start with', '<p><strong>Start at 3x to 5x and do not move up until your record says you should.</strong> Not because it is virtuous, but because at 5x a 20% move is your stop-out distance, and 20% is more than most majors do in a week - which means the market has to genuinely disagree with you, not merely wobble, before you are removed.</p><p>Then size the position from the risk, not from the leverage. Decide what you are willing to lose on the trade - 1% to 2% of the account is the conventional answer and it is conventional because it survives a losing streak - put the stop where the idea is wrong, and let those two numbers set the position. Leverage becomes an output of that arithmetic rather than a dial you pick first. The <a href="/calculators?c=liq">liquidation calculator</a> will show you exactly where the position dies before you open it.</p><p>And check the funding rate before holding anything leveraged overnight. A crowded side pays the other one every few hours, and at high leverage that bill lands against your margin rather than against the position.</p>'],
      ['Find your number where it costs nothing', '<p>Every figure on this page came from people doing exactly this on <a href="/paper-trade">MarginPad’s paper terminal</a> - real live prices, real fees on both legs, funding on held positions, and liquidation checked against one-minute candle extremes rather than closes, so a wick that retraced still takes the position the way it would on a real venue.</p><p>Run the leverage you are considering for a couple of weeks and read your own <a href="/trading-report/">trading report</a>: it will tell you your win rate, your worst leverage band and your worst hour of the day, from your own closes rather than from a feeling. If your record at 25x is worse than your record at 5x - and for most people it is - that is the answer, and it cost nothing to find out.</p><p>One caveat about the numbers above, stated plainly: the traders in them are risking simulated money, which makes them bolder than they would be with their own. Read the high bands as an upper bound on what recklessness costs, not as a forecast for a funded account.</p>'],
    ],
    related: [['/paper-trade', 'practice at your chosen leverage, free'], ['/calculators?c=liq', 'liquidation price calculator'], ['/practice-for-a-funded-account/', 'practising for a funded account'], ['/academy/', 'the Academy, from the words up']],
    faq: [
      ['What leverage should a beginner use in crypto futures?', 'Start at 3x to 5x. At 5x it takes roughly a 20% move against you to wipe the margin, which is more than most major coins do in a week - so the market has to genuinely disagree with you before you are removed, rather than merely wobble. MarginPad measures this on its own closed trades: the share of trades ending in liquidation rises sharply through the middle bands, and above 100x the median trade loses the entire margin. The live figures are at /api/leverage.'],
      ['Is 10x leverage too much for a beginner?', '10x is survivable but unforgiving: it takes about a 10% move against you to end the position, and majors cover that in a bad day. It is a reasonable place to arrive after a few weeks of records at 3-5x that show you can hold a stop, and a poor place to start. The distance to liquidation is roughly 1 divided by the leverage, minus the maintenance margin the venue holds back.'],
      ['What percentage of leveraged trades get liquidated?', 'It depends almost entirely on the leverage. On MarginPad, measured across 30 days of closed trades on real live prices with fees, funding and wick-checked liquidation, the share ending in a forced close is a fraction of a percent in the lowest band and rises past half of all trades above 100x. The current figures, with the sample size for each band, are published at /api/leverage - a band with fewer than 200 closes is marked thin and no rate is printed for it.'],
      ['Does high leverage increase profit?', 'It increases both sides equally on the same price move, and then subtracts: the taker fee is charged on the whole position rather than on your margin, so at high leverage the round trip is a large share of the money you actually put up, and funding on a held position lands against that margin too. This is why the median return in the high bands measured here is negative while the median in the low bands is positive.'],
      ['Should a beginner use cross or isolated margin?', 'Isolated. It caps the loss at what you committed to that one trade, while cross margin lets a single losing position reach into the rest of the account. Check which mode is selected before the first order rather than after, because the default is not the same on every venue.'],
    ],
  },
  {
    // 2026-09-21. The commercial question with the best answer on this site, for one structural reason: every firm
    // that sells a funded-account challenge has a reason NOT to publish its pass rate, and we sell no challenges,
    // so we can publish ours. The number comes from /api/funded - our own closed trades scored against a real
    // one-step rule set - so the page cannot drift from the engine the way typed-in prose does.
    slug: 'practice-for-a-funded-account',
    kind: 'funded',
    h1: 'Where can I practice for a funded account challenge?',
    title: 'Where to Practice for a Funded Account Challenge - Free, With Our Own Pass Rate | MarginPad',
    desc: 'Practice a prop-firm challenge free on real live prices - fees, funding and liquidation all simulated server-side. We publish how many of our own traders would pass under a real one-step rule set, because we sell no challenges.',
    lead: 'Answered with our own measurement: how many traders here would have passed a standard one-step challenge this season, under which rules, and what actually ends most runs.',
    dataset: ['MarginPad funded-account scan', 'Every account that closed at least one paper trade in the current 14-day season, scored in time order against a one-step prop-firm rule set: profit target, maximum daily loss, maximum drawdown from peak equity, and a minimum number of trading days. A verdict is decided the moment a rule breaks, not at the end of the season.', 'https://marginpad.io/api/funded'],
    body: [
      ['What a funded account actually is, before you pay for one', '<p>A proprietary trading firm sells you an <strong>evaluation</strong>. You pay a one-time fee - roughly $50 for a small account, up to around $1,000 for a large one - and you are given a trading account with a profit target and a set of loss limits. Hit the target without breaking a limit and you are "funded": you trade the firm’s capital and keep most of the profit, commonly 80% to 90%.</p><p>One thing is worth knowing before you pay, and most firms are quiet about it: <strong>the account is almost always simulated too.</strong> The firm is not usually placing your orders on an exchange. It pays winners out of the fee pool collected from everyone who did not pass. That is not a scandal - it is the business model, and the published rules are honest about the rules if not about the odds - but it does mean the thing you are paying for is <em>permission to be measured</em>, not access to capital.</p><p>Which is exactly why practicing first costs you nothing but time, and skipping it costs the fee.</p>'],
      ['The rule that ends most runs is not the profit target', '<p>Every challenge has three numbers and one of them does almost all the damage.</p><p><strong>The profit target</strong> (commonly 8-12%) is the one people focus on. It is rarely the problem.</p><p><strong>The maximum drawdown</strong> (commonly 6-10%) is measured <em>from your peak equity</em>, not from your starting balance. This catches people out constantly: run an account to +8% and a 6% drawdown rule now stops you at +1.6%, not at -6%. Every dollar you make tightens the floor underneath you.</p><p><strong>The maximum daily loss</strong> (commonly 3-5%) resets each day and is the one that actually ends runs. It does not care that you were up yesterday. One bad session inside a single day - a revenge trade after a stop, a position sized for a recovery - is the most common way a challenge dies, and our own measurement says the same thing: the daily rule breaks more runs here than the drawdown rule does.</p><p>Some firms add a <strong>consistency rule</strong> (no single day may be more than 20-40% of total profit), which quietly disqualifies the one-big-winner style that gets most people to the target in the first place. Read that clause before you pay; it is the most common reason a passed challenge does not pay out.</p>'],
      ['What we measured, and why we can publish it', '<p>Every trader on MarginPad is practicing with simulated money on real live prices. That gives us a population nobody selling challenges has an incentive to report on: real traders, real prices, real fees and funding, a real liquidation engine - and no fee collected from any of them.</p><p>So we run the scan and publish what comes out. Each account’s closed trades are replayed in time order on a notional $10,000 book and scored against a one-step rule set: <strong>+10% target, 4% maximum daily loss, 6% maximum drawdown from peak, and at least 5 trading days.</strong> A verdict is decided the moment a rule breaks - a breach cannot be undone by a recovery later in the week, which is precisely how a real challenge behaves and precisely why "best return on the leaderboard" is a different question from "would have passed".</p><p>Two things about the number are worth stating plainly. It is measured on people with <strong>nothing at stake</strong>, so it is a floor, not a forecast - the same person with a paid fee on the line generally trades tighter. And accounts that moved more than the entire book on a single trade are set aside rather than scored, because a sequence that cannot be executed on a $10,000 account is not a failed challenge, it is a different account.</p>'],
      ['How to practice so that it transfers', '<p>Practicing without the constraints is what produces a trader who blows a challenge in two days having "done well in demo for months". Four things make a practice run mean something.</p><p><strong>Impose the limits before you need them.</strong> Write down the daily loss number and the drawdown-from-peak number and stop for the day when you reach them. The rule you have never once obeyed in practice is not a rule you will obey with a fee on the line.</p><p><strong>Turn realistic fills on.</strong> By default a paper fill lands on the live price at a flat maintenance rate, which is right for learning and optimistic for proving something. <a href="/trading-api/#realism">Switch on slippage and size-tiered maintenance margin</a> and the fill moves against you the way a real book does, while your liquidation moves closer as the position grows. Any result you intend to act on should be measured with these on.</p><p><strong>Trade the size you would actually be given.</strong> A challenge account is $5,000 to $200,000 with modest leverage. Practising at 100x on a $50 balance trains the exact reflex - recovering a loss with size - that the daily-loss rule exists to catch. Our own scan is blunt about this: the accounts that fail fastest are the ones trading margins the book could not contain.</p><p><strong>Keep the record.</strong> The <a href="/trading-report/">trading report</a> gives you win rate, expectancy, and your own worst leverage and time-of-day buckets from your real closes. A firm will not tell you why you failed. That will.</p>'],
      ['Then, if you do go and pay', '<p>Practise until you can produce the target without touching the limits, more than once. Then read the firm’s rules for the three clauses that decide whether a pass turns into money: the <strong>consistency rule</strong>, the <strong>payout schedule and proof</strong> (are payouts published, and how long do they take), and what happens to your account after the first payout. Prefer firms that state their drawdown as static rather than trailing if you are new - a trailing drawdown that follows your peak intraday is materially harder than the same number measured on closed equity.</p><p>And treat the fee as the cost of a measurement, not an investment. On the published numbers across the industry, most people pay it more than once.</p>'],
    ],
    related: [['/paper-trade', 'start practicing now, free'], ['/leaderboards/', 'the live boards and who is ahead'], ['/trading-report/', 'your own stats from real closes'], ['/where-can-i-test-a-trading-bot/', 'testing a bot instead of trading by hand']],
    faq: [
      ['Where can I practice for a funded account challenge for free?', 'On MarginPad. You trade a simulated futures account on real live crypto prices, with taker fees on both legs, funding on held positions, and liquidation checked against one-minute candle extremes. There is no deposit, no card and no KYC. Impose the challenge limits on yourself - a profit target, a maximum daily loss and a maximum drawdown from peak equity - and you are running the same test a prop firm sells, at no cost.'],
      ['What percentage of traders pass a funded account challenge?', 'No firm that sells challenges publishes its own pass rate, so any figure quoted from one is an estimate. Industry-wide estimates commonly put it at 5-10%. MarginPad publishes a measured figure for its own traders at /api/funded: every account that closed trades in the current season, scored in time order against a +10% target with a 4% daily loss limit, a 6% maximum drawdown from peak and a 5 trading-day minimum. Read it as a floor rather than a forecast, because our traders have no fee at stake.'],
      ['Is a funded account real money?', 'The evaluation account is simulated at essentially every firm, and the funded account usually is too - the firm pays winners from the pool of fees collected from everyone who did not pass. Your payouts are real; the account you trade generally is not. This is normal for the industry rather than a sign of a bad firm, but it is worth knowing that what you are buying is permission to be measured, not access to capital.'],
      ['Which challenge rule fails most people?', 'The maximum daily loss. It resets every day and ignores how far ahead you were, so a single bad session - typically a revenge trade after a stop, sized to recover it - ends the run. In our own scan of MarginPad traders under a one-step rule set, the daily-loss rule breaks more runs than the maximum-drawdown rule does. The profit target is rarely what stops people.'],
      ['How long should I practice before paying for a challenge?', 'Until you can reach the target without touching the limits, more than once, with realistic fills switched on and at a position size a challenge account would actually give you. There is no fixed number of weeks - the point is repeatability. Practising is free and a challenge fee is $50 to $1,000, so the arithmetic favours proving it first.'],
    ],
  },
  {
    // The agent-facing question (2026-09-16). Measured first: over 30 days /trading-api/ took ZERO crawler hits
    // while bingbot made 28,308, and the API pages brought 53 of 2,567 assistant referrals. The narrow pages in
    // this family are the shape that gets LINKED rather than harvested, and "is there an MCP server for crypto
    // trading" is the exact question an assistant is asked before it recommends anything.
    slug: 'mcp-server-for-crypto-trading',
    kind: 'mcp',
    h1: 'Is there an MCP server for crypto trading?',
    title: 'Is There an MCP Server for Crypto Trading? - Free, 28 Tools | MarginPad',
    desc: 'Yes. MarginPad runs a remote MCP server at https://marginpad.io/mcp: live crypto market data with no API key, plus a full futures paper-trading account for Claude, ChatGPT or Cursor. Nothing there can touch real money.',
    lead: 'Answered from the server’s own tool table: what the MCP endpoint is, which tools need a key, and what an agent connected to it can and cannot do.',
    dataset: ['MarginPad MCP server (tool catalogue)', 'The tools a Model Context Protocol client receives from https://marginpad.io/mcp on tools/list: keyless market-data tools for price, klines, funding, open interest, liquidations, the screener and the economic calendar, plus paper-trading tools that operate a simulated futures account behind a free API key.', 'https://marginpad.io/.well-known/mcp.json'],
    body: [
      ['What you actually connect to', '<p>The Model Context Protocol lets an assistant call tools on a server instead of guessing from training data. MarginPad runs one at <code>https://marginpad.io/mcp</code> over Streamable HTTP, so there is nothing to install, nothing to host and no local process to keep alive. In a client that accepts remote MCP servers the whole configuration is one object:</p><p><code>{"mcpServers":{"marginpad":{"url":"https://marginpad.io/mcp"}}}</code></p><p>A machine-readable descriptor sits at <a href="/.well-known/mcp.json">/.well-known/mcp.json</a> if your tooling prefers to discover it rather than be told.</p>'],
      ['Two halves: reading the market, and trading a paper account', '<p>The market-data half needs <strong>no key at all</strong>. An assistant can ask for the price of BTC, the last hour of candles, current funding across venues, open interest, the long/short split, today’s liquidations, a scored screener or the economic calendar, and get a measured number back. Those calls are keyless because the data is ours to give: the liquidations come from our own collector subscribed to nine exchanges’ public websockets, not resold from a vendor.</p><p>The trading half operates a <strong>paper futures account</strong>: open a leveraged position at the live price, close part of it, move a stop, rest a limit or stop order, read the balance, the open book and the trading report. Those tools take a free API key in an <code>X-API-Key</code> header, minted from <a href="/trading-api/">the Bot API page</a> with an email and nothing else.</p><p>The split matters for trust. An agent can be given the reading half with no credential at all, and the trading half only when you actually want it placing orders.</p>'],
      ['What an agent connected to this cannot do', '<p>It cannot deposit, withdraw, or touch an exchange account, because there is no route to one: MarginPad holds no custody, takes no deposits and asks for no exchange API keys. Positions settle on our servers in simulated dollars against real live prices.</p><p>That is the whole reason to hand an agent this endpoint rather than a live exchange key. An agent that misreads a signal, loops on an order, or sizes a position ten times too large costs you a log line here. The same mistake on a funded account costs the account. Forward-test the agent where the consequences are removed and the prices are not.</p>'],
      ['Why the answers are worth more than a scrape', '<p>An assistant answering from a scraped page is quoting whatever was cached the last time a crawler passed. These tools return the figure at the moment of asking, with a timestamp, from the same feed our own charts and paper terminal run on.</p><p>Two habits are worth building into any agent that uses this: call <code>get_economic_calendar</code> for event dates rather than answering from memory, since a model’s idea of when CPI lands is frequently a year stale, and call <code>get_markets</code> before sizing a trade, because leverage caps and fee rates differ per symbol and asset class.</p>'],
      ['If you would rather not use MCP', '<p>Everything the MCP server does is also a plain HTTP call. The same account is reachable over REST, the OpenAPI 3.1 spec is at <a href="/openapi.json">/openapi.json</a>, positions can be streamed over WebSocket instead of polled, and signed webhooks push fills and liquidations to a URL you control. Official clients with no dependencies install with <code>pip install marginpad</code> or <code>npm install marginpad</code>.</p><p>The free plan carries all of it: 120 requests a minute per key, three keys, fifty open positions, the WebSocket stream, replay of a past day and every keyless market-data endpoint. Paid plans raise ceilings and add webhooks and AI market reads - they do not unlock the product.</p>'],
    ],
    related: [['/trading-api/', 'the full Bot API reference'], ['/where-can-i-test-a-trading-bot/', 'where to test a trading bot'], ['/free-crypto-api/', 'the keyless market-data API'], ['/api-docs/', 'try the calls live']],
    faq: [
      ['Is there an MCP server for crypto trading?', 'Yes. MarginPad runs a remote MCP server at https://marginpad.io/mcp over Streamable HTTP, with 28 tools. The market-data tools - price, klines, funding, open interest, liquidations, the screener, the economic calendar - need no API key at all. The paper-trading tools operate a simulated futures account and take a free key in an X-API-Key header. Nothing there can touch real money.'],
      ['How do I add it to Claude, ChatGPT or Cursor?', 'Add it as a remote MCP server with the URL https://marginpad.io/mcp. In a config file that is {"mcpServers":{"marginpad":{"url":"https://marginpad.io/mcp"}}}. There is nothing to install and no local process to run. A descriptor for tooling that discovers servers automatically is at https://marginpad.io/.well-known/mcp.json.'],
      ['Can an AI agent place real trades through it?', 'No, and that is deliberate. MarginPad takes no deposits, holds no custody and never asks for an exchange API key, so there is no path from this server to real money. Positions are simulated dollars settled against real live prices, which is what makes it safe to let an agent trade while you are still finding out how it behaves.'],
      ['Does it cost anything?', 'No. Market data is keyless and free, and the free API plan carries the whole paper-trading engine: 120 requests a minute per key, three keys, fifty open positions, the WebSocket stream and replay. Paid plans ($29, $79 and $159 a month) raise those ceilings and add webhooks and AI market reads.'],
