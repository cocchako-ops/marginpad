...
(async () => {
  ...
  await withBrowser(async (browser) => {
...

    {
      ...
      chk('member: strip chips carry tier, call, goals, happy hour', m.chips.length >= 4 && m.chips.some(c => /tier 4/.test(c)) && m.chips.some(c => /call made/.test(c)) && m.chips.some(c => /goals 1\/2/.test(c)), m.chips);
