...
(async () => {
  ...
  chk('page: the pro track sums itself up in chips (Ticks, $, items, supplies, Premium, gifts) and every extra is drawn in its tier cell', member && member.sum >= 6 && /\$1\.00/.test(member.sumText) && /Premium/.test(member.sumText) && /skin gift/.test(member.sumText) && member.usdCells === 5 && member.giftCells === 2 && member.supCells === 4 && member.premCells === 1, member && { sum: member.sum, text: member.sumText, usd: member.usdCells, gift: member.giftCells, sup: member.supCells, prem: member.premCells });
