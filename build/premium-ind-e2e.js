  ...
  await withBrowser(async (browser) => {
    ...
    for (const vp of [{ w: 1366, h: 860, t: 'desktop' }, { w: 390, h: 800, t: 'phone' }]) {
      ...
      const g = await page.evaluate(() => {
        ...
        return { reach: !!(hit && card.contains(hit)), cta: !!document.querySelector('.cta-row') && (function(){var e=document.querySelector('.cta-row');return e?getComputedStyle(e).display:'none';})() !== 'none', status: !!document.getElementById('pmStatus') && document.getElementById('pmStatus').hidden, sheetHidden: document.getElementById('indSheet').hidden };
      });
    ...
    }
    {
      ...
      const m = await page.evaluate(() => { const st = document.getElementById('pmStatus'); const r = st.getBoundingClientRect(); const hit = document.elementFromPoint(r.left + r.width / 2, r.top + r.height / 2); const vv=(p)=>[...document.querySelectorAll('[data-buy="'+p+'"]')].filter(b=>b.offsetParent&&getComputedStyle(b).display!=='none');const vis=(p)=>vv(p).filter(b=>getComputedStyle(b).pointerEvents!=='none').length;const inert=(p)=>vv(p).filter(b=>getComputedStyle(b).pointerEvents==='none').length; return { plusBuy: vis('plus'), monthlyBuy: vis('monthly'), monthlyHeld: inert('monthly'), shown: !st.hidden, reach: !!(hit && st.contains(hit)), until: (document.getElementById('pmUntil') || {}).textContent, cta: (function(){var e=document.querySelector('.cta-row');return e?getComputedStyle(e).display:'none';})(), head: (document.querySelector('section .sh .k') || {}).textContent, final: (function(){var e=document.querySelector('.final');return e?getComputedStyle(e).display:'none';})(), member: document.body.classList.contains('pm-member') }; });
      chk('member: status card shown and reachable with the expiry + days left, header says WHAT YOU HAVE, and the PLUS buttons stay reachable so a Premium member can still upgrade, and the Premium button reads as held rather than vanishing', m.shown && m.reach && /Active until/.test(m.until) && /(19|20|21) days left/.test(m.until) && m.head === 'WHAT YOU HAVE' && m.member && m.plusBuy > 0 && m.monthlyBuy === 0 && m.monthlyHeld > 0, m);
