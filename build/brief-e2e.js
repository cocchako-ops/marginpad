  ...
  try {
    ...
    await withBrowser(async (browser) => {
      for (const [w, h, label] of [[1366, 900, 'DESKTOP'], [390, 780, 'PHONE']]) {
        ...
        chk(label + ': the brief opens with a bias line and the reading-order sections (positions, week, next up, setups, majors)', !!m && /Market/.test(m.bias) && ['Your positions', 'Your week', 'Next up', 'Where the setups are', 'Majors'].every(s => m.secs.indexOf(s) >= 0), m && { bias: m.bias, secs: m.secs, w: m.w });
