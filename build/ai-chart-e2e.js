  ...
  await withBrowser(async (browser) => {
    { // desktop
      ...
      const d = await page.evaluate(() => {
        ...
        const last = w.bars.length - 1, zone = ai.filter(s => s.t === 'rect')[0], arw = ai.filter(s => s.t === 'path')[0], txts = ai.filter(s => s.t === 'text');
        ...
        return { width: Math.round(p.getBoundingClientRect().width), card: !!card, bias: card && card.getAttribute('data-bias'), rows: card ? card.querySelectorAll('.aipr').length : 0, lines: (w._aiPlan || []).length, planRects: ov.filter(s => s.t === 'rect').length, planTexts: ov.filter(s => s.t === 'text').length, aiKinds: ai.map(s => s.t), aiTexts: txts.map(s => s.txt), aiBatch: ai.length ? ai.every(s => s.aiB === ai[0].aiB) : false, rsi: !!w.inds.rsi, subShown: !!(sub && sub.style.display !== 'none' && sub.offsetHeight > 30), receipts: [...p.querySelectorAll('.aiacts span')].map(s => s.textContent), undo: !!p.querySelector('.aiundo'), chips: [...p.querySelectorAll('.cwin-ai-chips .cwin-ai-chip')].map(c => c.textContent), sym: w.sym, tf: w.tf, price: +w.bars[w.bars.length - 1].close, prose: (p.querySelector('.aimsg.ai .aitxt') || {}).textContent || '',
          ...
          arrowStartsAtMarket: !!arw && arw.pts && (last - arw.pts[0].l) <= 12, arrowEndsAhead: !!arw && arw.pts && arw.pts[arw.pts.length - 1].l > last,
          pathPts: arw && arw.pts ? arw.pts.length : 0, pathIntegers: !!arw && arw.pts.every(q => Number.isInteger(q.l)), pathCurves: (function () { if (!arw || !arw.pts || arw.pts.length < 5) return false;
            const ps = arw.pts.map(q => q.p), a = ps[0], b = ps[ps.length - 1]; let maxDev = 0;
            for (let i = 1; i < ps.length - 1; i++) { const lin = a + (b - a) * (i / (ps.length - 1)); maxDev = Math.max(maxDev, Math.abs(ps[i] - lin)); }
            return maxDev / Math.abs(b - a || 1) > 0.02; })(),
      ...
      chk('desktop: the model drew with the drawing engine - a ray, a zone band, a level and a forward path with their labels, tagged by:ai, one batch', d.aiKinds.indexOf('ray') >= 0 && d.aiKinds.indexOf('rect') >= 0 && d.aiKinds.indexOf('hline') >= 0 && d.aiKinds.indexOf('path') >= 0 && d.aiTexts.indexOf('Downtrend') >= 0 && d.aiTexts.indexOf('Resistance') >= 0 && d.aiBatch, { kinds: d.aiKinds, texts: d.aiTexts, batch: d.aiBatch });
      ...
      chk('desktop: GEOMETRY - the expected path starts at the newest candles and runs into the future, never a diagonal across the whole chart', d.arrowStartsAtMarket && d.arrowEndsAhead, { start: d.arrowStartsAtMarket, end: d.arrowEndsAhead, pts: d.pathPts });
      chk('desktop: the path is a CURVE the chart shaped, sampled one point per candle on WHOLE logical indices (a fractional index projects to x=0 in Lightweight Charts and painted lines across the whole chart)', d.pathPts >= 5 && d.pathIntegers && d.pathCurves, { pts: d.pathPts, integers: d.pathIntegers, curved: d.pathCurves });
