...
(async () => {
  ...
  const drawOk = draws.length >= 1 && draws.every(a => { if (a.shape === 'hline' || a.shape === 'text') return near(+a.p); if (a.shape === 'zone') return near(+a.from) && near(+a.to); if (a.shape === 'vline') return true; return near(+a.p1) && near(+a.p2); });
  ...
  await withBrowser(async (browser) => {
    { // desktop
      ...
      const d = await page.evaluate(() => {
        ...
        const last = w.bars.length - 1, zone = ai.filter(s => s.t === 'rect')[0], arw = ai.filter(s => s.t === 'arrow')[0], txts = ai.filter(s => s.t === 'text');
        ...
        return { width: Math.round(p.getBoundingClientRect().width), card: !!card, bias: card && card.getAttribute('data-bias'), rows: card ? card.querySelectorAll('.aipr').length : 0, lines: (w._aiPlan || []).length, planRects: ov.filter(s => s.t === 'rect').length, planTexts: ov.filter(s => s.t === 'text').length, aiKinds: ai.map(s => s.t), aiTexts: txts.map(s => s.txt), aiBatch: ai.length ? ai.every(s => s.aiB === ai[0].aiB) : false, rsi: !!w.inds.rsi, subShown: !!(sub && sub.style.display !== 'none' && sub.offsetHeight > 30), receipts: [...p.querySelectorAll('.aiacts span')].map(s => s.textContent), undo: !!p.querySelector('.aiundo'), chips: [...p.querySelectorAll('.cwin-ai-chips .cwin-ai-chip')].map(c => c.textContent), sym: w.sym, tf: w.tf, price: +w.bars[w.bars.length - 1].close, prose: (p.querySelector('.aimsg.ai .aitxt') || {}).textContent || '',
          ...
          arrowStartsAtMarket: !!arw && (last - arw.l1) <= 12, arrowEndsAhead: !!arw && arw.l2 > last };
      });
      ...
      chk('desktop: the model drew with the drawing engine - a ray, a zone band, a level and a forward arrow with their labels, tagged by:ai, one batch', d.aiKinds.indexOf('ray') >= 0 && d.aiKinds.indexOf('rect') >= 0 && d.aiKinds.indexOf('hline') >= 0 && d.aiKinds.indexOf('arrow') >= 0 && d.aiTexts.indexOf('Downtrend') >= 0 && d.aiTexts.indexOf('Resistance') >= 0 && d.aiBatch, { kinds: d.aiKinds, texts: d.aiTexts, batch: d.aiBatch });
      ...
      chk('desktop: GEOMETRY - the arrow starts at the newest candles and points into the future, never a diagonal across the whole chart', d.arrowStartsAtMarket && d.arrowEndsAhead, { start: d.arrowStartsAtMarket, end: d.arrowEndsAhead });
      ...
      const g = await page.evaluate(() => { // the guard rails, run against the real pure function with deliberately bad actions
        ...
        const far = S({ shape: 'arrow', p1: px, barsAgo1: 300, p2: px * 1.02, barsAgo2: -10 });
        const mad = S({ shape: 'hline', p: px * 9 });
        const zr = wall.filter(s => s.t === 'rect')[0], pr = past.filter(s => s.t === 'rect')[0], fr = flat.filter(s => s.t === 'rect')[0], ar = far.filter(s => s.t === 'arrow')[0];
        return { wallH: (zr.p2 - zr.p1) / (hi - lo), pastEnd: pr.l2 - last, flatH: (fr.p2 - fr.p1) / px, stub: stub, madPrice: mad, arrowStart: last - ar.l1 };
      });
      chk('desktop: GEOMETRY guard rails hold on bad input - a full-range zone is clamped to a band, a zone stranded in the past is extended past the newest candle, a flat zone gets a real height, a 2-bar trend and an off-chart price are refused, a 300-bar-back arrow is pulled to the market', g.wallH <= 0.29 && g.pastEnd >= 6 && g.flatH > 0.002 && g.stub === null && g.madPrice === null && g.arrowStart <= 12, g);
    ...
    { // mobile, landscape phone
      ...
      chk('mobile: the actions executed on the pane - RSI on, the model\'s shapes in the pane\'s engine, receipt + the model\'s chips (the default chips offered "Draw the setup" before)', m.rsi && m.aiKinds.indexOf('ray') >= 0 && m.aiKinds.indexOf('rect') >= 0 && m.receipts.some(t => /^Opened RSI/.test(t)) && m.receipts.some(t => /^Drew 4/.test(t)) && m.chips[0] === 'Draw the 4H too' && chips0.some(c => /Draw the setup/.test(c)), { rsi: m.rsi, kinds: m.aiKinds, receipts: m.receipts, chips: m.chips, chips0 });
