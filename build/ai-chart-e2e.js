...
(async () => {
  ...
  const drawOk = draws.length >= 1 && draws.every(a => { if (a.shape === 'hline' || a.shape === 'text' || a.shape === 'level' || a.shape === 'hray') return near(+a.p); if (a.shape === 'zone') return near(+a.from) && near(+a.to); if (a.shape === 'arrow' || a.shape === 'forecast') return near(+a.p2); if (a.shape === 'position') return near(+a.entry) && near(+a.stop); if (a.shape === 'pattern') return (a.points || []).every(q => near(+q.price)); if (a.shape === 'vline') return true; return near(+a.p1) && near(+a.p2); });
  ...
  await withBrowser(async (browser) => {
    { // desktop
      ...
      const d = await page.evaluate(() => {
        ...
        const last = w.bars.length - 1, zone = ai.filter(s => s.t === 'zone')[0], gb = w._ghostBars || [], txts = ai.filter(s => s.t === 'text');
        ...
        return { width: Math.round(p.getBoundingClientRect().width), card: !!card, bias: card && card.getAttribute('data-bias'), rows: card ? card.querySelectorAll('.aipr').length : 0, lines: (w._aiPlan || []).length, planRects: ov.filter(s => s.t === 'rect' || s.t === 'zone').length, planTexts: ov.filter(s => s.t === 'text').length, aiKinds: ai.map(s => s.t), aiTexts: txts.map(s => s.txt), shapeTxts: ai.filter(s => s.txt).map(s => s.t + ':' + s.txt), aiBatch: ai.length ? ai.every(s => s.aiB === ai[0].aiB) : false, rsi: !!w.inds.rsi, subShown: !!(sub && sub.style.display !== 'none' && sub.offsetHeight > 30), receipts: [...p.querySelectorAll('.aiacts span')].map(s => s.textContent), undo: !!p.querySelector('.aiundo'), chips: [...p.querySelectorAll('.cwin-ai-chips .cwin-ai-chip')].map(c => c.textContent), sym: w.sym, tf: w.tf, price: +w.bars[w.bars.length - 1].close, prose: (p.querySelector('.aimsg.ai .aitxt') || {}).textContent || '',
          zoneEndsAfterLast: !!zone && zone.l2 === null, zoneHeightPctOfRange: zone ? (zone.p2 - zone.p1) / (hi - lo) * 100 : -1,
      ...
      chk('desktop: the model drew with the drawing engine - a ray and a zone band carrying their labels AT the shape (so the reader sees where a line starts), a level, all tagged by:ai in one batch', d.aiKinds.indexOf('ray') >= 0 && d.aiKinds.indexOf('zone') >= 0 && d.aiKinds.indexOf('hline') >= 0 && d.shapeTxts.indexOf('ray:Downtrend') >= 0 && d.shapeTxts.indexOf('zone:Long liqs') >= 0 && d.aiTexts.indexOf('Resistance') >= 0 && d.aiBatch, { kinds: d.aiKinds, onShapes: d.shapeTxts, texts: d.aiTexts, batch: d.aiBatch });
      ...
      chk('desktop: GEOMETRY - the zone band runs to the right edge, its height is a band and not a wall, and every label sits in the gutter right of the last candle, right-aligned', d.zoneEndsAfterLast && d.zoneHeightPctOfRange > 0 && d.zoneHeightPctOfRange <= 28 && d.labelsInGutter && d.labelsRightAligned, { zoneEnd: d.zoneEndsAfterLast, h: d.zoneHeightPctOfRange, gutter: d.labelsInGutter, ar: d.labelsRightAligned });
      ...
      const g = await page.evaluate(() => { // the guard rails, run against the real pure function with deliberately bad actions
        ...
        const zr = wall.filter(s => s.t === 'zone')[0], pr = past.filter(s => s.t === 'zone')[0], fr = flat.filter(s => s.t === 'zone')[0];
        ...
        return { wallH: (zr.p2 - zr.p1) / (hi - lo), pastEdge: pr.l2 === null, flatH: (fr.p2 - fr.p1) / px, stub: stub, madPrice: mad,
          slopeH: H, slopeRatio: typical ? +(moved / typical).toFixed(2) : null };
      });
      chk('desktop: GEOMETRY guard rails hold on bad input - a full-range zone is clamped to a band, a zone always runs to the right edge so it can never be stranded in the past, a flat zone gets a real height, a 2-bar trend and an off-chart price are refused, ', g.wallH <= 0.29 && g.pastEdge === true && g.flatH > 0.002 && g.stub === null && g.madPrice === null, g);
    ...
    { // mobile, landscape phone
      ...
      chk('mobile: the actions executed on the pane - RSI on, the model\'s shapes in the pane\'s engine, receipt + the model\'s chips (the default chips offered "Draw the setup" before)', m.rsi && m.aiKinds.indexOf('ray') >= 0 && m.aiKinds.indexOf('zone') >= 0 && m.ghostN >= 15 && m.receipts.some(t => /^Opened RSI/.test(t)) && m.receipts.some(t => /^Drew 4/.test(t)) && m.chips[0] === 'Draw the 4H too' && chips0.some(c => /Find an opportunity/.test(c)), { rsi: m.rsi, kinds: m.aiKinds, receipts: m.receipts, chips: m.chips, chips0 });
