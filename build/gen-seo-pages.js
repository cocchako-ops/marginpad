...
const EX = [
  { name:'Bybit', accent:'#f7a600', fg:'#0a0b0d',   slug:'bybit',   ref:'https://www.bybit.com/invite?ref=LZKBERJ',                                                              lev:100, mmr:0.5, maker:0.02, taker:0.055, founded:2018, region:'Global; not available to residents of the United States, the UK (retail) or a handful of sanctioned regions', us:false, funding:'every 8 hours',
  ...
  { name:'Binance', accent:'#f0b90b', fg:'#0a0b0d', slug:'binance', ref:'https://www.binance.com/register?ref=MAOZM9DS',                                                          lev:125, mmr:0.4, maker:0.02, taker:0.04,  founded:2017, region:'Global via Binance.com; US residents are routed to the separate Binance.US, which does not offer futures', us:false, funding:'every 8 hours (occasionally 4h in extreme funding)',
  ...
  { name:'OKX', accent:'#cfd3da', fg:'#0a0b0d',     slug:'okx',     ref:'https://okx.com/join/96160298',                                                                          lev:125, mmr:0.5, maker:0.02, taker:0.05,  founded:2017, region:'Global; not available to residents of the United States', us:false, funding:'every 8 hours',
  ...
  { name:'KuCoin', accent:'#23af91', fg:'#0a0b0d',  slug:'kucoin',  ref:'https://www.kucoin.com/r/rf/VHP8AYKY',                                                                   lev:100, mmr:0.5, maker:0.02, taker:0.06,  founded:2017, region:'Global; KuCoin has wound down US access, so it is effectively non-US', us:false, funding:'every 8 hours',
  ...
  { name:'Gate', accent:'#17e6a1', fg:'#0a0b0d',    slug:'gate',    ref:'https://www.gate.com/VFIWB10KUG?ref=VFIWB10KUG&ref_type=103&ut-m_cmp=rXJBDjtJ&activity_id=1778642196063', lev:100, mmr:0.5, maker:0.02, taker:0.05, founded:2013, region:'Global; not serving US users', us:false, funding:'every 8 hours',
  ...
  { name:'Kraken', accent:'#6a54f0', fg:'#ffffff',  slug:'kraken',  ref:'https://invite.kraken.com/JDNW/guj2tf28',                                                                lev:50,  mmr:0.5, maker:0.02, taker:0.05, founded:2011, region:'US-friendly — Kraken is a long-regulated US exchange, though futures availability varies by state and country', us:true, funding:'every 4 hours',
  ...
  { name:'Bitget', accent:'#00e7d8', fg:'#0a0b0d',  slug:'bitget',  ref:'https://www.bitget.com/referral/register?clacCode=DSSSQKGK',                                             lev:125, mmr:0.5, maker:0.02, taker:0.06, founded:2018, region:'Global; not available to residents of the United States', us:false, funding:'every 8 hours',
  ...
  { name:'MEXC', accent:'#1966ff', fg:'#ffffff',    slug:'mexc',    ref:'https://promote.mexc.com/r/GND4jI97o0',                                                                                 lev:500, mmr:0.5, maker:0.01, taker:0.04, founded:2018, region:'Global; not available to residents of the United States and several restricted jurisdictions', us:false, funding:'every 8 hours',
...
// Head-to-head link for an exchange calculator page: only a compare page that exists on disk (the pairs are
// generated bybit-first, e.g. /bybit-vs-bitget/, and Bybit vs MEXC does not exist) — six calculators linked
// /<x>-vs-bybit/ 404s until 2026-09-02.
function cmpLink(ex) {
  const other = ex.slug === 'bybit' ? 'binance' : 'bybit', otherName = ex.slug === 'bybit' ? 'Binance' : 'Bybit';
  const dist = path.join(__dirname, '..', 'dist');
  for (const slug of [ex.slug + '-vs-' + other, other + '-vs-' + ex.slug]) {
    if (fs.existsSync(path.join(dist, slug, 'index.html'))) return '<a href="/' + slug + '/">' + ex.name + ' vs ' + otherName + '</a>';
  }
  return '<a href="/exchanges/">the exchange comparison</a>';
}
...
var u='/api/track?t=exchange&e='+encodeURIComponent(ex)+'&p='+encodeURIComponent(location.pathname);var ms='';try{ms=sessionStorage.getItem('mp_src')||'';if(!ms){var f0=JSON.parse(localStorage.getItem('mp_src0')||'null');if(f0&&f0.s&&(Date.now()-(+f0.ts||0))<=7776e6)ms=String(f0.s);}}catch(e9){}if(ms)u+='&src='+encodeURIComponent(ms.slice(0,40));if(navigator.sendBeacon)navigator.sendBeacon(u);
}catch(_){}});});})();
</script>
</body>
</html>
...
<script type="application/ld+json">{"@context":"https://schema.org","@type":"FAQPage","mainEntity":[{"@type":"Question","name":"How do I calculate my liquidation price on ${ex.name}?","acceptedAnswer":{"@type":"Answer","text":"Enter entry price, leverage and direction. Liquidation = Entry x (1 - 1/Leverage + MMR) for a long and Entry x (1 + 1/Leverage - MMR) for a short."}},{"@type":"Question","name":"What is the maximum leverage on ${ex.name}?","acceptedAnswer":{"@type":"Answer","text":"${ex.name} offers up to ${ex.lev}x leverage on flagship perpetuals. The maximum varies by contract, region and account tier."}},{"@type":"Question","name":"What are ${ex.name}'s trading fees?","acceptedAnswer":{"@type":"Answer","text":"${ex.name}'s base USDT-perpetual fees are about ${pct(ex.maker)} maker and ${pct(ex.taker)} taker, before VIP tiers or token discounts. Funding is exchanged between longs and shorts ${ex.funding}."}},{"@type":"Question","name":"Is ${ex.name} available in the US?","acceptedAnswer":{"@type":"Answer","text":"${availText}"}},{"@type":"Question","name":"How do I avoid liquidation on ${ex.name}?","acceptedAnswer":{"@type":"Answer","text":"Use lower leverage, set a stop-loss inside your liquidation level, and size positions from your stop distance rather than a round dollar figure."}}]}</script>`;
  return head({ title, desc, url, crumb: `${ex.name} liquidation calculator`, bcName: `${ex.name} Liquidation Calculator`, ld, hreflang: hreflang(ex.slug + '-liquidation-calculator'),
    ...
    ${compareTable(ex)}
    <p>Higher leverage and a lower maintenance margin both push liquidation closer to your entry, so the "best" venue depends on how much buffer you want. See the full <a href="/best-crypto-futures-exchange/">best crypto futures exchange</a> breakdown, or a head-to-head such as ${cmpLink(ex)}.</p>
