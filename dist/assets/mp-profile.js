...
var __esD_mpprofile = {"loadingTrader": "<div class=\"lbm-load\">Cargando trader…</div>", "noPublicProfileFor": "<div class=\"lbm-load\">Este trader aún no tiene un perfil público.</div>", "titleMessage": "\" title=\"Mensaje\">", "titleChallengeToA": "\" title=\"Retar a un duelo\">", "thisIsYou": "<div class=\"lbm-self\">Este eres tú</div>", "signInToFollow": "<button type=\"button\" class=\"lbm-follow\" data-auth-open>Inicia sesión para seguir</button>", "premiumMember": "MIEMBRO PREMIUM", "seasonBoardWin": " victoria en la tabla de temporada", "seasonWinner": "</svg><span>GANADOR DE TEMPORADA</span></div>", "followsYou": "<span class=\"lbm-fw\" title=\"Te sigue\">Te sigue</span>", "tradesSeason": "Operaciones · temporada", "winRate": "Tasa de acierto", "bestTrade": "Mejor operación", "tradesWk": " operaciones · sem", "thisSeason": "esta temporada", "biggestWin": "mayor ganancia", "inARow": " seguidas", "winStreak": "racha de victorias", "closesInADay": "cierres en un día", "couldNotLoadThis": "<div class=\"lbm-load\">No se pudo cargar este trader.</div>", "pctTo": "% para ", "maxTier": " · nivel máximo"};
function __esT_mpprofile(k, en) { try { if ((document.documentElement.lang || "").slice(0, 2).toLowerCase() === "es" && __esD_mpprofile[k] != null) return __esD_mpprofile[k]; } catch (e) {} return en; }
...
(function () {
  ...
  var CSS = ''
    ...
    + '.lbm-medal{display:flex;align-items:center;gap:9px;margin:0 0 10px;padding:6px 12px 6px 8px;border-radius:14px;border:1px solid rgba(255,215,90,.42);background:linear-gradient(100deg,rgba(255,215,90,.10),rgba(255,215,90,.03));box-shadow:0 0 22px -10px rgba(255,200,60,.8),inset 0 1px 0 rgba(255,255,255,.08)}.lbm-medal svg{flex:none;filter:drop-shadow(0 2px 5px rgba(0,0,0,.45))}.lbm-medal span{font:800 9.5px "Space Mono",monospace;letter-spacing:.16em;color:#ffd75a;white-space:nowrap}.lbm-premtag{display:inline-block;font:800 9px "Space Mono",monospace;letter-spacing:.18em;color:#0a0b0d;background:linear-gradient(90deg,#c2f64a,#e5ff8a);border-radius:20px;padding:3px 11px;margin-bottom:10px}'
  ...
  var TAGC = { bronze: ['#c97f4a', '#f0c39a'], silver: ['#c3cdda', '#eef3fa'], gold: ['#ffcf3f', '#ffe98a'], platinum: ['#8fe6ff', '#d8f6ff'], diamond: ['#b795ff', '#e6d8ff'], legendary: ['#ff8a2a', '#ffcf8a'], unranked: ['#8b95a1', '#c9d2dc'], neon: ['#c2f64a', '#e5ff8a'], aurora: ['#8b5cff', '#5ad0ff'], founder: ['#ffd75a', '#fff3c0'], owner: ['#7a4d0a', '#ffd75a'], default: ['#c2f64a', '#e5ff8a'], carbon: ['#9aa3ad', '#cdd3da'], jade: ['#4ade80', '#a7f3c8'], royal: ['#a06bff', '#d0b5ff'], blood: ['#ff5a4d', '#ff9a8a'], matrix: ['#2ebd85', '#7de8bc'], ice: ['#35c0e8', '#a8f0ff'], ember: ['#8a5a3a', '#ff7a4a'], sakura: ['#ff9ad0', '#ffd0e8'], void: ['#8a6cff', '#c0aaff'], sovereign: ['#f0e6c8', '#fffaf0'], eclipse: ['#ff9a2a', '#ffcf8a'], inferno: ['#ff6a1a', '#ffb056'], streak7: ['#ffb84a', '#ffdf8a'], streak30: ['#ff6a2a', '#ffae4a'], streak100: ['#ffffff', '#dff6f6'], dwell10: ['#d29a6a', '#eccaa8'], dwell100: ['#34d99a', '#9fe9c8'], dwell500: ['#b8895a', '#e0c8a8'], closer: ['#ff5a4d', '#ffb3ad'], closer2k: ['#ff3b30', '#ffd0cc'], operative: ['#a9b96a', '#d5e0a8'], mission500: ['#ffd75a', '#14120a'], og180: ['#b87333', '#5ec8ae'], xp100k: ['#e05a4a', '#ffd75a'], dragonfire: ['#ff5a28', '#ffc060'], singularity: ['#7a5cff', '#9adfff'], midas: ['#ffd75a', '#fff3c0'], realtrader: ['#2ee6a8', '#ffe98a'], champion: ['#ffd75a', '#fff3c0'], deadeye: ['#c3cdda', '#ff6a5e'], overdrive: ['#ff2ad0', '#ff9af0'], tycoon: ['#2ebd85', '#ffd75a'] }; /* member-tag tint follows the equipped frame */
  ...
  window.mpOpenProfile = function (name) {
    ...
    body.innerHTML = __esT_mpprofile("loadingTrader",'<div class="lbm-load">Loading trader…</div>'); card.style.setProperty('--lc', '#c97f4a'); card.className = 'lbm-card';
    ...
    fetch('/api/lb/user?name=' + encodeURIComponent(name)).then(function (r) { return r.json(); }).then(function (d) {
      if (!d || !d.exists) { body.innerHTML = __esT_mpprofile("noPublicProfileFor",'<div class="lbm-load">No public profile for this trader yet.</div>'); return; }
      ...
      var dmBtn = (!isSelf && signedIn()) ? '<button type="button" class="lbm-dm" data-lbdm="' + esc(d.name) + __esT_mpprofile("titleMessage",'" title="Message">') + CHAT_SVG + '</button><button type="button" class="lbm-dm lbm-duel" data-lbduel="' + esc(d.name) + __esT_mpprofile("titleChallengeToA",'" title="Challenge to a duel">') + DUEL_SVG + '</button>' : '';
      var mBtn = '<div class="lbm-acts">' + (isSelf ? __esT_mpprofile("thisIsYou",'<div class="lbm-self">This is you</div>') : (signedIn() ? '<button type="button" class="lbm-follow' + (following ? ' on' : '') + '" data-lbfollow="' + esc(d.name) + '" data-tuid="' + esc(d.uid) + '">' + (following ? 'Following' : 'Follow') + '</button>' : __esT_mpprofile("signInToFollow",'<button type="button" class="lbm-follow" data-auth-open>Sign in to follow</button>'))) + dmBtn + '</div>';
      ...
      var bioHtml = d.bio ? '<div class="lbm-bio"' + '' + '>' + esc(d.bio) + '</div>' : '';
      var coinsHtml = (d.coins && d.coins.length) ? '<div class="lbm-coins">' + d.coins.map(function (c) { return '<span class="lbm-coin"' + '' + '>' + esc(c) + '</span>'; }).join('') + '</div>' : '';
      body.innerHTML =
        (d.otag ? '<div class="lbm-premtag lbm-ownertag">' + d.otag + '</div>' : d.owner ? '<div class="lbm-premtag lbm-ownertag">OWNER</div>' : d.premium ? (function(){var tc=TAGC[String(d.frame||'').replace(/[^a-z0-9_]/g,'')]||TAGC['default'];return '<div class="lbm-premtag" data-sheen style="background:linear-gradient(100deg,'+tc[0]+' 0%,'+tc[1]+' 42%,#ffffff 50%,'+tc[1]+' 58%,'+tc[0]+' 100%);background-size:280% 100%;background-position:120% 0;box-shadow:0 0 16px -6px '+tc[0]+'">' + (d.plus ? 'PREMIUM PLUS' : __esT_mpprofile("premiumMember",'PREMIUM MEMBER')) + '</div>';})() : '') + (function(){var w=d.wins;if(!w||!(+w.n>0))return '';var n=+w.n||0,ttl=n+__esT_mpprofile("seasonBoardWin",' season board win')+(n===1?'':'s')+(w.boards&&w.boards.length?' ('+w.boards.join(', ')+')':'');return '<div class="lbm-medal" title="'+esc(ttl)+'" aria-label="'+esc(ttl)+'">'+'<svg viewBox="0 0 44 56" width="34" height="43" aria-hidden="true">'+'<defs><linearGradient id="mdg" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stop-color="#fff3c0"/><stop offset=".45" stop-color="#ffd75a"/><stop offset=".6" stop-color="#b8860b"/><stop offset="1" stop-color="#ffe98a"/></linearGradient>'+'<linearGradient id="mdr" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#ff5a4d"/><stop offset="1" stop-color="#8f1f18"/></linearGradient></defs>'+'<path d="M13 2h7l-3 17h-8z" fill="url(#mdr)"/><path d="M31 2h-7l3 17h8z" fill="url(#mdr)" opacity=".85"/>'+'<circle cx="22" cy="37" r="16" fill="url(#mdg)" stroke="#7a5a10" stroke-width="1.2"/>'+'<circle cx="22" cy="37" r="11.5" fill="none" stroke="#7a5a10" stroke-width=".9" opacity=".55"/>'+'<text x="22" y="42" text-anchor="middle" font-family="Space Mono, monospace" font-size="13" font-weight="700" fill="#4a3608">'+(n>9?'9+':n)+'</text>'+__esT_mpprofile("seasonWinner",'</svg><span>SEASON WINNER</span></div>');})()
        + '<div class="lbm-head"><div class="lbm-badge">' + svg + '</div><div class="lbm-id"><div class="lbm-name">' + av + (d.premium ? '<span class="mp-progold">' + esc(d.name) + '</span>' : esc(d.name)) + (d.mutual ? '<span class="lbm-friend" title="You follow each other">★ Friends</span>' : (d.followsMe ? __esT_mpprofile("followsYou",'<span class="lbm-fw" title="Follows you">Follows you</span>') : '')) + '</div><div class="lbm-lvl" style="color:' + lc + '">' + esc(L.name || 'Bronze') + (L.stars && window.mpPrestigeSvg ? window.mpPrestigeSvg(L.stars, lc) : '') + (L.next ? ' · ' + (L.pct || 0) + __esT_mpprofile("pctTo", '% to ') + esc(L.next) : __esT_mpprofile("maxTier", ' · max tier')) + '</div></div></div>'
        + bioHtml + coinsHtml
        + '<div class="lbm-bar"><i style="width:' + (L.pct || 0) + '%;background:' + lc + '"></i></div>'
        + '<div class="lbm-grid">'
          + stat((s2.trades || 0), s2.season ? __esT_mpprofile("tradesSeason",'Trades · season') : 'Trades')
          + stat((s2.winRate || 0) + '%', __esT_mpprofile("winRate",'Win rate'), s2.winRate >= 50 ? '#34d99a' : '')
          + stat((s2.bestRoe == null ? '-' : pctC(s2.bestRoe)), 'Best ROE', s2.bestRoe > 0 ? '#34d99a' : '')
          + stat((s2.bestPnl == null ? '-' : moneyC(s2.bestPnl)), __esT_mpprofile("bestTrade",'Best trade'), s2.bestPnl > 0 ? '#34d99a' : '')
  ...
  (function(){ if (window.__mpSheen) return; window.__mpSheen = 1;
    ...
    setInterval(function(){
      ...
      var els = document.querySelectorAll('[data-sheen]');
      if (!els.length) return;
      t = (t + 3) % 260;
      for (var i = 0; i < els.length; i++) els[i].style.backgroundPosition = (120 - t) + '% 0';
    }, 45);
  })();
})();
