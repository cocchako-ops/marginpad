...
(function(){
  ...
  function poll(){if(document.hidden||document.body.getAttribute('data-prod')!=='plan')return;if(_wsT&&Date.now()-_wsT<6000)return;var sym=((document.getElementById('planSym')||{}).value||'');if(!sym)return;fetch('/api/price?symbol='+encodeURIComponent(sym)+(window.__mpPQ?window.__mpPQ(sym,'frm'):''),{cache:'no-store'}).then(function(r){return r.json();}).then(function(pd){if(pd&&pd.price>0&&(!_wsT||Date.now()-_wsT>=6000)){live=+pd.price;liveChg=+pd.chg||0;showLive();calc();}}).catch(function(){});}
...
(function(){
  ...
  window.__mpPQ=window.__mpPQ||function(sym,ctx){try{var s=window.__mpWsSeen,w=(s&&s[sym]&&Date.now()-s[sym]<30000)?1:0;return '&ctx='+ctx+'&ws='+w;}catch(e){return '&ctx='+ctx;}}; /* TEMP pxtag: append call-site + WS-covered bool to /api/price polls; remove with worker PXTAG_UNTIL 2026-08-02 */
  ...
  function refreshKlinesQuiet(){var sym=chartSym,tf=chartTf;if(!candle||document.hidden||loadingMore)return; // don't race loadMore's pagination (setData mid-append → duplicate/non-monotonic bars)
    ...
    fetch('/api/klines?symbol='+encodeURIComponent(sym)+'&interval='+tf,{cache:'no-store'}).then(function(r){return r.ok?r.json():null;}).catch(function(){return null;}).then(function(kd){
      ...
      kd=sanitizeBars(kd);
      try{var _fl=kd[kd.length-1];if(lastBar&&_fl&&_fl.time===lastBar.time){_fl.open=+lastBar.open;_fl.close=+lastBar.close;var _mx=Math.max(+lastBar.open,+lastBar.close),_mn=Math.min(+lastBar.open,+lastBar.close);if(_fl.high<_mx)_fl.high=_mx;if(_fl.low>_mn)_fl.low=_mn;}}catch(e){}/* FORMING bar hardening (2026-07-30, mirror of the mp-charts merge): keep the LOCAL open+close — the server close is cache-stale and setData painting it produced a one-second color flip + value snap until the next live tick repainted the local bar (lastBar is not re-pointed here, so the two writers disagreed). Server high/low kept (phantom-wick cleanup), clamped to contain the local open/close. */
      klCache[sym+'|'+tf]=kd;bars=kd;try{candle.setData(kd);}catch(e){}try{chart.priceScale('right').applyOptions({autoScale:true});}catch(e){}/* re-assert autoscale: after a big move (e.g. a +15% pump) the price scale can lock/drift so data keeps updating but the chart LOOKS frozen — re-fitting every quiet refresh self-heals it */lastBar=kd[kd.length-1];_lgp=lastBar&&lastBar.close||0;_rej=0;_dispP=null;_linesSig=null;try{applySignals();}catch(e){}try{drawLines();}catch(e){}
    }); }
...
(function(){
  ...
  function stage(sym,price,chg){if(!(price>0))return;lastTick=Date.now();lastWsTick=Date.now();try{(window.__mpWsSeen=window.__mpWsSeen||{})[sym]=lastWsTick;}catch(e){}/* TEMP pxtag (worker PXTAG_UNTIL 2026-08-02): per-symbol WS-tick time for the /api/price call-site diagnostic */pend[sym]={p:price,c:chg};if(!pendSched){pendSched=true;requestAnimationFrame(flushPend);}}
