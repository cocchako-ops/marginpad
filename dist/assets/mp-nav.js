...
(function () {
  ...
  var html = '<div class="mpnav-sheet"><div class="mpnav-head"><span class="mpnav-title">' + TR('brBrowse') + '</span><button type="button" class="mpnav-x" aria-label="' + TR('navClose') + '">&#10005;</button></div>'
    ...
    + row('brHeatT', 'brHeatS', '/heatmap', '#ffb347', 'heat', 'heatmap heat map liquidation levels clusters pools zones magnet where price goes')
    + xrow('maps', 'brMapsT', 'brMapsS', '#ff8c5a', 'map', 'liquidation map maps levels clusters btc bitcoin eth ethereum sol solana xrp per coin')
    + '<div class="mpnav-sub" data-sub="maps" hidden>'
    + sub('/liquidations/', 'All liquidation maps &amp; the live hub', 'hub all maps index liquidations overview')
    + sub('/btc-liquidation-map/', 'Bitcoin liquidation map', 'btc bitcoin map levels')
    + sub('/eth-liquidation-map/', 'Ethereum liquidation map', 'eth ethereum map levels')
    + sub('/sol-liquidation-map/', 'Solana liquidation map', 'sol solana map levels')
    + sub('/xrp-liquidation-map/', 'XRP liquidation map', 'xrp ripple map levels')
    + '</div>'
