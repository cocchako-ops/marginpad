...
(function(){
  ...
  function buildMobileChart(){
    ...
    indBtn.addEventListener('click',function(){
      var sheet=el('<div class="cwm-sheet"><div class="cwm-sheet-p"><div class="cwm-sheet-h">Indicators<button class="cwm-sheet-x" type="button">✕</button></div><div>'+INDS.map(function(t){var _lk2=MP_INDS[t[0]]&&!indAllowed();return '<button type="button" class="cwm-ind-opt'+(mw.inds[t[0]]?' on':'')+'" data-ind="'+t[0]+'"><span'+(MP_INDS[t[0]]?' style="color:#c2f64a;font-weight:700'+(_lk2?';opacity:.5':'')+'"':'')+'>'+t[1]+(_lk2?' \ud83d\udd12':'')+'</span><span class="dot"></span></button>';}).join('')+'</div></div></div>');
