...
(function(){
  ...
  var MP_INDS={mom:1,sqz:1,liqr:1,casc:1,brain:1}; // MarginPad-exclusive signals — lime in every menu so they stand out
  ...
  function applyInds(w){ if(!w.candle)return; var bars=w.bars||[];
    ...
    w._brain=null; if(w.inds.brain){try{w._brain=brainCalc(w,bars);}catch(e){}if(w._brain)_addSig('BRAIN',w._brain.mk);}
    w._sigStats=_ss;
  ...
  function buildMobileChart(){
    ...
    indBtn.addEventListener('click',function(){
      var sheet=el('<div class="cwm-sheet"><div class="cwm-sheet-p"><div class="cwm-sheet-h">Indicators<button class="cwm-sheet-x" type="button">✕</button></div><div>'+INDS.map(function(t){return '<button type="button" class="cwm-ind-opt'+(mw.inds[t[0]]?' on':'')+'" data-ind="'+t[0]+'"><span'+(MP_INDS[t[0]]?' style="color:#c2f64a;font-weight:700"':'')+'>'+t[1]+'</span><span class="dot"></span></button>';}).join('')+'</div></div></div>');
