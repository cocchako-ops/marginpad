...
var __esD_mpcharts = {"thisDeviceRanOut":"Este dispositivo se quedó sin espacio, así que se eliminaron de él las operaciones cerradas más antiguas. Las posiciones abiertas y tus estadísticas están a salvo: tus estadísticas viven en tu cuenta, no en este dispositivo.","greenArrowTheTrend":"Flecha verde = la tendencia acaba de girar hacia ARRIBA (posible compra). Flecha roja = giró hacia ABAJO (posible venta). Luego aparece un círculo verde \"TP\" en la vela donde el precio alcanza por primera vez el objetivo de take-profit de esa señal (su tamaño depende de la volatilidad reciente, ATR).","liquidationMagnetOurData":"Liquidation Magnet (datos propios): el precio es atraído hacia grandes bolsas de órdenes de liquidación. Esto dibuja la bolsa dominante ARRIBA (liquidaciones short, rojo) y ABAJO (liquidaciones long, verde) del precio actual; la línea EN NEGRITA es el imán más fuerte — el próximo objetivo más probable. Estimado a partir de nuestro modelo de liquidaciones. Exclusivo de MarginPad.","aFullTrendSystem":"Un sistema de tendencia completo: el cruce de Tenkan (rápida) sobre Kijun (lenta) indica impulso; Span A frente a Span B (la nube) muestra la tendencia de fondo.","aSmoothLineOf":"Una línea suavizada del precio promedio, donde los precios recientes pesan más. Precio por encima de la línea = subiendo; por debajo = bajando.","thePlainAveragePrice":"El precio promedio simple de las últimas velas. Una línea de tendencia sencilla: si el precio está por encima, es alcista.","aSuperSmoothFast":"Una línea de precio promedio súper suave y rápida, con menos oscilación y menos retraso que la EMA/SMA.","theAveragePriceWeighted":"El precio promedio ponderado por el volumen operado. Los grandes traders lo tratan como el precio justo del día.","twoBandsHuggingPrice":"Dos bandas que envuelven el precio. Cerca de la banda superior = caro, cerca de la inferior = barato. Bandas anchas = mercado agitado, estrechas = calma.","bandsBasedOnHow":"Bandas basadas en cuánto suele moverse el precio. Que el precio se salga de ellas indica un movimiento fuerte.","theHighestHighAnd":"El máximo más alto y el mínimo más bajo de las últimas velas. Que el precio rompa por encima = una ruptura alcista.","linesWherePriceOften":"Líneas donde el precio suele rebotar (soporte) o ser rechazado (resistencia).","keyDailyPriceLevels":"Niveles de precio diarios clave (pivote) donde el precio tiende a pausarse o girar.","howMuchWasTraded":"Cuánto se operó en cada vela. Barras altas = mucha gente operando en ese momento.","showsIfTheTrend":"Muestra si la tendencia se acelera o se frena. Por encima de la línea 0 = los compradores tienen el control.","likeRsiForTiming":"Como el RSI para cronometrar giros. Por encima de 80 = sobrecompra, por debajo de 20 = sobreventa.","howMuchPriceUsually":"Cuánto suele moverse el precio por vela. ATR alto = mercado agitado → usa stops más amplios.","williamsRLikeAn":"Williams %R: como un RSI invertido. Cerca de 0 = sobrecompra, cerca de −100 = sobreventa.","showsHowFarPrice":"Muestra qué tan lejos está el precio de su promedio. Por encima de +100 = fuerte impulso alcista; por debajo de −100 = fuerte impulso bajista.","dragToResizeThe":"Arrastra para cambiar el tamaño del panel de indicadores","howThisSignalScored":"Cómo se calificó esta señal en el historial cargado de ESTE símbolo y temporalidad: cada flecha se evalúa 1R contra 1R (objetivo y stop, ambos a 1.5×ATR del cierre de la señal).","u00b7FuelBelowPrice":" · combustible DEBAJO del precio (liquidaciones long): una caída puede extenderse","u00b7FuelAbovePrice":" · combustible ARRIBA del precio (liquidaciones short): una subida puede extenderse","noStrongEdge":"sin ventaja clara","brainDrivenBy":"BRAIN impulsado por: ","theFactorsTheModel":"Los factores a los que el modelo da más peso ahora mismo en este símbolo y temporalidad (aprendidos del historial cargado, walk-forward).","upU00b7Proj":"% al alza · proy ","kNearestNeighbourForecast":"Pronóstico k-vecinos-más-cercanos: las 25 velas pasadas cuyo estado de 8 factores más se parece al actual, y lo que hizo el precio en las 6 velas siguientes (6 horas en un gráfico de 1h). La línea discontinua MEM es el nivel proyectado, el porcentaje indica cuántos análogos subieron. Para una posición: es una tasa base, no una predicción; una tasa de subida del 70% con un movimiento proyectado pequeño indica deriva, no ruptura.","theDominantLiquidationPool":"La bolsa de liquidación dominante en cada lado atrae al precio; la línea en negrita es el imán más fuerte (tamaño x proximidad), estimado a partir de nuestro modelo de escalera de apalancamiento. Para una posición: la bolsa en negrita es la próxima visita más probable, un objetivo si está de tu lado, o el nivel más allá del cual conviene poner el stop si no lo está.","pinnedHere":" fijado aquí","notesPinnedToThis":"<div class=\"cwin-ind-menu-h\">Notas fijadas en este gráfico</div>","titleUnpinFloatAgain":"\" title=\"Desanclar (volver a flotar)\">&#10548;</button></div>","noNotesPinnedHere":"<div class=\"cws-lay-empty\">Todavía no hay notas fijadas aquí. Crea una nota y arrástrala a esta ventana para fijarla.</div>","lockedU2014MarginpadExclus":"Bloqueado — exclusivo de MarginPad. Pídenos que lo desbloqueemos para tu cuenta.","unlockThePremiumIndicators":"Desbloquea los indicadores premium","premiumIndicatorSignIn":"Indicador Premium: inicia sesión y actualiza tu plan para usarlo.","ofTheLast5":"% del volumen de las últimas 5 velas operado en velas alcistas","fromTheSameCandles":"de las mismas velas agregadas x","aLevelHereOutranks":". Un nivel aquí tiene prioridad sobre uno del gráfico visible; indica de qué marco temporal hablas.","squeezeUnusuallyQuietFor":"compresión - inusualmente tranquilo para este mercado, aquí empiezan las rupturas","expansionUnusuallyWildStop":"expansión - inusualmente agitado, se barren los stops y los rangos fallan","liquiditySweepOfThe":"barrido de liquidez del máximo en ","stopsAboveWereTaken":"se barrieron los stops de arriba y el precio cerró de nuevo por debajo - a menudo el techo del movimiento","liquiditySweepOfThe2":"barrido de liquidez del mínimo en ","stopsBelowWereTaken":"se barrieron los stops de abajo y el precio cerró de nuevo por encima - a menudo el suelo del movimiento","failedBreakOfThe":"ruptura fallida de ","priceWentThroughAnd":"el precio lo atravesó y no pudo sostenerlo - el nivel sigue mandando","theLast20Candles":"las últimas 20 velas caben dentro de ","energyIsBuildingFor":"% - la energía se acumula para una ruptura, la dirección aún no está decidida","drawItWithA":"dibújalo con una vline cuando el límite del día importe","theLastSwingHigh":"el último máximo swing","theLastSwingLow":"el último mínimo swing","anchoredVwapTheAverage":"VWAP anclado: el precio promedio que realmente pagó todo el que operó desde ese punto de anclaje, ponderado por volumen. Por encima de él, la masa desde ese anclaje está en ganancia y tiende a defenderlo; por debajo, está en pérdida y tiende a vender ante cualquier fuerza. Es una línea mucho mejor de trazar que una media móvil en la que nadie transaccionó jamás, y el precio reacciona a ella - úsala como nivel, y di a qué anclaje te refieres.","candlesLoadedLimitedRead":" velas cargadas - lectura limitada.","estimatedFromALeverage":"estimado a partir de un modelo de escalera de apalancamiento de posiciones abiertas en estas velas","belowPriceLongLiquidations":"debajo del precio (liquidaciones long)","abovePriceShortLiquidation":"arriba del precio (liquidaciones short)","noStrongLean":"sin inclinación clara","barsago0TheNewest":"barsAgo 0 = la vela más reciente; ","candlesAreLoadedSo":" velas están cargadas, así que barsAgo va de 0-","intoThePastNegative":" hacia atrás; un valor negativo proyecta hacia el futuro (máx -30)","higherHighsAndHigher":"máximos crecientes y mínimos crecientes (estructura alcista)","lowerHighsAndLower":"máximos decrecientes y mínimos decrecientes (estructura bajista)","theTwoFramesAbove":"los dos marcos temporales por encima del que estás viendo, a partir de las mismas velas. Léelos juntos: el marco visible es la entrada, el siguiente indica si el movimiento tiene margen, y el más alto muestra hacia dónde se inclina todo el conjunto. Cuando no coincidan, dilo y deja que eso reduzca tu confianza.","resetsIn":"se reinicia en ","askAiReadsUsed":"Lecturas de Ask AI usadas hoy. El límite se reinicia a las 00:00 UTC.","nDidOnThe":"\n[Hecho en el gráfico: ","nRefusedDoNot":"\n[Rechazado, no vuelvas a pedir esto tal cual: ","notWorthTakingAt":"no vale la pena tomarlo con R:R ","theLevelsForWatching":" - los niveles, para vigilar","waitingForNotA":"esperando - todavía no es una operación","notWorthItThe":"<div class=\"aipc-inv aipc-rr\"><span>No vale la pena</span>El primer objetivo paga ","xTheRiskUnder":"x el riesgo - por debajo del 1.5x que este asistente considera una operación, así que los niveles se marcan para vigilar, no para tomar.</div>","wrongIf":"<div class=\"aipc-inv\"><span>Se invalida si</span>","yourPosition":"<div class=\"aipc-inv\"><span>Tu posición</span>","onChart":"\">En el gráfico</button>","tradeIt":"\">Operar</button>","itsTwoAnchorsAre":"sus dos anclajes están a menos de 4 velas de distancia - dos mechas adyacentes no forman una tendencia, elige pivotes más separados","measureNeedsTwoReal":"measure necesita dos precios reales","aMeasureBoxNeeds":"un cuadro measure necesita que sus dos anclajes estén separados al menos 2 velas","entryOrStopIs":"la entrada o el stop están demasiado lejos del precio actual para ser reales (deben estar entre 0.4x y 2.5x de él)","theFirstTargetPays":"el primer objetivo paga ","xTheRiskUnder2":"x el riesgo, por debajo del ","xFloorMakeIt":"x mínimo - conviértelo en una espera, o busca un objetivo que pague lo suficiente","higherHighsAndHigher2":"máximos crecientes y mínimos crecientes","lowerHighsAndLower2":"máximos decrecientes y mínimos decrecientes","isLockedPremiumIndicator":" está bloqueado (indicador Premium)","switchedTo":"Cambiado a ","zoomedTo":"Ampliado a ","showOnChart":"Mostrar en el gráfico","waitFor":"ESPERAR","wouldStop":"STOP SI","wouldTp":"TP SI","onChart2":"En el gráfico","planDrawnOn":" plan dibujado en ","levelsDrawnOn":"Niveles dibujados en ","tapARowIn":" - toca una fila de la tarjeta para localizar un nivel.","findMeTheBest":"Encuéntrame la mejor oportunidad en este gráfico ahora mismo - ¿dónde estaría una buena entrada, y qué la convertiría en un error?","findAnOpportunity":"Buscar una oportunidad","whereWouldYouWant":"¿Dónde te gustaría que se ejecutara la orden, y por qué no al precio actual?","whereAreTheKey":"¿Dónde están los niveles clave de soporte y resistencia?","whatWouldMakeThis":"¿Qué convertiría esto en un error garrafal?","whatToAvoid":"Qué evitar","howRiskyIsMy":"¿Qué tan riesgosa es mi posición abierta en este gráfico ahora mismo?","myPosition":"Mi posición","reReadNow":"Releer ahora","whyThatEntryAnd":"¿Por qué esa entrada y no el precio actual?","whyThatEntry":"Por qué esa entrada","whatExactlyWouldProve":"¿Qué demostraría exactamente que este plan está equivocado?","whatKillsIt":"Qué lo invalida","whatIsThePlan":"¿Cuál es el plan si en cambio pasa lo contrario?","otherSide":"El otro lado","askAiComesWith":"<div class=\"cwin-ai-gate\"><b>Ask AI viene con Premium.</b><br>Un analista integrado que lee el gráfico que estás mirando - las mediciones reales, no una captura de pantalla - y dibuja la configuración sobre él.<br><b>","readADayWith":" lectura al día con Premium ($11.99/mes)</b>, ","aDayWithPremium":" al día con Premium Plus ($159/mes).<br><button class=\"g-btn\" type=\"button\">Consigue Premium - $11.99/mes</button></div>","unlockAskAiOn":"Desbloquea Ask AI en tus gráficos","signInToUse":"<div class=\"cwin-ai-gate\">Inicia sesión para usar Ask AI - un analista integrado que lee el gráfico que tienes delante y dibuja la configuración sobre él. Viene con Premium: <b>","readADayAt":" lectura al día por $11.99/mes</b>, ","aDayOnPremium":" al día con Premium Plus.<br><button class=\"g-btn\" type=\"button\">Inicia sesión gratis</button></div>","clearThisChartS":"¿Borrar el historial de chat con la IA de este gráfico y los dibujos de la IA?","thoseDrawingsAreAlready":"Esos dibujos ya no están.","giveMeASharp":"Dame una lectura precisa y práctica de este gráfico ahora mismo.","thatWasYour":"Esa fue tu ","readForTodayIt":" lectura de hoy - se reinicia a medianoche UTC. Premium Plus ofrece ","youVeUsedAll":"Has usado todas tus ","aiQuestionsTodayResets":" preguntas a la IA de hoy - se reinicia mañana.","aiIsNotSwitched":"La IA aún no está activada.","couldNotReachAi":"No se pudo contactar con la IA - inténtalo de nuevo.","streamingNotSupportedHere":"La transmisión en vivo no es compatible aquí.","noAnswerCameBack":"<span style=\"color:#ff8a80\">No llegó ninguna respuesta - inténtalo de nuevo.</span>","networkErrorTryAgain":"Error de red - inténtalo de nuevo.","alertRemoved":"Alerta eliminada.","signInFreeTo":"Inicia sesión (gratis) para configurar alertas de precio.","weLlEmailYou":" · te avisaremos por correo · haz clic en la línea para eliminarla","tooManyActiveAlerts":"Demasiadas alertas activas (máx. 25). Elimina algunas en /alerts.","couldnTSetThe":"No se pudo configurar la alerta. Inténtalo de nuevo.","networkErrorAlertNot":"Error de red - no se configuró la alerta.","priceAlert":"Alerta de precio","poorRisksMoreThan":"  MALO - arriesga más de lo que gana","noMatchPressEnter":"<div class=\"cwin-sym-empty\">sin coincidencias - pulsa Enter para usarlo</div>","linkCopied":"Enlace copiado","myChartsOnMarginpad":"Mis gráficos en MarginPad","openAtLeastOne":"Abre al menos un gráfico primero.","nameThisLayout":"Nombra este diseño:","savedLayouts":"<div class=\"cwin-ind-menu-h\">Diseños guardados</div>","titleDeleteAriaLabel":"\" title=\"Eliminar\" aria-label=\"Eliminar\">&#10005;</button></div>","noSavedLayoutsYet":"<div class=\"cws-lay-empty\">Todavía no hay diseños guardados - organiza tus gráficos y luego pulsa “Guardar diseño”.</div>","thePriceYouWant":"El precio al que quieres que se ejecute.","theMarketFillsIf":" el mercado - se ejecuta si ","placeLimitOrder":"Colocar orden límite","openPosition":"Abrir posición","maxTradeSizeIs":"El tamaño máximo de operación es $100,000","enterAnAmount":"Ingresa un monto.","enterALimitPrice":"Ingresa un precio límite.","waitingForTheLive":"Esperando el precio en vivo - inténtalo de nuevo en un segundo.","limitOrdersAreStill":"Las órdenes límite todavía se están cargando - inténtalo de nuevo en un segundo.","isWaitingSeeIt":" está en espera - míralo en <b>My Trades &rarr; Órdenes</b>.","couldNotPlaceThe":"No se pudo colocar la orden.","marketIsClosedYou":" el mercado está cerrado - podrás operarlo cuando reabra.","timesOpenedSeeIt":"&times; abierto - míralo en <b>My Trades</b>.","fetchingPrice":"Obteniendo precio…","couldNotGetPrice":"No se pudo obtener el precio. Inténtalo de nuevo.","quickPaperTrade10005":"<div class=\"cqt-modal\" hidden><div class=\"cqt-panel\"><div class=\"cqt-head\"><span>Paper Trade rápido</span><button class=\"cqt-x\" type=\"button\" aria-label=\"Cerrar\">&#10005;</button></div><label class=\"cqt-l\">Símbolo</label><select class=\"cqt-sym\">","prefilledFromTheAi":"Prellenado desde el plan de la IA - revisa cada número antes de abrir.","liquidationPrice":"<span>Precio de liquidación</span><b style=\"color:#ff6258\">","liquidationPrice2":"<span>Precio de liquidación</span><b>-</b>","positionSize":"<span>Tamaño de posición</span><b>","positionSize2":"<span>Tamaño de posición</span><b>-</b>","entryPrice":"Precio de entrada","exitPrice":"Precio de salida","marginUsd":"Margen (USD)","accountBalanceUsd":"Saldo de la cuenta (USD)","stopPrice":"Precio de stop","alignedTechnicalSignalsOn":"<li>Señales técnicas alineadas en el gráfico de 4 horas.</li>","stopLoss":"Stop-loss","riskReward":"Riesgo : beneficio","xIsExtremeRisk":"x es riesgo extremo - un ~","moveAgainstYouLiquidation":"% de movimiento en tu contra = liquidación. Usa siempre el stop y opera con tamaños pequeños.</div>","drawOnChart":"Dibujar en el gráfico","myTradesTrade":"</div><div class=\"cwm-chart\"></div><div class=\"cwm-foot\"><button class=\"cwm-mt\" type=\"button\" hidden>My Trades</button><button class=\"cwm-trade\" type=\"button\">Operar ","lockedU2014MarginpadExclus2":"Bloqueado — exclusivo de MarginPad. Pídenos que lo desbloqueemos."};
function __esT_mpcharts(k, en) { try { if ((document.documentElement.lang || "").slice(0, 2).toLowerCase() === "es" && __esD_mpcharts[k] != null) return __esD_mpcharts[k]; } catch (e) {} return en; }
  ...
  try{
    ...
    while(keep>0){
      ...
      try{
        ...
        if(!window.__mpQuotaSaid){window.__mpQuotaSaid=1;try{if(window.mpLimitToast)window.mpLimitToast(__esT_mpcharts("thisDeviceRanOut",'This device ran out of storage, so the oldest closed trades were removed from it. Open positions and your stats are safe - your stats live on your account, not on this device.'));}catch(_){}}
        return true;
      }catch(e2){}
    }
...
(function(){
  ...
  var INDS=[['sig','Buy / Sell signals (+ TP)'],['casc','Cascade Radar'],['brain','Market Brain (adaptive AI)'],['memory','Market Memory (AI forecast)'],['magnet','Liquidation Magnet'],['ema','EMA'],['sma','SMA'],['hma','Hull MA (21)'],['vwap','VWAP'],['bb','Bollinger Bands'],['kc','Keltner Channels'],['dc','Donchian Channel'],['ichi','Ichimoku Cloud'],['psar','Parabolic SAR'],['sr','Support / Resistance'],['pp','Pivot Points'],['vol','Volume'],['rsi','RSI (14)'],['macd','MACD (12/26/9)'],['stoch','Stochastic (14/3)'],['atr','ATR (14)'],['wr','Williams %R (14)'],['cci','CCI (20)']];
  var MP_INDS={casc:1,brain:1,memory:1,magnet:1}; // the FOUR Premium indicators since 2026-09-17 (were eight: Momentum shift, Squeeze, Liquidation reversal and Sentiment Flip retired - generic TA or folded into the Brain's factors). Guide: /ai-indicators/
  var INDACC={ok:false};
  ...
  var ITIPS={sig:__esT_mpcharts("greenArrowTheTrend",'Green arrow = the trend just flipped UP (a possible buy). Red arrow = flipped DOWN (a possible sell). A green "TP" circle then appears on the bar where price first reaches the take-profit target for that signal (sized by recent volatility, ATR).'),memory:'Market Memory (our AI forecast): finds the past bars whose full market state (8 factors) most resembles RIGHT NOW, then shows what happened NEXT on average. The lower panel is the expected 6-bar move, the dashed line is the projected target, and the legend tells you how many analogs matched and how often they went up. It WILL be wrong sometimes \u2014 it is the market\u2019s own memory, an edge, not a crystal ball. Exclusive to MarginPad.',magnet:__esT_mpcharts("liquidationMagnetOurData",'Liquidation Magnet (our data): price is pulled toward big pools of liquidation orders. This draws the dominant pool ABOVE (short liqs, red) and BELOW (long liqs, green) the current price; the BOLD line is the stronger magnet \u2014 the more likely next target. Estimated from our liquidation model. Exclusive to MarginPad.'),brain:'Market Brain (our adaptive AI): blends 8 factors - trend, momentum, volume, liquidation fragility, REAL liquidation flow, funding history, crowd long/short positioning and market structure - and LEARNS which of them actually predict THIS coin on THIS timeframe from the loaded history (walk-forward, no look-ahead). Factors that do not work here weight themselves to zero. BRAIN arrows fire only when the learned blend is strongly one-sided, and stay silent around high-impact macro events (FOMC/CPI). The legend shows what is driving it right now and its own live hit-rate. Exclusive to MarginPad.',casc:'Cascade Radar (our model): measures how much leveraged-liquidation fuel sits CLOSE to the current price versus how much volume the market can absorb. High readings = fragile market - one push can trigger a chain of forced liquidations. A CASC arrow fires when fragility is extreme and one-sided: red = the fuel is below price (long liqs, price tends to hunt down), green = above (short-squeeze fuel). Exclusive to MarginPad.',ichi:__esT_mpcharts("aFullTrendSystem",'A full trend system: Tenkan (fast) crossing Kijun (slow) signals momentum; Span A vs Span B (the cloud) shows the bigger-picture trend.'),psar:'Trailing dots that flip sides of price. Dots UNDER price = uptrend (trail your stop up); dots ABOVE = downtrend. A flip is an exit/reverse cue.',ema:__esT_mpcharts("aSmoothLineOf",'A smooth line of the average price, but recent prices count more. Price above the line = going up; below = going down.'),sma:__esT_mpcharts("thePlainAveragePrice",'The plain average price over the last bars. A simple trend line - price above it is bullish.'),hma:__esT_mpcharts("aSuperSmoothFast",'A super-smooth, fast average price line - less wobble and less lag than EMA/SMA.'),vwap:__esT_mpcharts("theAveragePriceWeighted",'The average price weighted by how much was traded. Big traders treat it as today’s fair price.'),bb:__esT_mpcharts("twoBandsHuggingPrice",'Two bands hugging price. Near the top band = pricey, near the bottom = cheap. Wide bands = wild, tight = calm.'),kc:__esT_mpcharts("bandsBasedOnHow",'Bands based on how much price usually moves. Price popping outside them signals a strong move.'),dc:__esT_mpcharts("theHighestHighAnd",'The highest high and lowest low of the last bars. Price breaking above = an upside breakout.'),sr:__esT_mpcharts("linesWherePriceOften",'Lines where price often bounces (support) or gets rejected (resistance).'),pp:__esT_mpcharts("keyDailyPriceLevels",'Key daily price levels (pivot) where price tends to pause or turn.'),vol:__esT_mpcharts("howMuchWasTraded",'How much was traded each bar. Tall bars = lots of people trading right then.'),rsi:'A 0–100 speed meter. Above 70 = maybe too high (overbought); below 30 = maybe too low (oversold).',macd:__esT_mpcharts("showsIfTheTrend",'Shows if the trend is speeding up or slowing down. Above the 0 line = buyers in control.'),stoch:__esT_mpcharts("likeRsiForTiming",'Like RSI for timing turns. Above 80 = overbought, below 20 = oversold.'),atr:__esT_mpcharts("howMuchPriceUsually",'How much price usually moves per bar. Big ATR = wild market → use wider stops.'),wr:__esT_mpcharts("williamsRLikeAn",'Williams %R - like an upside-down RSI. Near 0 = overbought, near −100 = oversold.'),cci:__esT_mpcharts("showsHowFarPrice",'Shows how far price is from its average. Above +100 = strong up push; below −100 = strong down push.')};
  ...
  function signalPlan(d){var n=d?d.length:0,P=10,M=3;if(n<P+3)return {mk:[],last:null};
    ...
    var fU=[],fL=[],dir=[],mk=[],last=null,flips=[];for(i=0;i<n;i++){var hl2=(d[i].high+d[i].low)/2,bU=hl2+M*atr[i],bL=hl2-M*atr[i];
      ...
      if(i>P&&cd!==pd){mk.push(cd===1?{time:d[i].time,position:'belowBar',color:'#2ebd85',shape:'arrowUp',text:'BUY'}:{time:d[i].time,position:'aboveBar',color:'#ff6258',shape:'arrowDown',text:'SELL'});last={dir:cd,entry:d[i].close,time:d[i].time,atr:atr[i]};flips.push({i:i,dir:cd,entry:d[i].close,atr:atr[i]});}}
    return {mk:mk,last:last,flips:flips};}
  /* arrows + a TP marker: for each Buy/Sell signal, walk forward and drop a green "TP" circle on the bar where price first reaches the ATR-based target (entry ± 3·ATR). No SL/entry lines. */
  function computeSignals(d){var pl=signalPlan(d),mk=pl.mk.slice(),fl=pl.flips||[];
  ...
  function cascadeCalc(d){var n=d?d.length:0;if(n<60)return null;
    ...
    var alive={},score=new Array(n),dirA=new Array(n),i,k;
    for(i=0;i<n;i++){var b=d[i];
      ...
      var below=0,above=0,px=b.close,R=px*0.05;
      for(k in alive){var a2=alive[k],pr3=(+k+0.5)*binH,dd=Math.abs(pr3-px);if(dd>R)continue;var wp=a2.w*(1-dd/R);if(pr3<px)below+=wp;else above+=wp;}
      var ab=0,cnt=0;for(var j2=Math.max(0,i-20);j2<i;j2++){ab+=(((+d[j2].vol||0)*d[j2].close)||Math.abs(d[j2].close-d[j2].open)*1e4);cnt++;}
      ab=cnt?ab/cnt:0;var X=below+above;
      score[i]=(ab>0)?X/ab:0; // raw fragility; percentile-normalized below
      dirA[i]=X>0?(below-above)/X:0;}
    // adaptive 0-100: percentile rank vs the trailing 200 bars -> thresholds mean the same on every coin/TF
    var raw=score.slice();
    for(i=0;i<n;i++){var lo2=Math.max(0,i-200),c2=0,ls=0;for(var j3=lo2;j3<i;j3++){c2++;if(raw[j3]<raw[i])ls++;}score[i]=c2>=30?100*ls/c2:0;}
    var mk=[],armed=false; // start disarmed: the ladder needs warm-up bars before scores mean anything
    for(i=40;i<n;i++){if(!armed){if(score[i]<60)armed=true;continue;}
      if(score[i]>=90&&Math.abs(dirA[i])>=0.3){
        if(dirA[i]>0)mk.push({time:d[i].time,position:'aboveBar',color:'#ff6258',shape:'arrowDown',text:'CASC'});
        else mk.push({time:d[i].time,position:'belowBar',color:'#2ebd85',shape:'arrowUp',text:'CASC'});
        armed=false;}}
  ...
  function brainFactors(w,bars){var n=bars?bars.length:0;
    ...
    if(w._liqEvents&&w._liqSym===w.sym&&w._liqEvents.length){var iv=parseInt(w.tf,10)*60||3600,bk={};
      for(i=0;i<w._liqEvents.length;i++){var ev=w._liqEvents[i];if(!ev||!(ev.ts>0)||!(ev.notional>0))continue;var bt=Math.floor(ev.ts/1000/iv)*iv,o=bk[bt]||(bk[bt]={l:0,s:0});if(ev.side==='long_liquidated')o.l+=ev.notional;else o.s+=ev.notional;}
      var tots=[];for(i=0;i<n;i++){var b2=bk[bars[i].time];tots.push(b2?b2.l+b2.s:0);}
      for(i=0;i<n;i++){var b3=bk[bars[i].time];if(!b3)continue;var t3=b3.l+b3.s,av3=0,c3=0;for(j=Math.max(0,i-20);j<i;j++){av3+=tots[j];c3++;}av3=c3?av3/c3:0;f4[i]=t3>0?((b3.s-b3.l)/t3)*cl(av3>0?t3/av3/4:0,0,1):0;}}
    var f5=new Array(n);for(z=0;z<n;z++)f5[z]=0;
    if(w._fundH&&w._fundSym===w.sym&&w._fundH.length>3){var fh=w._fundH,fi=0,rates=fh.map(function(x){return +x.fundingRate;}),med=rates.slice().sort(function(x,y){return x-y;})[Math.floor(rates.length/2)]||0;
      for(i=0;i<n;i++){var bt2=bars[i].time*1000;while(fi+1<fh.length&&+fh[fi+1].fundingTime<=bt2)fi++;if(+fh[fi].fundingTime<=bt2)f5[i]=-cl(((+fh[fi].fundingRate)-med)/0.0005,-1,1);}}
    var f6=new Array(n);for(z=0;z<n;z++)f6[z]=0;
    if(w._crowdH&&w._crowdSym===w.sym&&w._crowdH.length>3){var chh=w._crowdH,ci=0;
  ...
  function brainCalc(w,bars){var n=bars?bars.length:0;if(n<80)return null;
    ...
    var LB=280,score=new Array(n),wts=new Array(F.length);for(z=0;z<n;z++)score[z]=0;for(z=0;z<F.length;z++)wts[z]=0;
    for(i=60;i<n;i++){
      if(i===60||i%10===0){var lo=Math.max(0,i-LB),hi=Math.max(lo,i-K);for(k=0;k<F.length;k++){var ic=corr(F[k],lo,hi);wts[k]=Math.abs(ic)>=0.03?cl(ic,-0.5,0.5):0;}}
      var num=0,den=0;for(k=0;k<F.length;k++){num+=wts[k]*F[k][i];den+=Math.abs(wts[k]);}
      score[i]=den>1e-6?cl(num/den,-1,1)*100:0;}
    var guard=window.__mpCalHi||null;
    function nearEvent(tSec){if(!guard)return false;for(var g=0;g<guard.length;g++)if(Math.abs(guard[g]-tSec*1000)<9e6)return true;return false;}
    var mk=[],armed=true;
    for(i=70;i<n;i++){var sc=score[i];
      if(armed&&Math.abs(sc)>=55){if(!nearEvent(bars[i].time))mk.push(sc>0?{time:bars[i].time,position:'belowBar',color:'#2ebd85',shape:'arrowUp',text:'BRAIN'}:{time:bars[i].time,position:'aboveBar',color:'#ff6258',shape:'arrowDown',text:'BRAIN'});armed=false;}
      else if(!armed&&Math.abs(sc)<25)armed=true;}
    var contrib=[],last=n-1;for(k=0;k<F.length;k++)contrib.push([Math.abs(wts[k]*F[k][last]),names[k]]);
  ...
  function psarCalc(b){var n=b.length,out=new Array(n),step=0.02,maxAf=0.2;if(n<2){for(var z=0;z<n;z++)out[z]=NaN;return out;}
    ...
    for(var i=1;i<n;i++){sar=sar+af*(ep-sar);
      if(up){if(b[i].low<sar){up=false;sar=ep;ep=b[i].low;af=step;}else if(b[i].high>ep){ep=b[i].high;af=Math.min(maxAf,af+step);}}
      else{if(b[i].high>sar){up=true;sar=ep;ep=b[i].high;af=step;}else if(b[i].low<ep){ep=b[i].low;af=Math.min(maxAf,af+step);}}
  ...
  function cwLeg(w,param){if(!w||!w.legEl)return;if(!w.legItems||!w.legItems.length){w.legEl.style.display='none';w.legEl.innerHTML='';return;}w.legEl.style.display='';
    ...
    w.legEl.innerHTML=_tg+w.legItems.map(function(it){if(it.raw)return '<span style="color:'+it.color+';font-weight:700"'+(it.title?' title="'+it.title+'"':'')+'>'+it.label+'</span>';var v=it.last;if(param&&param.seriesData){var sd=param.seriesData.get(it.series);if(sd!=null)v=(typeof sd==='object'?(sd.value!=null?sd.value:sd.close):sd);else if(param.time!=null&&it.arr){var lo=0,hi=it.arr.length-1;while(lo<hi){var mid=(lo+hi+1)>>1;if(it.arr[mid].time<=param.time)lo=mid;else hi=mid-1;}if(it.arr[lo]&&it.arr[lo].time===param.time&&isFinite(it.arr[lo].value))v=it.arr[lo].value;}}return '<span style="color:'+it.color+'">'+it.label+' <b>'+cwFmt(v,it.dec)+'</b></span>';}).join('');}
  ...
  function applyInds(w){ if(!w.candle)return; var bars=w.bars||[];
    ...
    (w.indSeries||[]).forEach(function(s){try{w.chart.removeSeries(s);}catch(e){}}); w.indSeries=[];(w.subSeries||[]).forEach(function(s){try{if(w.sub)w.sub.removeSeries(s);}catch(e){}}); w.subSeries=[]; w.legItems=[];
    try{var _st=w._sigStats||{};for(var _sk in _st){var _sv=_st[_sk];w.legItems.push({raw:true,color:_sv.pct>=55?'#2ebd85':_sv.pct<=45?'#ff6258':'#8fa3c4',label:_sk+' '+_sv.pct+'% ('+_sv.w+'W/'+_sv.l+'L)',title:__esT_mpcharts("howThisSignalScored",'How this signal scored on the loaded history of THIS symbol and timeframe: each arrow graded 1R vs 1R (target and stop both 1.5\u00d7ATR from the signal close).')});}}catch(e){}
    ...
    var paneKeys=['vol','rsi','macd','stoch','atr','wr','cci','casc','brain','memory'].filter(function(k){return w.inds[k];}); var paneN=paneKeys.length;
    ...
    function addS(opts,data,leg){try{var _tc=(opts.priceScaleId&&w.sub)?w.sub:w.chart;var s=_tc.addLineSeries(Object.assign({lineWidth:1,priceLineVisible:false,lastValueVisible:false,crosshairMarkerVisible:false},opts));s.setData(data);if(_tc===w.sub)w.subSeries.push(s);else w.indSeries.push(s);if(leg){var last=null;for(var i=data.length-1;i>=0;i--){if(data[i]&&data[i].value!=null&&isFinite(data[i].value)){last=data[i].value;break;}}w.legItems.push({label:leg.label,series:s,color:opts.color,dec:leg.dec,last:last,arr:data});}return s;}catch(e){}}
    function addPL(o){try{w.indLines.push(w.candle.createPriceLine(o));}catch(e){}}
    if(w.inds.psar)addS({color:'#c2f64a',lineWidth:1,lineStyle:1},mapVal(bars,psarCalc(bars)),{label:'PSAR',dec:null});
    if(w.inds.ichi){var _ic=ichimoku(bars);addS({color:'#3fd8e6',lineWidth:1.6},mapVal(bars,_ic.conv),{label:'Tenkan',dec:null});addS({color:'#ff9f4d',lineWidth:1.6},mapVal(bars,_ic.base),{label:'Kijun',dec:null});addS({color:'rgba(46,189,133,.7)',lineStyle:2},mapVal(bars,_ic.spanA),{label:'Span A',dec:null});addS({color:'rgba(255,98,88,.7)',lineStyle:2},mapVal(bars,_ic.spanB),{label:'Span B',dec:null});}
    ...
    if(w.inds.casc&&w._casc){var _cl=w._casc.score[w._casc.score.length-1],_cd=(w._casc.dir&&w._casc.dir[w._casc.dir.length-1])||0;if(isFinite(_cl))w.legItems.push({raw:true,color:_cl>=90?'#ff6258':_cl>=70?'#ffb020':'#8fa3c4',label:'Cascade '+Math.round(_cl)+'/100 \u2014 '+(_cl>=90?'EXTREME fragility':_cl>=70?'fragile market':'calm')+(Math.abs(_cd)>=0.3?(_cd>0?__esT_mpcharts("u00b7FuelBelowPrice",' \u00b7 fuel BELOW price (long liqs): a dip can run'):__esT_mpcharts("u00b7FuelAbovePrice",' \u00b7 fuel ABOVE price (short liqs): a pop can run')):' \u00b7 fuel balanced'),title:'0-100: liquidation fuel within 5% of price vs what 20 bars of volume can absorb, ranked against the last 200 bars. Above 70 = fragile, above 90 = one push can cascade. For a position: fuel BELOW price is a chain of long liquidations under a long (and a target under a short); fuel ABOVE is the reverse.'+' Full guide: marginpad.io/ai-indicators/'});addS({color:'#ffb020',lineWidth:1.6,priceScaleId:'casc'},mapVal(bars,w._casc.score),{label:'CASCADE',dec:0});addS({color:'rgba(255,215,90,.4)',lineStyle:2,priceScaleId:'casc'},bars.map(function(b){return {time:b.time,value:90};}));pscale('casc','casc');}
    if(w.inds.brain&&w._brain){var _bl=w._brain.score[w._brain.score.length-1];if(isFinite(_bl))w.legItems.push({raw:true,color:_bl>=55?'#2ebd85':_bl<=-55?'#ff6258':'#8fa3c4',label:'Brain '+(_bl>=0?'+':'')+Math.round(_bl)+' \u2014 '+(_bl>=55?'strong bullish lean':_bl<=-55?'strong bearish lean':__esT_mpcharts("noStrongEdge",'no strong edge')),title:'-100..+100: the learned 8-factor blend (trend, momentum, volume, fragility, real liquidation flow, funding, crowd positioning, structure), weights re-learned every 10 bars from the trailing 280 bars, never the future. Beyond \u00b155 the model sees a strongly one-sided market; between, it has no strong opinion. For a position: trade WITH a strong lean, size down inside the band, and note that arrows go quiet around FOMC/CPI on purpose.'+' Full guide: marginpad.io/ai-indicators/'});addS({color:'#c2f64a',lineWidth:1.6,priceScaleId:'brain'},mapVal(bars,w._brain.score),{label:'BRAIN',dec:0});addS({color:'rgba(120,130,140,.35)',lineStyle:2,priceScaleId:'brain'},bars.map(function(b){return {time:b.time,value:0};}));addS({color:'rgba(255,215,90,.3)',lineStyle:2,priceScaleId:'brain'},bars.map(function(b){return {time:b.time,value:55};}));addS({color:'rgba(255,215,90,.3)',lineStyle:2,priceScaleId:'brain'},bars.map(function(b){return {time:b.time,value:-55};}));pscale('brain','brain');
      try{if(w._brain.top&&w._brain.top.length)w.legItems.push({raw:true,color:'#c2f64a',label:__esT_mpcharts("brainDrivenBy",'BRAIN driven by: ')+w._brain.top.join(' + '),title:__esT_mpcharts("theFactorsTheModel",'The factors the model currently weights highest on this symbol and timeframe (learned from the loaded history, walk-forward).')});}catch(e){}}
    if(w.inds.memory&&w._mem){var _mo=[];for(var _mi=0;_mi<bars.length;_mi++){var _mv=w._mem.forecast[_mi];if(isFinite(_mv))_mo.push({time:bars[_mi].time,value:Math.max(-100,Math.min(100,_mv*30))});}addS({color:'#b18cff',lineWidth:1.6,priceScaleId:'memory'},_mo,{label:'MEMORY',dec:0});addS({color:'rgba(120,130,140,.35)',lineStyle:2,priceScaleId:'memory'},bars.map(function(b){return {time:b.time,value:0};}));pscale('memory','memory');
      if(w._mem.proj){addPL({price:w._mem.proj.p,color:w._mem.proj.ret>=0?'#2ebd85':'#ff6258',lineWidth:1,lineStyle:2,axisLabelVisible:true,title:'MEM'});
        w.legItems.push({raw:true,color:'#c2f64a',label:'Memory: '+w._mem.proj.n+' analogs \u00b7 '+Math.round(w._mem.proj.up*100)+__esT_mpcharts("upU00b7Proj",'% up \u00b7 proj ')+(w._mem.proj.ret>=0?'+':'')+(w._mem.proj.ret*100).toFixed(2)+'% / 6 bars',title:__esT_mpcharts("kNearestNeighbourForecast",'k-nearest-neighbour forecast: the 25 past bars whose 8-factor state most resembles now, and what price did over the following 6 bars (6 hours on a 1h chart). The dashed MEM line is the projected level, the percentage is how many analogs went up. For a position: it is a base rate, not a call - a 70% up-rate with a small projected move says drift, not breakout.')+' Full guide: marginpad.io/ai-indicators/'});}}
    if(w.inds.magnet&&w._mag){var _mg=w._mag;
      ...
      var _mp=[];if(_mg.up)_mp.push('above '+cwFmt(_mg.up.price)+' (+'+(_mg.up.dist*100).toFixed(1)+'%, '+money(_mg.up.w)+' short liqs)');if(_mg.dn)_mp.push('below '+cwFmt(_mg.dn.price)+' (\u2212'+(_mg.dn.dist*100).toFixed(1)+'%, '+money(_mg.dn.w)+' long liqs)');
      if(_tg)w.legItems.push({raw:true,color:'#c2f64a',label:'Magnet: pulled '+(_mg.side==='up'?'UP':'DOWN')+' \u00b7 '+_mp.join(' \u00b7 '),title:__esT_mpcharts("theDominantLiquidationPool",'The dominant liquidation pool on each side pulls price; the bold line is the stronger magnet (size x proximity), estimated from our leverage-ladder model. For a position: the bold pool is the likelier next visit - a target when it is on your side, the level to keep a stop beyond when it is not.')+' Full guide: marginpad.io/ai-indicators/'});}
    if(w.inds.vol){var vhs;try{vhs=(w.sub||w.chart).addHistogramSeries({priceFormat:{type:'volume'},priceScaleId:'vol',lastValueVisible:false,priceLineVisible:false});}catch(e){}
      if(vhs){var vhd=[];for(var vhi=0;vhi<bars.length;vhi++){var vhb=bars[vhi],vhv=+vhb.vol;if(isFinite(vhv)&&vhv>0)vhd.push({time:vhb.time,value:vhv,color:(+vhb.close>=+vhb.open)?'rgba(46,189,133,.45)':'rgba(255,98,88,.45)'});}try{vhs.setData(vhd);}catch(e){}if(w.sub)w.subSeries.push(vhs);else w.indSeries.push(vhs);}
      pscale('vol','vol');}
    cwLeg(w);
  }
  ...
  function mtAutoscale(w){ return function(orig){ try{
    if(!w.bars||!w.bars.length)return orig?orig():null;
    var vr=null;try{vr=w.chart.timeScale().getVisibleLogicalRange();}catch(e){}
    var n=w.bars.length,from=vr?Math.max(0,Math.floor(vr.from)):Math.max(0,n-160),to=vr?Math.min(n-1,Math.ceil(vr.to)):n-1;
  ...
  function updateNotesBtn(w){var b=w.el&&w.el.querySelector('.cwin-notes-btn');if(!b)return;var arr=notesFor(w);b.hidden=arr.length===0;var k=Math.min(arr.length,5);/* one pinned note = one icon, up to 5 (overlapping stack) */var ic='<svg viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M15 3H5a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h10l6-6V5a2 2 0 0 0-2-2z"/><path d="M15 21v-6h6"/></svg>';var h='';for(var i=0;i<k;i++)h+='<span style="display:inline-flex;'+(i?'margin-left:-4px':'')+'">'+ic+'</span>';b.innerHTML=h;b.title=arr.length+' note'+(arr.length===1?'':'s')+__esT_mpcharts("pinnedHere",' pinned here')+(arr.length>5?' (showing 5)':'');}
  var winNotesEl=null,winNotesW=null;
  function renderWinNotes(){var arr=notesFor(winNotesW);winNotesEl.innerHTML=__esT_mpcharts("notesPinnedToThis",'<div class="cwin-ind-menu-h">Notes pinned to this chart</div>')+(arr.length?('<div class="cwin-ind-menu-list">'+arr.map(function(n,i){var vis=n.el.style.display!=='none';return '<div class="cws-lay-item"><button type="button" class="cws-lay-open" data-noteidx="'+i+'" style="color:'+(n.color||'#e9e7df')+'">'+escHtml((n.text||'(empty note)').slice(0,42))+(vis?' <small>open</small>':'')+'</button><button type="button" class="cws-lay-del" data-unpin="'+i+__esT_mpcharts("titleUnpinFloatAgain",'" title="Unpin (float again)">&#10548;</button></div>');}).join('')+'</div>'):__esT_mpcharts("noNotesPinnedHere",'<div class="cws-lay-empty">No notes pinned here yet. Make a note, then drag it onto this window to pin it.</div>'));}
  ...
  function buildIndMenu(){
    indMenuEl=el('<div class="cwin-ind-menu" hidden><div class="cwin-ind-menu-h">Indicators</div><div class="cwin-ind-menu-list">'+INDS.map(function(t){var sel='';if(t[0]==='ema')sel='<span class="cwin-ind-pers" data-p="ema" title="Pick up to 3 EMA lengths">'+[9,21,50,100,200].map(function(p){return '<b data-pp="'+p+'">'+p+'</b>';}).join('')+'</span>';else if(t[0]==='sma')sel='<span class="cwin-ind-pers" data-p="sma" title="Pick up to 3 SMA lengths">'+[10,20,50,100,200].map(function(p){return '<b data-pp="'+p+'">'+p+'</b>';}).join('')+'</span>';var _lk=MP_INDS[t[0]]&&!indAllowed();return '<button type="button" class="cwin-ind-item'+(_lk?' locked':'')+'" data-ind="'+t[0]+'" title="'+(_lk?__esT_mpcharts("lockedU2014MarginpadExclus",'Locked \u2014 MarginPad exclusive. Ask us to unlock it for your account.'):(ITIPS[t[0]]||'').replace(/"/g,'&quot;'))+'"><span class="cwin-ind-ck"></span><span class="cwin-ind-it-lbl"'+(MP_INDS[t[0]]?' style="color:#c2f64a;font-weight:700'+(_lk?';opacity:.5':'')+'"':'')+'>'+t[1]+(_lk?' <b style="color:#5c6b84;font-weight:400;font-size:9.5px;letter-spacing:.05em">LOCKED</b>':'')+'</span>'+sel+'</button>';}).join('')+'</div></div>');
    ...
    indMenuEl.addEventListener('click',function(e){
      ...
      var b=e.target.closest&&e.target.closest('.cwin-ind-item');if(!b||!indMenuW)return;var k=b.getAttribute('data-ind');if(MP_INDS[k]&&!indAllowed()){if(window.mpPremium&&window.mpPremium.show)window.mpPremium.show(__esT_mpcharts("unlockThePremiumIndicators",'Unlock the premium indicators'));else chartToast(__esT_mpcharts("premiumIndicatorSignIn",'Premium indicator - sign in and upgrade to use it.'));return;}indMenuW.inds[k]=!indMenuW.inds[k];b.classList.toggle('on',!!indMenuW.inds[k]);if(indMenuW.inds[k]){try{var _il=(b.querySelector('.cwin-ind-it-lbl')||{}).textContent||k;window.__mpTrack&&window.__mpTrack('ind',_il);}catch(_){}}applyInds(indMenuW);updateIndN(indMenuW);savePersist();
    });
  ...
  var aiEl=null,aiW=null,aiBusy=false,aiRaf=0,aiLastL=null,aiLastT=null,aiLimit=50,aiPlans={premium:1,plus:50},aiOffX=8,aiOffY=0,aiDragging=false;
  ...
  /* SWING PIVOTS (2026-09-17, owner: "crtezi nisu bas najbolji, ne stoje na predvidjenom mestu"): a drawing is only as good as
     what it is anchored to. The model used to get two swing extremes and 24 closes and had to INVENT the bar index for every
     line, so trendlines floated in empty space. It now gets the real fractal pivots - a bar whose high is the highest within
     +/-k bars - with the barsAgo the draw action takes verbatim, so a trendline can touch the same highs a human would draw it
     through. k scales with how many bars are loaded, so a 1m chart and a 1d chart both return structure instead of noise. */
  function pivotsOf(bars,k,max){var n=bars.length,out=[];
    for(var i=k;i<n-k;i++){var hi=+bars[i].high,lo=+bars[i].low,isH=true,isL=true;
      for(var j=i-k;j<=i+k;j++){if(j===i)continue;if(+bars[j].high>=hi)isH=false;if(+bars[j].low<=lo)isL=false;if(!isH&&!isL)break;}
      if(isH)out.push({i:i,barsAgo:n-1-i,price:hi,kind:'high'});
      if(isL)out.push({i:i,barsAgo:n-1-i,price:lo,kind:'low'});}
  ...
  function volOf(bars){
    ...
    return {lastBarVsMedian20:+(last/med).toFixed(2),
      ...
      last5BuyersVsSellers:(up+dn)>0?Math.round(up/(up+dn)*100)+__esT_mpcharts("ofTheLast5",'% of the last 5 bars’ volume traded on up-candles'):null,
  ...
  function htfOf(bars,tf,price,mult){
    ...
    return {timeframe:lab,structure:st,
      ...
      mult:f,note:__esT_mpcharts("fromTheSameCandles",'from the same candles aggregated x')+f+__esT_mpcharts("aLevelHereOutranks",'. A level here outranks one on the chart in view; say which frame you are quoting.')};}
  ...
  function volRegimeOf(bars){
    ...
    return {atrPercentileVsOwnHistory:pct,
      regime:pct<=20?__esT_mpcharts("squeezeUnusuallyQuietFor",'squeeze - unusually quiet for this market, breakouts start here'):pct>=80?__esT_mpcharts("expansionUnusuallyWildStop",'expansion - unusually wild, stops get run and ranges fail'):'normal'};}
    ...
    try{
      ...
      cands.forEach(function(p){
        for(var i=Math.max(1,n-4);i<n;i++){if((n-1-i)>=p.barsAgo)continue;var b=bars[i];
          if(p.kind==='high'&&+b.high>p.price&&+b.close<p.price)out.push({name:__esT_mpcharts("liquiditySweepOfThe",'liquidity sweep of the high at ')+_p6(p.price),meaning:__esT_mpcharts("stopsAboveWereTaken",'stops above were taken and price closed back under - often the top of the move')});
          else if(p.kind==='low'&&+b.low<p.price&&+b.close>p.price)out.push({name:__esT_mpcharts("liquiditySweepOfThe2",'liquidity sweep of the low at ')+_p6(p.price),meaning:__esT_mpcharts("stopsBelowWereTaken",'stops below were taken and price closed back above - often the bottom of the move')});}});
      ...
      (lvls||[]).slice(0,3).forEach(function(l){
        ...
        if(broke&&backIn)out.push({name:__esT_mpcharts("failedBreakOfThe",'failed break of the ')+l.kind+' at '+_p6(l.price),meaning:__esT_mpcharts("priceWentThroughAnd",'price went through and could not hold it - the level is still in charge')});});
      ...
      if(vr&&vr.atrPercentileVsOwnHistory<=25){
        ...
        if((hi-lo)/price<0.02)out.push({name:'range compression',meaning:__esT_mpcharts("theLast20Candles",'the last 20 candles sit inside ')+(+((hi-lo)/price*100).toFixed(2))+__esT_mpcharts("energyIsBuildingFor",'% - energy is building for a break, direction not yet decided')});}
    }catch(e){}
  ...
  function sessionOf(bars){
    ...
    for(var i=n-1;i>0&&i>n-400;i--){
      ...
      if(a!==b)return {lastUtcDayOpenBarsAgo:n-1-i,note:__esT_mpcharts("drawItWithA",'draw it with a vline when the day boundary matters')};}
    return null;}
  ...
  function avwapOf(bars,piv,price){
    ...
    var a=mk(hi,__esT_mpcharts("theLastSwingHigh",'the last swing high')),b2=mk(lo,__esT_mpcharts("theLastSwingLow",'the last swing low'));
  ...
  function aiContext(w){
    ...
    if(n<10||!(price>0))return {symbol:w.sym,timeframe:tfWords(w.tf),price:price||null,note:'Only '+n+__esT_mpcharts("candlesLoadedLimitedRead",' candles loaded - limited read.')};
    var c=closes(bars);
    ...
    var ro=null;try{var _r={};if(w.inds.casc&&w._casc){var _cs=w._casc.score[w._casc.score.length-1],_cd=(w._casc.dir&&w._casc.dir[w._casc.dir.length-1])||0;if(isFinite(_cs))_r.cascadeRadar={fragility0to100:Math.round(_cs),fuelSide:Math.abs(_cd)>=0.3?(_cd>0?__esT_mpcharts("belowPriceLongLiquidations",'below price (long liquidations)'):__esT_mpcharts("abovePriceShortLiquidation",'above price (short liquidations)')):'balanced'};}
      ...
      if(w.inds.brain&&w._brain){var _bs=w._brain.score[w._brain.score.length-1];if(isFinite(_bs))_r.marketBrain={score:Math.round(_bs),lean:_bs>=55?'strong bullish':_bs<=-55?'strong bearish':__esT_mpcharts("noStrongLean",'no strong lean'),drivenBy:(w._brain.top||[]).join(' + ')||null,hitRate:(w._sigStats&&w._sigStats.BRAIN)?w._sigStats.BRAIN.pct+'% ('+w._sigStats.BRAIN.w+'W/'+w._sigStats.BRAIN.l+'L)':null};}
    ...
    try{var _k=Math.max(2,Math.min(9,Math.round(n/40))),_P=pivotsOf(bars,_k,16);
      ...
      if(_hs.length>=2&&_ls.length>=2){var _hh=_hs[0].price>_hs[1].price,_hl=_ls[0].price>_ls[1].price;
        struct=(_hh&&_hl)?__esT_mpcharts("higherHighsAndHigher",'higher highs and higher lows (uptrend structure)'):((!_hh&&!_hl)?__esT_mpcharts("lowerHighsAndLower",'lower highs and lower lows (downtrend structure)'):'mixed - range or a transition');}
    }catch(e){}
  ...
  function aiSetQuota(used,limit){var q=aiEl&&aiEl.querySelector('.cwin-ai-quota');if(q&&used!=null){q.textContent=used+'/'+limit+' today \u00b7 '+aiResetIn();q.title=__esT_mpcharts("askAiReadsUsed",'Ask AI reads used today. The allowance resets at 00:00 UTC.');q.classList.toggle('low',(limit-used)<=2);}if(limit)aiLimit=limit;}
  ...
  function aiPlanCard(plan,price){if(!plan)return '';var bias=String(plan.bias||'').toLowerCase(),isW=!aiTradable(plan),_rrLow=aiWhyNotTradable(plan);
    ...
    var _rows=row('Entry',plan.entry,'e')+row('Stop',plan.stop,'s')+(plan.targets||[]).map(function(t,i){return row('Target '+(i+1),t,'t');}).join('');if(_rows){if(!isW)h+='<div class="aipc-rows">'+_rows+'</div>';else h+='<div class="aipc-wait">'+(_rrLow?__esT_mpcharts("notWorthTakingAt",'not worth taking at R:R ')+_rrLow+__esT_mpcharts("theLevelsForWatching",' - the levels, for watching'):__esT_mpcharts("waitingForNotA",'waiting for - not a trade yet'))+'</div><div class="aipc-rows w">'+_rows+'</div>';}
  ...
  function aiClearAi(w){aiGhostClear(w);if(!w||!w.dr||!w.dr.shapes)return 0;var n=w.dr.shapes.length;w.dr.shapes=w.dr.shapes.filter(function(s){return s.by!=='ai';});var k=n-w.dr.shapes.length;if(k){w.dr.sel=null;if(w.dr.redraw)w.dr.redraw();if(w.dr.save)w.dr.save();}aiRescale(w);return k;}
  function aiUndoBatch(w,b){if(w&&w._ghostB===b)aiGhostClear(w);if(!w||!w.dr||!w.dr.shapes)return 0;var n=w.dr.shapes.length;w.dr.shapes=w.dr.shapes.filter(function(s){return !(s.by==='ai'&&s.aiB===b);});var k=n-w.dr.shapes.length;if(k){w.dr.sel=null;if(w.dr.redraw)w.dr.redraw();if(w.dr.save)w.dr.save();}aiRescale(w);return k;}
  ...
  /* One draw action -> engine shapes. EVERY geometry rule a good chart drawing obeys is enforced here, not asked for in the
     prompt (2026-09-17, owner: "crtezi nisu bas najbolji, ne stoje na predvidjenom mestu ... nekad prikazuje neke zone i
     nacrta ih iza poslednje svece, sto znaci da je to proslo"):
       - a zone is a horizontal BAND that always reaches past the newest candle, never a floating box in dead space
       - a band's height is capped, so a "target zone" can never become a wall across half the chart
       - an arrow starts at the newest candles and points forward - it can never be a diagonal across the whole chart
  ...
  function aiShapeOf(a,n,px,ctx){if(!a||typeof a!=='object'||!(n>1))return null;
    ...
    var snapBar=function(p,ba,side){p=+p;if(!(p>0)||!bars||ba==null||ba==='')return p;var i=last-Math.round(+ba);if(!(i>=0&&i<n))return p;
      var b=bars[i];if(!b)return p;var hi=+b.high,lo=+b.low;
      var d=(side==='highs')?hi:((side==='lows')?lo:(Math.abs(p-hi)<=Math.abs(p-lo)?hi:lo));
      return (px>0&&Math.abs(d-p)/px<=0.015)?d:p;};
    /* WHICH SIDE OF THE CANDLES THE LINE RIDES (2026-09-17): a trend line is drawn along the HIGHS or along the LOWS, never one of
       each - and the model twice handed back a line it called "lower highs" anchored on a rising pair. The side is taken from what
       the line claims to be, then from the measured structure, and BOTH anchors are snapped to that side. The line then rides the
       wicks a human would have used, whatever the label says. */
    var sideOf=function(a2){var t=String((a2&&(a2.side||a2.label))||'').toLowerCase();
      if(/high|resist|down|bear|ceiling|supply/.test(t))return 'highs';
      if(/low|support|up|bull|floor|demand|base/.test(t))return 'lows';
      var st=String((ctx&&ctx.structure)||'').toLowerCase();
      if(/lower highs/.test(st))return 'highs';if(/higher highs/.test(st))return 'lows';return null;};
    /* A LINE MUST NOT CONTRADICT ITS OWN NAME (2026-09-17): the model kept handing back a RISING pair for a line it called
       "Downtrend" / "Lower highs". A line along the highs has to fall and a line along the lows has to rise, so when the pair it
       named breaks that, we do not draw the contradiction and we do not drop the idea either - we look through the pivots it was
       given and take the most recent pair of that kind which actually satisfies it. Nothing suitable: no line. */
    var fixPair=function(side,b1,q1,b2,q2){ // b1/q1 = older anchor, b2/q2 = newer
      if(!side)return [b1,q1,b2,q2];
      var okNow=(side==='highs')?(q1>q2):(q1<q2);
      if(okNow)return [b1,q1,b2,q2];
      var pv=(ctx&&ctx.piv)||[],kind=(side==='highs')?'high':'low';
      var arr=pv.filter(function(x){return x.kind===kind;}); // newest first
      for(var i=0;i<arr.length;i++)for(var j=i+1;j<arr.length;j++){
        var nw=arr[i],od=arr[j];
        if(Math.abs(od.barsAgo-nw.barsAgo)<4)continue;
        if((side==='highs')?(od.price>nw.price):(od.price<nw.price))return [od.barsAgo,od.price,nw.barsAgo,nw.price];}
      return null;};
    ...
    /* A LABEL GOES IN THE EMPTY SPACE PAST THE NEWEST CANDLE (2026-09-17, owner: "ne stoje na predvidjenom mestu"): dropping it on
    ...
    if(sh==='hline'){if(!okp(a.p))return null;var hp=snapLv(a.p);out.push(Object.assign({t:'hline',p:hp},base));label(hp);}
      ...
      out.push(Object.assign({},base,{t:'zone',l1:l1,p1:zl,l2:null,p2:zh,w:1,txt:lbl}));/* l2 null = run to the right edge; the engine prints both bounds and the band's width */
    }
    else if(sh==='channel'){
      if(!okp(a.p1)||!okp(a.p2))return null;
      var _cb1=(a.barsAgo1==null?Math.round(n*0.6):+a.barsAgo1),_cb2=(a.barsAgo2==null?0:+a.barsAgo2);
      var _csd=sideOf(a);
      var _cp1=snapBar(a.p1,_cb1,_csd),_cp2=snapBar(a.p2,_cb2,_csd);
      if(_cb2>_cb1){var _t=_cb1;_cb1=_cb2;_cb2=_t;var _tp2=_cp1;_cp1=_cp2;_cp2=_tp2;}
      var _cfx=fixPair(_csd,_cb1,_cp1,_cb2,_cp2);if(!_cfx)return null;_cb1=_cfx[0];_cp1=_cfx[1];_cb2=_cfx[2];_cp2=_cfx[3];
      var c1=L(_cb1,Math.round(n*0.6)),c2=L(_cb2,0);
      if(Math.abs(c2-c1)<4)return null;
      var wdt=+a.width;if(!(wdt>0))wdt=Math.abs(_cp2-_cp1)*0.5||rng*0.08;wdt=Math.min(wdt,bandMax);
      var up=(a.side==='below')?-1:1;
      out.push(Object.assign({},base,{t:'ray',l1:c1,p1:_cp1,l2:c2,p2:_cp2,anch:1,txt:lbl}));
      out.push(Object.assign({},base,{t:'ray',l1:c1,p1:_cp1+wdt*up,l2:c2,p2:_cp2+wdt*up,dash:true}));
    }
    else if(sh==='trend'||sh==='ray'||sh==='fib'){
      if(!okp(a.p1)||!okp(a.p2))return null;
      /* ANCHORS RUN OLDEST FIRST (2026-09-17, owner: "linija ide u nazad a ne znam odakle"). The model handed back the NEWER pivot
         as p1 and the older one as p2, so the ray extended backwards through the history instead of forward. Order is normalised
         here, which makes "a ray always points into the future" a property of the tool rather than a hope. */
      var _b1=(a.barsAgo1==null?Math.round(n*0.5):+a.barsAgo1),_b2=(a.barsAgo2==null?0:+a.barsAgo2);
      var _sd=(sh==='fib')?null:sideOf(a);
      var _p1=snapBar(a.p1,_b1,_sd),_p2=snapBar(a.p2,_b2,_sd);
      ...
      if(Math.abs(tt2-t1)<4)return _rej(__esT_mpcharts("itsTwoAnchorsAre",'its two anchors are fewer than 4 candles apart - two adjacent wicks are not a trend, pick pivots further apart'),a); // two anchors one candle apart are not a line
    ...
    else if(sh==='level'||sh==='hray'){ // a level that became relevant at a candle: starts there, runs right, price printed at the anchor
      if(!okp(a.p))return null;var hp3=snapLv(a.p);
      out.push(Object.assign({},base,{t:'hray',p:hp3,l1:L(a.barsAgo1!=null?a.barsAgo1:a.barsAgo,Math.round(n*0.25)),txt:lbl}));}
    else if(sh==='pattern'||sh==='poly'){ // head-and-shoulders, triangles, wedges, flags, Elliott counts - one primitive
      var raw=Array.isArray(a.points)?a.points:[];if(raw.length<2)return null;
      var pp2=[];raw.slice(0,12).forEach(function(q){if(!q||!okp(q.price))return;
        pp2.push({l:L(q.barsAgo,0),p:snapBar(q.price,q.barsAgo),lab:String(q.label||'').slice(0,14)});});
      ...
      if(!okp(a.p1)||!okp(a.p2))return _rej(__esT_mpcharts("measureNeedsTwoReal",'measure needs two real prices'),a);
      var ml1=L(a.barsAgo1,12),ml2=L(a.barsAgo2,0);
      if(Math.abs(ml1-ml2)<2)return _rej(__esT_mpcharts("aMeasureBoxNeeds",'a measure box needs its two anchors at least 2 candles apart'),a);
      ...
      if(!okp(a.entry)||!okp(a.stop))return _rej(__esT_mpcharts("entryOrStopIs",'entry or stop is too far from the current price to be real (must sit between 0.4x and 2.5x of it)'),a);
      ...
      try{var _t0=(Array.isArray(a.targets)?a.targets:[a.target]).map(Number).filter(function(v){return v>0;})[0];
        if(_t0>0){var _rk=Math.abs(+a.entry-+a.stop),_rw=Math.abs(_t0-+a.entry);
          if(_rk>0&&(_rw/_rk)<AI_MIN_RR)return _rej(__esT_mpcharts("theFirstTargetPays",'the first target pays ')+(_rw/_rk).toFixed(2)+__esT_mpcharts("xTheRiskUnder2",'x the risk, under the ')+AI_MIN_RR+__esT_mpcharts("xFloorMakeIt",'x floor - make it a wait, or find a target that pays'),a);}}catch(e){}
      ...
      if(!okp(a.p1)||!okp(a.p2)||!okp(a.p3))return null;
      out.push(Object.assign({},base,{t:'fibx',l1:L(a.barsAgo1,Math.round(n*0.5)),p1:snapBar(a.p1,a.barsAgo1),l2:L(a.barsAgo2,Math.round(n*0.3)),p2:snapBar(a.p2,a.barsAgo2),l3:L(a.barsAgo3,Math.round(n*0.1)),p3:snapBar(a.p3,a.barsAgo3),w:1}));}
    else if(sh==='pitchfork'||sh==='fork'){
      if(!okp(a.p1)||!okp(a.p2)||!okp(a.p3))return null;
      out.push(Object.assign({},base,{t:'fork',l1:L(a.barsAgo1,Math.round(n*0.6)),p1:snapBar(a.p1,a.barsAgo1),l2:L(a.barsAgo2,Math.round(n*0.4)),p2:snapBar(a.p2,a.barsAgo2),l3:L(a.barsAgo3,Math.round(n*0.2)),p3:snapBar(a.p3,a.barsAgo3),txt:lbl}));}
  ...
  function travelOf(bars,px){var out=[],ks=[3,5,8,13,21,34,55,89],n=bars.length;
    ...
    return out;}
  /* What this market typically travels over a window of k candles, read off the ladder and interpolated; outside it, price moves
     scale with roughly the square root of time, so that is how it extrapolates. This is the number that keeps the forecast honest:
     over 18 candles a market covers what it covers, and the drawn run may not exceed it. */
  function travelAt(travel,k){if(!travel||!travel.length||!(k>0))return 0;
    if(k<=travel[0].k)return travel[0].med*Math.sqrt(k/travel[0].k);
    for(var i=1;i<travel.length;i++){if(travel[i].k>=k){var a=travel[i-1],b=travel[i],f=(b.k>a.k)?(k-a.k)/(b.k-a.k):0;return a.med+(b.med-a.med)*f;}}
    var l=travel[travel.length-1];return l.med*Math.sqrt(k/l.k);}
  function rhythmOf(bars,piv,px){var legB=[],legA=[],i;
    ...
    return {legBars:Math.max(2,Math.min(40,lb)),legAmp:Math.max(0.0015,Math.min(0.08,la)),drift:drift,barRange:perBar,travel:travelOf(bars,px)};}
  ...
  function aiPathPts(fromP,toP,horizon,rhythm,px,last){
    ...
    /* A FORECAST IS 15-20 CANDLES (owner 2026-09-17: "nek predvidi 15-20 sveca"). The length is fixed, so the honest question is
       not how long the target takes but HOW FAR this market plausibly gets in that many candles: `travelAt` answers it from the
       measured travel ladder, and when the target is further the run covers that ground and points at it instead of pretending to
       arrive. The target is already marked by its own zone and the TP lines, so nothing is lost. */
    var H=Math.max(15,Math.min(20,Math.round(+horizon||18)));if(!isFinite(H))H=18;
    var typ=(r.travel&&r.travel.length)?travelAt(r.travel,H):0;
    if(typ>0&&dist>typ){var reach=typ/dist;toP=fromP+(toP-fromP)*reach;dist=typ;}
    ...
    return out;}
  /* THE FORECAST IS DRAWN AS CANDLES, NOT AN ARROW (2026-09-17, owner: "umesto tih strelica ... da da providne predikcije sveca ...
     kao da su svece vec bile ali neka okvirna predikcija i da budu malo vise providne u odnosu na prave svece"). They are a REAL
     Lightweight Charts candlestick series on the same price and time axes, so they are welded to the chart exactly like the real
     candles - no canvas projection to drift - and they are drawn translucent with a faint border so nobody can mistake them for
     something that happened. Each bar opens where the last one closed and closes on the measured path, so the run of green and red
     bodies comes out of the market's own rhythm rather than being decided by hand. */
  function aiGhostClear(w){if(!w)return;if(w._ghost){try{w.chart.removeSeries(w._ghost);}catch(e){}w._ghost=null;}w._ghostBars=null;w._ghostB=null;}
  function aiGhost(w,pts,rhythm,batch){
    if(!w||!w.chart||!pts||pts.length<2||!w.bars||!w.bars.length)return 0;
    var iv=parseInt(w.tf,10)*60;if(!(iv>0))return 0;
    var last=w.bars[w.bars.length-1];if(!last||!(+last.time>0))return 0;
    var r=rhythm||{},px=+last.close||0,rng=Math.max(px*0.0004,(r.barRange>0?r.barRange:0.004)*px);
    var seed=Math.abs(Math.round((pts[pts.length-1].p||1)*100))%9973;
    var rnd=function(k){var x=Math.sin((seed+k*7.31)*12.9898)*43758.5453;return x-Math.floor(x);};
    var data=[],prev=px,i;
    for(i=1;i<pts.length;i++){
      var o=prev,c=+pts[i].p;if(!(c>0))continue;
      var top=rng*(0.12+rnd(i)*0.45),bot=rng*(0.12+rnd(i+499)*0.45);
      data.push({time:+last.time+i*iv,open:o,high:Math.max(o,c)+top,low:Math.min(o,c)-bot,close:c});
      prev=c;}
    if(data.length<2)return 0;
    if(!w._ghost){var fmt=null;try{fmt=window.mpPriceFmt?window.mpPriceFmt(w.bars,px):null;}catch(e){}
      try{w._ghost=w.chart.addCandlestickSeries({upColor:'rgba(46,189,133,.30)',downColor:'rgba(255,90,77,.30)',borderVisible:true,borderUpColor:'rgba(46,189,133,.58)',borderDownColor:'rgba(255,90,77,.58)',wickUpColor:'rgba(46,189,133,.46)',wickDownColor:'rgba(255,90,77,.46)',priceLineVisible:false,lastValueVisible:false,crosshairMarkerVisible:false,priceFormat:fmt||undefined});}catch(e){w._ghost=null;}}
    if(!w._ghost)return 0;
  ...
  function aiDrawActs(w,acts,batch){if(!w||!w.dr||!w.dr.shapes||!w.bars||w.bars.length<2)return 0;_aiRej=[];var n=w.bars.length,px=+w.bars[n-1].close,k=0;
    ...
    (acts||[]).forEach(function(a){if(!a||a.a!=='draw')return;
      ...
      if(_sh==='arrow'||_sh==='forecast'||_sh==='path'){ // the expected path is a run of translucent candles now, not a line
        ...
        if(!(hp2>0))return;
        var hz=(a.barsAgo2!=null&&+a.barsAgo2<0)?-Math.round(+a.barsAgo2):(+a.horizonBars>0?Math.round(+a.horizonBars):0);
        var pp=aiPathPts(px,hp2,hz,rhythm,px,n-1);
        var g=aiGhost(w,pp,rhythm,batch);
        if(g){ghost=g;k++;
  ...
  function aiSetInd(w,id,on,periods){var known=false;for(var i=0;i<INDS.length;i++)if(INDS[i][0]===id)known=true;if(!known)return null;
    if(MP_INDS[id]&&!indAllowed())return indName(id)+__esT_mpcharts("isLockedPremiumIndicator",' is locked (Premium indicator)');
  ...
  function aiExec(w,acts,batch){var out=[];if(!w||!Array.isArray(acts)||!acts.length)return Promise.resolve(out);
    ...
    acts.forEach(function(a){if(!a||typeof a!=='object')return;try{
      if(a.a==='symbol'&&a.sym){if(setWinSym(w,a.sym)){reload=true;out.push(__esT_mpcharts("switchedTo",'Switched to ')+w.sym);}}
  ...
  /* drew = the model drew the setup itself with actions. Then the plan contributes ONLY the trade levels (entry, stop, targets):
     its `levels` list and its risk/reward rectangles would land on top of the same prices the drawing already marks, and two sets
  ...
  function aiDrawPlan(w,plan,drew){
    ...
    aiRescale(w);
    chartToast((plan.bias&&plan.bias!=='wait'?plan.bias.toUpperCase()+__esT_mpcharts("planDrawnOn",' plan drawn on '):__esT_mpcharts("levelsDrawnOn",'Levels drawn on '))+w.sym+__esT_mpcharts("tapARowIn",' - tap a row in the card to find a level.'));
  }
  ...
  function aiSetChips(w){var box=aiEl&&aiEl.querySelector('.cwin-ai-chips');if(!box)return;var chips=[[__esT_mpcharts("findMeTheBest",'Find me the best opportunity on this chart right now - where would a good entry be, and what would make it a mistake?'),__esT_mpcharts("findAnOpportunity",'Find an opportunity')],[__esT_mpcharts("whereWouldYouWant",'Where would you want to be filled, and why not at the current price?'),'Best entry'],[__esT_mpcharts("whereAreTheKey",'Where are the key support and resistance levels?'),'Levels'],[__esT_mpcharts("whatWouldMakeThis",'What would make this a blunder?'),__esT_mpcharts("whatToAvoid",'What to avoid')]];var hasPos=false;try{hasPos=jload().some(function(e){return e.status==='open'&&e.sym===w.sym;});}catch(e){}if(hasPos)chips.push([__esT_mpcharts("howRiskyIsMy",'How risky is my open position on this chart right now?'),__esT_mpcharts("myPosition",'My position')]);
    try{var _hh=aiHistLoad(w),_lp=null,_la=null;for(var _i=_hh.length-1;_i>=0;_i--){if(!_la&&_hh[_i].role==='ai')_la=_hh[_i];if(_hh[_i].plan){_lp=_hh[_i].plan;break;}}if(_lp){chips=[['',__esT_mpcharts("reReadNow",'Re-read now')],[__esT_mpcharts("whyThatEntryAnd",'Why that entry and not the current price?'),__esT_mpcharts("whyThatEntry",'Why that entry')],[__esT_mpcharts("whatExactlyWouldProve",'What exactly would prove this plan wrong?'),__esT_mpcharts("whatKillsIt",'What kills it')],[__esT_mpcharts("whatIsThePlan",'What is the plan if it goes the other way instead?'),__esT_mpcharts("otherSide",'Other side')]];if(hasPos)chips.push([__esT_mpcharts("howRiskyIsMy",'How risky is my open position on this chart right now?'),__esT_mpcharts("myPosition",'My position')]);}
      ...
      if(_la&&_la.chips&&_la.chips.length){chips=_la.chips.map(function(c){return [c,c];});chips.push(['',__esT_mpcharts("reReadNow",'Re-read now')]);if(hasPos)chips.push([__esT_mpcharts("howRiskyIsMy",'How risky is my open position on this chart right now?'),__esT_mpcharts("myPosition",'My position')]);}}catch(e){}
    box.innerHTML=chips.map(function(c){return '<button class="cwin-ai-chip" data-q="'+escAttr(c[0])+'">'+escHtml(c[1])+'</button>';}).join('');}
  var aiPremium=false;
  function aiShowPremiumGate(){if(!aiEl)return;aiEl.classList.add('gated');var body=aiEl.querySelector('.cwin-ai-body');if(!body)return;body.innerHTML=__esT_mpcharts("askAiComesWith",'<div class="cwin-ai-gate"><b>Ask AI comes with Premium.</b><br>A built-in analyst that reads the chart you are looking at - the real measurements, not a screenshot - and draws the setup on it.<br><b>')+aiPlans.premium+__esT_mpcharts("readADayWith",' read a day with Premium ($11.99/mo)</b>, ')+aiPlans.plus+__esT_mpcharts("aDayWithPremium",' a day with Premium Plus ($159/mo).<br><button class="g-btn" type="button">Get Premium - $11.99/mo</button></div>');var g=body.querySelector('.g-btn');if(g)g.addEventListener('click',function(){if(window.mpPremium&&window.mpPremium.show)window.mpPremium.show(__esT_mpcharts("unlockAskAiOn",'Unlock Ask AI on your charts'));});}
  function aiShowGate(){if(!aiEl)return;aiEl.classList.add('gated');var body=aiEl.querySelector('.cwin-ai-body');body.innerHTML=__esT_mpcharts("signInToUse",'<div class="cwin-ai-gate">Sign in to use Ask AI - a built-in analyst that reads the chart in front of you and draws the setup on it. It comes with Premium: <b>')+aiPlans.premium+__esT_mpcharts("readADayAt",' read a day at $11.99/mo</b>, ')+aiPlans.plus+__esT_mpcharts("aDayOnPremium",' a day on Premium Plus.<br><button class="g-btn" type="button">Sign in free</button></div>');var g=body.querySelector('.g-btn');if(g)g.addEventListener('click',function(){try{if(window.mpAuth&&window.mpAuth.open)window.mpAuth.open();}catch(e){}});}
  ...
  function aiBuild(){
    ...
    aiEl.querySelector('.cwin-ai-clear').addEventListener('click',function(){if(aiW&&confirm(__esT_mpcharts("clearThisChartS",'Clear this chart’s AI chat history and the AI drawings?'))){aiHistSave(aiW,[]);aiClearAi(aiW);aiClearPlan(aiW);aiRenderBody(aiW);aiSetChips(aiW);}});
  ...
  function askAi(question){
    ...
    var w=aiW,q=String(question||'').trim();if(!q)q=__esT_mpcharts("giveMeASharp",'Give me a sharp, practical read on this chart right now.');
    ...
    fetch('/api/ai/chart',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({context:aiContext(w),question:q,history:payloadHist,stream:true,lang:(window.mpLang||document.documentElement.lang||'en')})}).then(function(resp){
      ...
      function finish(){aiBusy=false;bub.classList.remove('streaming');var sp=aiSplit(acc),prose=sp.prose||acc;if(!got||!prose){bub.innerHTML=__esT_mpcharts("noAnswerCameBack",'<span style="color:#ff8a80">No answer came back - try again.</span>');return;}var atB=atBottom();
      ...
      pump();
    }).catch(function(){fail(__esT_mpcharts("networkErrorTryAgain",'Network error - try again.'));});
  }
  ...
  function createChartAlert(w,y){
    ...
    fetch('/api/alerts',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({sym:w.sym,target:price,dir:dir,channel:'email'})}).then(function(r){return r.json();}).then(function(d){
      if(d&&d.ok){rec.id=d.id;chartToast('Alert set - '+w.sym+' '+(dir==='up'?'≥':'≤')+' '+cwFmt(price)+__esT_mpcharts("weLlEmailYou",' · we’ll email you · click the line to remove'));}
      else{chAlertDel(w,rec,true);
        if(d&&d.error==='too_many')chartToast(__esT_mpcharts("tooManyActiveAlerts",'Too many active alerts (max 25). Remove some on /alerts.'));
        else if(d&&d.error==='not_signed_in'){chartToast(__esT_mpcharts("signInFreeTo",'Sign in (free) to set price alerts.'));try{if(window.mpAuth&&window.mpAuth.open)window.mpAuth.open();}catch(e){}}
        else chartToast(__esT_mpcharts("couldnTSetThe",'Couldn’t set the alert. Try again.'));}
    }).catch(function(){chAlertDel(w,rec,true);chartToast(__esT_mpcharts("networkErrorAlertNot",'Network error - alert not set.'));});
  }
  ...
  function setupDraw(w,bodyEl){
    ...
    /* ZONE: from the candle it formed on, far to the right, with BOTH bounds and the band's own height printed on it - the owner
       could not read where a liquidity pool started or ended, and a box with no numbers is decoration. */
    function drawZone(s){var q=proj2(s);if(q.y1==null||q.y2==null)return;
      var x1=(s.l1==null)?0:xOf(s.l1),x2=(s.l2==null)?w.dr.W:xOf(s.l2);
      if(x1==null)x1=0;if(x2==null)x2=w.dr.W;
      var xa=Math.min(x1,x2),xb=Math.max(x1,x2),ya=Math.min(q.y1,q.y2),yb=Math.max(q.y1,q.y2);
      if(xb-xa<4)xb=xa+4;if(yb-ya<3){ya-=1.5;yb+=1.5;}
      ctx.save();ctx.setLineDash(s.dash?[6,5]:[]);ctx.lineWidth=1;ctx.strokeStyle=s.color;
      ctx.fillStyle=s.color;ctx.globalAlpha=.10;ctx.fillRect(xa,ya,xb-xa,yb-ya);
      ctx.globalAlpha=.75;ctx.beginPath();ctx.moveTo(xa,ya);ctx.lineTo(xb,ya);ctx.moveTo(xa,yb);ctx.lineTo(xb,yb);ctx.stroke();
      ctx.globalAlpha=.35;ctx.beginPath();ctx.moveTo(xa,ya);ctx.lineTo(xa,yb);ctx.stroke();ctx.restore();
      var hi=Math.max(+s.p1,+s.p2),lo=Math.min(+s.p1,+s.p2),th=_pct(hi,lo);
      inkTxt(xa+6,ya-3,cwFmt(hi),s.color,10);
    ...
    function drawHray(s){var y=yOf(s.p),x1=xOf(s.l1);if(y==null)return;if(x1==null)x1=0;
      ...
      anchDot(x1,y,s.color);inkTxt(x1+8,y-5,cwFmt(s.p)+(s.txt?'  '+s.txt:''),s.color,10);}
    /* POLYLINE with named points: one primitive for head-and-shoulders, triangles, wedges, flags and Elliott counts */
    function drawPoly(s){var p=s.pts||[];if(p.length<2)return;var pts=[],i;
      for(i=0;i<p.length;i++){var xx=xOf(p[i].l),yy=yOf(p[i].p);if(xx==null||yy==null)continue;pts.push([xx,yy,p[i].lab]);}
      if(pts.length<2)return;
      ctx.save();setStyle(s);ctx.beginPath();ctx.moveTo(pts[0][0],pts[0][1]);
      for(i=1;i<pts.length;i++)ctx.lineTo(pts[i][0],pts[i][1]);ctx.stroke();ctx.restore();
      for(i=0;i<pts.length;i++){anchDot(pts[i][0],pts[i][1],s.color);
        if(pts[i][2])inkTxt(pts[i][0],pts[i][1]-8,String(pts[i][2]).slice(0,14),s.color,10,'c');}}
    /* POSITION / RISK-REWARD tool - the one every broker terminal has: entry, the risk block and the reward block, with the
       prices, the distances and the reward-to-risk printed on them. */
    function drawPos(s){var ye=yOf(s.p),ys=yOf(s.stop),x1=xOf(s.l1);if(ye==null||ys==null)return;if(x1==null)x1=0;
      var tg=(s.tgts||[]).map(Number).filter(function(v){return v>0;}),xb=w.dr.W-2;
      var g=tg.length?yOf(tg[tg.length-1]):null;
      ctx.save();
      if(ys!=null){ctx.fillStyle='#ff5a4d';ctx.globalAlpha=.09;ctx.fillRect(x1,Math.min(ye,ys),xb-x1,Math.abs(ys-ye));
        ctx.globalAlpha=.6;ctx.strokeStyle='#ff5a4d';ctx.lineWidth=1;ctx.setLineDash([]);ctx.strokeRect(x1,Math.min(ye,ys),xb-x1,Math.abs(ys-ye));}
      if(g!=null){ctx.fillStyle='#2ebd85';ctx.globalAlpha=.09;ctx.fillRect(x1,Math.min(ye,g),xb-x1,Math.abs(g-ye));
        ctx.globalAlpha=.6;ctx.strokeStyle='#2ebd85';ctx.lineWidth=1;ctx.strokeRect(x1,Math.min(ye,g),xb-x1,Math.abs(g-ye));}
      ctx.globalAlpha=1;ctx.strokeStyle='#3fd8e6';ctx.lineWidth=1.6;ctx.setLineDash([]);ctx.beginPath();ctx.moveTo(x1,ye);ctx.lineTo(xb,ye);ctx.stroke();ctx.restore();
      var rr=null;if(tg.length&&Math.abs(+s.p-+s.stop)>0)rr=Math.abs(tg[0]-+s.p)/Math.abs(+s.p-+s.stop);
      ...
      if(rr!=null){if(rr<1){rc='#ff5a4d';rw=__esT_mpcharts("poorRisksMoreThan",'  POOR - risks more than it makes');rwShort='  POOR';}else if(rr<1.5){rc='#ffb020';rw='  thin';rwShort='  thin';}else if(rr>=2.5)rc='#41e3a3';}
      if(_labTight){
    ...
    function drawFibx(s){var pA=+s.p1,pB=+s.p2,pC=+s.p3,xa=xOf(s.l1),xb=xOf(s.l2),xc=xOf(s.l3);
      if(!(pA>0&&pB>0&&pC>0))return;var LV=[0,0.618,1,1.618,2.618];
      ctx.save();ctx.lineWidth=1;ctx.font="10px 'Space Mono',monospace";
      if(xa!=null&&xb!=null&&xc!=null){ctx.globalAlpha=.4;ctx.strokeStyle=s.color;ctx.setLineDash([4,3]);
        ctx.beginPath();ctx.moveTo(xa,yOf(pA));ctx.lineTo(xb,yOf(pB));ctx.lineTo(xc,yOf(pC));ctx.stroke();ctx.setLineDash([]);}
      var x0=(xc==null?0:xc);
      LV.forEach(function(L){var pr=pC+(pB-pA)*L,yy=yOf(pr);if(yy==null)return;
        ctx.globalAlpha=(L===1||L===1.618)?.85:.45;ctx.strokeStyle=s.color;ctx.beginPath();ctx.moveTo(x0,yy);ctx.lineTo(w.dr.W,yy);ctx.stroke();
        ctx.globalAlpha=1;inkTxt(x0+6,yy-3,(L*100).toFixed(1)+'%  '+cwFmt(pr),s.color,9.5);});
      ctx.restore();}
    /* ANDREWS PITCHFORK - median line from A through the midpoint of BC, with the two parallels */
    function drawFork(s){var xa=xOf(s.l1),xb=xOf(s.l2),xc=xOf(s.l3),ya=yOf(s.p1),yb=yOf(s.p2),yc=yOf(s.p3);
      if(xa==null||xb==null||xc==null||ya==null||yb==null||yc==null)return;
      var mx=(xb+xc)/2,my=(yb+yc)/2,en=rayEnd(xa,ya,mx,my),dx=(yb-yc)?0:0;
      ctx.save();setStyle(s);ctx.globalAlpha=.9;
      ctx.beginPath();ctx.moveTo(xa,ya);ctx.lineTo(en.x,en.y);ctx.stroke();
      ctx.globalAlpha=.6;ctx.setLineDash([5,4]);
      [[xb,yb],[xc,yc]].forEach(function(pt){var ox=pt[0]-mx,oy=pt[1]-my,e2=rayEnd(xa+ox,ya+oy,pt[0],pt[1]);
        ctx.beginPath();ctx.moveTo(xa+ox,ya+oy);ctx.lineTo(e2.x,e2.y);ctx.stroke();});
    ...
    function strokeShape(s){
      ...
      if(s.t==='fibx'){drawFibx(s);return;}
      if(s.t==='fork'){drawFork(s);return;}
      ...
      if(s.t==='pen'){var p=s.pts;if(!p||!p.length){ctx.restore();return;}ctx.beginPath();var started=false;for(var i=0;i<p.length;i++){var xx=xOf(p[i].l),yy=yOf(p[i].p);if(xx==null||yy==null)continue;if(!started){ctx.moveTo(xx,yy);started=true;}else ctx.lineTo(xx,yy);}ctx.stroke();}
        ...
        if(s.anch){anchDot(q.x1,q.y1,s.color);anchDot(q.x2,q.y2,s.color);if(s.txt)inkTxt(q.x1+8,q.y1-6,String(s.txt).slice(0,20),s.color,10);}/* a line that starts off-screen with a label in the gutter told the reader nothing about where it comes FROM (owner 2026-09-17) */
    ...
    /* THE CANVAS HAS TO FOLLOW THE PRICE AXIS TOO (2026-09-17, owner: "crtezi nisu zalepljeni na chart kao sto su svece
       zalepljene, pa kad pomeram chart slika se poremeti"). subscribeVisibleLogicalRangeChange fires for the TIME axis only, so a
       vertical drag, an autoscale re-fit or an indicator pane resizing the price scale moved the candles while the drawings stayed
       exactly where they were last painted - measured at 127px of drift after one vertical pan (build/glue-probe.js). Earlier runs
       read 1px only because a live tick happened to repaint in between, which is why this hid for so long. One rAF per window
       compares where two reference prices project to and repaints only when that actually moved; with no drawings on the chart it
       is a single property read per frame, and rAF is already paused while the tab is hidden. */
    w._pwRaf=requestAnimationFrame(function pw(){
      if(w.dead){w._pwRaf=0;return;}
      try{
        if(w.dr&&w.dr.shapes&&(w.dr.shapes.length||w.dr.cur)&&w.candle){
          if(!(w._pwRef>0)&&w.bars&&w.bars.length){var _lc=+w.bars[w.bars.length-1].close;if(_lc>0)w._pwRef=_lc;}
          if(w._pwRef>0){var _a=w.candle.priceToCoordinate(w._pwRef),_b=w.candle.priceToCoordinate(w._pwRef*1.01);
            if(_a!==w._pwA||_b!==w._pwB){w._pwA=_a;w._pwB=_b;redraw();}}
        }
      }catch(e){}
      w._pwRaf=requestAnimationFrame(pw);
    });
    setTimeout(size,60);
  }
  ...
  function cdTick(){if(document.hidden)return;var nowS=(window.mpSrvNow?window.mpSrvNow():(Date.now()+(+window.__mpSrvSkew||0)))/1000;
    for(var i=0;i<wins.length;i++){var w=wins[i];if(w.dead||!w.el)continue;var el=w._cdEl||(w._cdEl=w.el.querySelector('.cwin-cd'));if(!el)continue;
      ...
      el.textContent=stale?'--:--':cdFmt(iv-(nowS%iv),iv>=3600);el.classList.toggle('stale',!!stale);}}
  function cdStart(){if(!_cdT){cdSkew();_cdT=setInterval(cdTick,1000);}}
  ...
  function buildChart(w){ loadLib(function(){ if(w.dead||!window.LightweightCharts)return;
    ...
    try{ w.chart=mpCreateChart(host,{layout:{background:{color:'transparent'},textColor:'#9aa3ad',fontFamily:"'Familjen Grotesk',system-ui,sans-serif",attributionLogo:false},grid:{vertLines:{color:'rgba(35,41,50,.35)'},horzLines:{color:'rgba(35,41,50,.35)'}},rightPriceScale:{borderColor:'#232932'},timeScale:{borderColor:'#232932',timeVisible:true,secondsVisible:false,rightOffset:6,barSpacing:6},crosshair:{mode:0},autoSize:true});
      ...
      /* FREE PAN (owner 2026-08-13): a vertical drag in the pane flips the price scale to manual so the user can pan
         up/down; a price-AXIS drag is remembered the same way. w._userPS gates every periodic autoScale re-assert
         (watchdog / new-bar) so the view NEVER snaps back on a timer; a symbol/TF load or a double-click on the
         price axis re-arms autofit (keeps the XRP-after-BTC re-fit working). */
      (function(){var st={d:0};host.addEventListener('pointerdown',function(e){var r=host.getBoundingClientRect();st.d=1;st.x=e.clientX;st.y=e.clientY;st.ax=(e.clientX>r.right-64);st.dec=0;},true);host.addEventListener('pointermove',function(e){if(!st.d||st.dec)return;var dx=Math.abs(e.clientX-st.x),dy=Math.abs(e.clientY-st.y);if(dx<5&&dy<5)return;st.dec=1;if(st.ax){w._userPS=true;return;}if(dy>dx){w._userPS=true;try{w.chart.priceScale('right').applyOptions({autoScale:false});}catch(_){}}},true);window.addEventListener('pointerup',function(){st.d=0;},true);host.addEventListener('dblclick',function(e){var r=host.getBoundingClientRect();if(e.clientX>r.right-64){w._userPS=false;try{w.chart.priceScale('right').applyOptions({autoScale:true});}catch(_){}}});})();
  ...
  setInterval(function(){ if(document.hidden)return; var now=Date.now();
    for(var i=0;i<wins.length;i++){ var w=wins[i]; if(w.dead||!w.candle)continue;
      try{var vr=w.chart.timeScale().getVisibleLogicalRange();if(!w._userPS&&(!vr||!w.bars||!w.bars.length||vr.to>=w.bars.length-2))w.chart.priceScale('right').applyOptions({autoScale:true});}catch(e){}
      // catch a stalled CHART even when the price source LOOKS fresh: newest bar >1.5 intervals behind now → force a re-sync
      try{if(w.lastBar){var _iv=parseInt(w.tf,10)*60,_nb=Math.floor(now/1000/_iv)*_iv;if((_nb-w.lastBar.time>_iv*1.5||w._hole)&&(!w._gapT||now-w._gapT>10000)){w._gapT=now;refreshData(w);continue;}}}catch(e){}
  ...
  function doShare(){
    if(!wins.length){var url=location.origin+'/charts';if(navigator.share)navigator.share({title:'MarginPad Charts',url:url}).catch(function(){});else if(navigator.clipboard&&navigator.clipboard.writeText){navigator.clipboard.writeText(url);shareFeedback(__esT_mpcharts("linkCopied",'Link copied'));}return;}
    ...
    try{cv.toBlob(function(blob){if(!blob){shareFeedback('Failed');return;}var file=null;try{file=new File([blob],'marginpad-charts.png',{type:'image/png'});}catch(e){}
      if(file&&navigator.canShare&&navigator.canShare({files:[file]})){navigator.share({files:[file],title:'MarginPad Charts',text:__esT_mpcharts("myChartsOnMarginpad",'My charts on MarginPad')}).then(function(){shareFeedback('Shared ✓');}).catch(function(){download(blob);});}
  ...
  var layMenuEl=null;
  function renderLayMenu(){var arr=layoutsLoad();layMenuEl.innerHTML=__esT_mpcharts("savedLayouts",'<div class="cwin-ind-menu-h">Saved layouts</div>')+(arr.length?('<div class="cwin-ind-menu-list">'+arr.map(function(l){var c=(l.wins||[]).length;return '<div class="cws-lay-item"><button type="button" class="cws-lay-open" data-name="'+escAttr(l.name)+'">'+escHtml(l.name)+' <small>'+c+' chart'+(c===1?'':'s')+'</small></button><button type="button" class="cws-lay-del" data-del="'+escAttr(l.name)+__esT_mpcharts("titleDeleteAriaLabel",'" title="Delete" aria-label="Delete">&#10005;</button></div>');}).join('')+'</div>'):__esT_mpcharts("noSavedLayoutsYet",'<div class="cws-lay-empty">No saved layouts yet - arrange your charts, then “Save layout”.</div>'));}
  ...
  function qtLimHint(){
    ...
    if(!isFinite(l)||l<=0){h.textContent=__esT_mpcharts("thePriceYouWant",'The price you want to be filled at.');h.className='cqt-limh';return;}
    if(!(p>0)){h.textContent='';h.className='cqt-limh';return;}
    var d=(l-p)/p*100;h.textContent=Math.abs(d).toFixed(2)+'% '+(d<0?'below':'above')+__esT_mpcharts("theMarketFillsIf",' the market - fills if it ')+(d<0?'drops':'rises')+' to '+fmtP(l)+'.';h.className='cqt-limh';
  }
  function qtSetType(t){
    ...
    var ob=qtEl.querySelector('.cqt-open');if(ob)ob.textContent=(qtType==='limit')?__esT_mpcharts("placeLimitOrder",'Place limit order'):__esT_mpcharts("openPosition",'Open position');
    updateQT();
  }
  ...
  function doOpenPos(){ var sym=qtEl.querySelector('.cqt-sym').value,lev=qtLev,amt=+qtEl.querySelector('.cqt-amt').value||0,msg=qtEl.querySelector('.cqt-msg');
    ...
    if(!(amt>0)){msg.style.color='#ff6258';msg.textContent=__esT_mpcharts("enterAnAmount",'Enter an amount.');return;}
    ...
    if(qtType==='limit'){ // rest the order instead of opening - mpOrders owns it (server-side when signed in)
      ...
      if(!isFinite(_lpx)||_lpx<=0){msg.style.color='#ff6258';msg.textContent=__esT_mpcharts("enterALimitPrice",'Enter a limit price.');return;}
      if(!(_mk>0)){msg.style.color='#ff6258';msg.textContent=__esT_mpcharts("waitingForTheLive",'Waiting for the live price - try again in a second.');return;}
      ...
      if(!window.mpOrders){msg.style.color='#ff6258';msg.textContent=__esT_mpcharts("limitOrdersAreStill",'Limit orders are still loading - try again in a second.');return;}
      window.mpOrders.add({sym:sym,side:qtSide,px:_lpx,lev:lev,margin:amt,sl:_sl9,tp:_tp9},function(){
        msg.style.color='#f0c35a';msg.innerHTML='Limit '+(_lng9?'long':'short')+' '+sym+' at '+fmtP(_lpx)+__esT_mpcharts("isWaitingSeeIt",' is waiting - see it in <b>My Trades &rarr; Orders</b>.');
        try{window.mpBuzz&&window.mpBuzz([12]);}catch(e){}
        try{if(window.__mpTrack)window.__mpTrack('limitorder',sym+' '+qtSide+' @'+_lpx);}catch(e){}
      },function(m){msg.style.color='#ff6258';msg.textContent=m||__esT_mpcharts("couldNotPlaceThe",'Could not place the order.');});
      return;
    }
    function open(p,srvT,cid){if(!srvT&&window.mpIsMktClosed&&window.mpIsMktClosed(sym)){if(window.mpLimitToast)window.mpLimitToast(sym+__esT_mpcharts("marketIsClosedYou",' market is closed - you can trade it when it reopens.'));return;} // stocks: block client opens while the exchange is shut (consistent with the plan form)
      ...
      msg.style.color='#2ebd85';msg.innerHTML=(qtSide==='long'?'Long':'Short')+' '+sym+' '+L+__esT_mpcharts("timesOpenedSeeIt",'&times; opened - see it in <b>My Trades</b>.');
  ...
  function doCalc(t,b){var v=function(id){var e=b.querySelector('#'+id);return e?parseFloat(e.value):NaN;},side=b._side||'long',res,rEl=b.querySelector('.cws-calc-res');
    if(t==='liq'){var en=v('clEntry'),lv=Math.max(1,v('clLev')||1);res=(isFinite(en)&&en>0)?(__esT_mpcharts("liquidationPrice",'<span>Liquidation price</span><b style="color:#ff6258">')+fMoney(side==='long'?en*(1-(1-0.005)/lv):en*(1+(1-0.005)/lv))+'</b>'):__esT_mpcharts("liquidationPrice2",'<span>Liquidation price</span><b>-</b>');}
    ...
    else{var bal=v('csBal'),rk=v('csRisk'),en3=v('csEntry'),st=v('csStop');if(isFinite(bal)&&isFinite(rk)&&isFinite(en3)&&isFinite(st)&&Math.abs(en3-st)>0){var ra=bal*rk/100,qty=ra/Math.abs(en3-st);res=__esT_mpcharts("positionSize",'<span>Position size</span><b>')+fMoney(qty*en3)+'</b> <small>'+qty.toLocaleString('en-US',{maximumFractionDigits:4})+' units · risk '+fMoney(ra)+'</small>';}else res=__esT_mpcharts("positionSize2",'<span>Position size</span><b>-</b>');}
  ...
  function renderCalc(t){var b=calcEl.querySelector('.cws-calc-body');
    ...
    if(t==='liq')b.innerHTML=fld('clEntry',__esT_mpcharts("entryPrice",'Entry price'),'')+fld('clLev','Leverage','10')+side+'<div class="cws-calc-res"></div>';
    else if(t==='pnl')b.innerHTML=fld('cpEntry',__esT_mpcharts("entryPrice",'Entry price'),'')+fld('cpExit',__esT_mpcharts("exitPrice",'Exit price'),'')+fld('cpAmt',__esT_mpcharts("marginUsd",'Margin (USD)'),'100')+fld('cpLev','Leverage','10')+side+'<div class="cws-calc-res"></div>';
    else b.innerHTML=fld('csBal',__esT_mpcharts("accountBalanceUsd",'Account balance (USD)'),'1000')+fld('csRisk','Risk %','1')+fld('csEntry',__esT_mpcharts("entryPrice",'Entry price'),'')+fld('csStop',__esT_mpcharts("stopPrice",'Stop price'),'')+'<div class="cws-calc-res"></div>';
    b._side='long';
  ...
  function showSetupPanel(sym,info,w){
    ...
    var reasons=sig.length?sig.map(function(s){return '<li>'+escHtml(s)+'</li>';}).join(''):__esT_mpcharts("alignedTechnicalSignalsOn",'<li>Aligned technical signals on the 4-hour chart.</li>');
    var setupCard=su?('<div style="margin-top:12px;border:1px solid '+(long?'rgba(46,189,133,.4)':'rgba(255,98,88,.4)')+';border-radius:11px;padding:11px 12px;background:'+(long?'rgba(46,189,133,.06)':'rgba(255,98,88,.06)')+';">'
      ...
      +su_row('Entry',su_fmt(su.entry),'#cdd3da')+su_row(__esT_mpcharts("stopLoss",'Stop loss'),su_fmt(su.sl),'#ff6258')+su_row('Target 1',su_fmt(su.tp1),'#2ebd85')+su_row('Target 2',su_fmt(su.tp2),'#2ebd85')+su_row('Target 3',su_fmt(su.tp3),'#2ebd85')+su_row(__esT_mpcharts("riskReward",'Risk : reward'),'1 : '+(su.rrr||1.5),'#cdd3da')
      +'<div style="margin-top:9px;font-size:10.5px;line-height:1.4;color:#ff9a90;">⚠ '+su.lev+__esT_mpcharts("xIsExtremeRisk",'x is extreme risk - a ~')+(100/su.lev).toFixed(1)+__esT_mpcharts("moveAgainstYouLiquidation",'% move against you = liquidation. Always use the stop and size small.</div>')
    ...
    var dr=setupPanel.querySelector('#cwsSetupDraw');if(dr&&su)dr.onclick=function(){drawSetupLines(setupW,su);dr.textContent='Drawn ✓';setTimeout(function(){dr.textContent=__esT_mpcharts("drawOnChart",'Draw on chart');},1400);};
  }
  ...
  function buildMobileChart(){
    ...
    indBtn.addEventListener('click',function(){
      var sheet=el('<div class="cwm-sheet"><div class="cwm-sheet-p"><div class="cwm-sheet-h">Indicators<button class="cwm-sheet-x" type="button">✕</button></div><div>'+INDS.map(function(t){var _lk2=MP_INDS[t[0]]&&!indAllowed();return '<button type="button" class="cwm-ind-opt'+(mw.inds[t[0]]?' on':'')+'" data-ind="'+t[0]+'"><span'+(MP_INDS[t[0]]?' style="color:#c2f64a;font-weight:700'+(_lk2?';opacity:.5':'')+'"':'')+'>'+t[1]+(_lk2?' <b style="color:#5c6b84;font-weight:400;font-size:9.5px">LOCKED</b>':'')+'</span><span class="dot"></span></button>';}).join('')+'</div></div></div>');
      ...
      sheet.addEventListener('click',function(e){if(e.target===sheet||e.target.closest('.cwm-sheet-x')){close();return;}var o=e.target.closest('[data-ind]');if(o){var k=o.getAttribute('data-ind');if(MP_INDS[k]&&!indAllowed()){try{chartToast(__esT_mpcharts("lockedU2014MarginpadExclus2",'Locked \u2014 MarginPad exclusive. Ask us to unlock it.'));}catch(e2){}return;}mw.inds[k]=!mw.inds[k];o.classList.toggle('on',!!mw.inds[k]);try{applyInds(mw);}catch(e2){}indN_();persist();}});
    });
    ...
    mw.poll=setInterval(function(){ if(mw.dead||!mw.candle)return; if(mw._wsT&&Date.now()-mw._wsT<30000)return; fetch('/api/price?symbol='+encodeURIComponent(mw.sym)+window.__mpPQ('mwch',mw.sym),{cache:'no-store'}).then(function(r){return r.ok?r.json():null;}).catch(function(){return null;}).then(function(j){if(j&&!mw.dead){var px=+(j.price||j.p||j.last||0);if(px>0)liveTick(mw,px);}}); },12000);
