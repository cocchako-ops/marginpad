...
(function(){
  ...
  function ensureSub(w,on){var body=w.el&&w.el.querySelector('.cwin-body');if(!body)return null;var sh=body.querySelector('.cwin-sub'),dc=body.querySelector('.cwin-draw'),host=body.querySelector('.cwin-chart');
    ...
    if(sh&&!sh.querySelector('.cwin-subrz')){var rz=document.createElement('div');rz.className='cwin-subrz';rz.title='Drag to resize the indicator panel';sh.appendChild(rz);
  ...
  function aiDrawActs(w,acts,batch){if(!w||!w.dr||!w.dr.shapes||!w.bars||w.bars.length<2)return 0;_aiRej=[];var n=w.bars.length,px=+w.bars[n-1].close,k=0;
    ...
    var struct2=null;try{var _pv2=pivotsOf(w.bars,Math.max(2,Math.min(9,Math.round(n/40))),16),_h2=_pv2.filter(function(q){return q.kind==='high';}).slice(0,3),_l2=_pv2.filter(function(q){return q.kind==='low';}).slice(0,3);
      if(_h2.length>=2&&_l2.length>=2)struct2=(_h2[0].price>_h2[1].price&&_l2[0].price>_l2[1].price)?'higher highs and higher lows':((_h2[0].price<_h2[1].price&&_l2[0].price<_l2[1].price)?'lower highs and lower lows':'mixed');}catch(e){}
  ...
  var aiPremium=false;
  function aiShowPremiumGate(){if(!aiEl)return;aiEl.classList.add('gated');var body=aiEl.querySelector('.cwin-ai-body');if(!body)return;body.innerHTML='<div class="cwin-ai-gate"><b>Ask AI comes with Premium.</b><br>A built-in analyst that reads the chart you are looking at - the real measurements, not a screenshot - and draws the setup on it.<br><b>'+aiPlans.premium+' read a day with Premium ($11.99/mo)</b>, '+aiPlans.plus+' a day with Premium Plus ($159/mo).<br><button class="g-btn" type="button">Get Premium - $11.99/mo</button></div>';var g=body.querySelector('.g-btn');if(g)g.addEventListener('click',function(){if(window.mpPremium&&window.mpPremium.show)window.mpPremium.show('Unlock Ask AI on your charts');});}
  function aiShowGate(){if(!aiEl)return;aiEl.classList.add('gated');var body=aiEl.querySelector('.cwin-ai-body');body.innerHTML='<div class="cwin-ai-gate">Sign in to use Ask AI - a built-in analyst that reads the chart in front of you and draws the setup on it. It comes with Premium: <b>'+aiPlans.premium+' read a day at $11.99/mo</b>, '+aiPlans.plus+' a day on Premium Plus.<br><button class="g-btn" type="button">Sign in free</button></div>';var g=body.querySelector('.g-btn');if(g)g.addEventListener('click',function(){try{if(window.mpAuth&&window.mpAuth.open)window.mpAuth.open();}catch(e){}});}
  ...
  function buildMobileChart(){
    ...
    indBtn.addEventListener('click',function(){
      ...
      sheet.addEventListener('click',function(e){if(e.target===sheet||e.target.closest('.cwm-sheet-x')){close();return;}var o=e.target.closest('[data-ind]');if(o){var k=o.getAttribute('data-ind');if(MP_INDS[k]&&!indAllowed()){try{chartToast('Locked \u2014 MarginPad exclusive. Ask us to unlock it.');}catch(e2){}return;}mw.inds[k]=!mw.inds[k];o.classList.toggle('on',!!mw.inds[k]);try{applyInds(mw);}catch(e2){}indN_();persist();}});
    });
