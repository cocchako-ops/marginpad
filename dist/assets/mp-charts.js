...
(function(){
  ...
  function signalPlan(d){var n=d?d.length:0,P=10,M=3;if(n<P+3)return {mk:[],last:null};
    ...
    var fU=[],fL=[],dir=[],mk=[],last=null;for(i=0;i<n;i++){var hl2=(d[i].high+d[i].low)/2,bU=hl2+M*atr[i],bL=hl2-M*atr[i];
      ...
      if(i>P&&cd!==pd){mk.push(cd===1?{time:d[i].time,position:'belowBar',color:'#2ebd85',shape:'arrowUp',text:'BUY'}:{time:d[i].time,position:'aboveBar',color:'#ff6258',shape:'arrowDown',text:'SELL'});last={dir:cd,entry:d[i].close,time:d[i].time,atr:atr[i]};}}
    return {mk:mk,last:last};}
  function computeSignals(d){return signalPlan(d).mk;}
  /* latest Buy/Sell signal → an ATR-based trade plan: entry, take-profit (2R) + stop-loss (1R). Drawn as price lines when "Buy / Sell signals" is on. */
  function signalTargets(d){var pl=signalPlan(d),L=pl.last;if(!L||!(L.atr>0))return null;var risk=1.5*L.atr;return {dir:L.dir,entry:L.entry,tp:L.entry+L.dir*2*risk,sl:L.entry-L.dir*risk,time:L.time};}
  ...
  function applyInds(w){ if(!w.candle)return; var bars=w.bars||[];
    ...
    if(w.inds.sig){var _sp=signalTargets(bars);if(_sp){var _lng=_sp.dir===1;
      addPL({price:_sp.entry,color:'#8aa0b4',lineWidth:1,lineStyle:0,axisLabelVisible:true,title:(_lng?'BUY':'SELL')+' entry'});
      addPL({price:_sp.tp,color:'#2ebd85',lineWidth:1,lineStyle:2,axisLabelVisible:true,title:'TP'});
      addPL({price:_sp.sl,color:'#ff6258',lineWidth:1,lineStyle:2,axisLabelVisible:true,title:'SL'});}}
