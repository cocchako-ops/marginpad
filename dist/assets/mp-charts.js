...
(function(){
  ...
  function computeMomentum(d){var n=d?d.length:0;if(n<40)return [];var c=closes(d),m=macdCalc(c),r=rsiCalc(c,14),mk=[],i;
    for(i=35;i<n;i++){if(!isFinite(m.macd[i])||!isFinite(m.signal[i])||!isFinite(m.macd[i-1])||!isFinite(m.signal[i-1])||!isFinite(r[i]))continue;
      ...
      else if(m.macd[i-1]>=m.signal[i-1]&&m.macd[i]<m.signal[i]&&r[i]<48)mk.push({time:d[i].time,position:'aboveBar',color:'#ff6258',shape:'arrowDown',text:'MOM'});}
    return mk;}
  /* Squeeze breakout (TTM-style): Bollinger(20,2) inside Keltner(EMA20 +/- 1.5*ATR20) = squeeze ON; the bar it releases fires in the close-vs-basis direction */
  function computeSqueeze(d){var n=d?d.length:0;if(n<40)return [];var c=closes(d),mid=sma(c,20),e=ema(c,20),a=atr(d,20),sq=[],mk=[],i,j;
    for(i=0;i<n;i++){if(i<20||!isFinite(mid[i])||!isFinite(a[i])||!isFinite(e[i])){sq.push(false);continue;}
      var ss=0;for(j=i-19;j<=i;j++){var dd=c[j]-mid[i];ss+=dd*dd;}var sd=Math.sqrt(ss/20);
      sq.push((mid[i]+2*sd)<(e[i]+1.5*a[i])&&(mid[i]-2*sd)>(e[i]-1.5*a[i]));}
    for(i=25;i<n;i++){if(sq[i-1]&&!sq[i]){if(c[i]>=mid[i])mk.push({time:d[i].time,position:'belowBar',color:'#2ebd85',shape:'arrowUp',text:'SQZ'});else mk.push({time:d[i].time,position:'aboveBar',color:'#ff6258',shape:'arrowDown',text:'SQZ'});}}
    return mk;}
  /* Liquidation reversal (our liq feed): bucket liq notional per bar; an outsized spike (>=4x the trailing avg, >$50k)
     where one side dominates = capitulation/squeeze -> contrarian arrow. Longs wiped = green (bounce), shorts wiped = red (top). */
  function computeLiqRev(d,events,ivSec){if(!d||!d.length||!events||!events.length||!(ivSec>0))return [];
    var buckets={},i,j;for(i=0;i<events.length;i++){var e=events[i];if(!e||!(e.ts>0)||!(e.notional>0))continue;var bt=Math.floor((e.ts/1000)/ivSec)*ivSec;var b=buckets[bt]||(buckets[bt]={l:0,s:0});if(e.side==='long_liquidated')b.l+=e.notional;else b.s+=e.notional;}
    var tot=[];for(i=0;i<d.length;i++){var bb=buckets[d[i].time];tot.push(bb?bb.l+bb.s:0);}
    var mk=[];for(i=20;i<d.length;i++){var b=buckets[d[i].time];if(!b)continue;var t=b.l+b.s;if(!(t>50000))continue;
      var avg=0,cnt=0;for(j=i-20;j<i;j++){avg+=tot[j];cnt++;}avg=cnt?avg/cnt:0;if(!(avg>0)||t<avg*4)continue;
      if(b.l>b.s*1.5)mk.push({time:d[i].time,position:'belowBar',color:'#2ebd85',shape:'arrowUp',text:'LIQ'});
      else if(b.s>b.l*1.5)mk.push({time:d[i].time,position:'aboveBar',color:'#ff6258',shape:'arrowDown',text:'LIQ'});}
    return mk;}
  ...
  function applyInds(w){ if(!w.candle)return; var bars=w.bars||[];
    ...
    if(w.inds.mom&&IA)_addSig('MOM',computeMomentum(bars));
    if(w.inds.sqz&&IA)_addSig('SQZ',computeSqueeze(bars));
    ...
    if(w.inds.casc&&w._casc){var _cl=w._casc.score[w._casc.score.length-1];if(isFinite(_cl))w.legItems.push({raw:true,color:_cl>=90?'#ff6258':_cl>=70?'#ffb020':'#8fa3c4',label:'Cascade '+Math.round(_cl)+'/100 \u2014 '+(_cl>=90?'EXTREME fragility':_cl>=70?'fragile market':'calm'),title:'0-100: how much liquidation fuel sits close to price vs what volume can absorb. Above 70 = fragile, above 90 = one push can cascade.'});addS({color:'#ffb020',lineWidth:1.6,priceScaleId:'casc'},mapVal(bars,w._casc.score),{label:'CASCADE',dec:0});addS({color:'rgba(255,215,90,.4)',lineStyle:2,priceScaleId:'casc'},bars.map(function(b){return {time:b.time,value:90};}));pscale('casc','casc');}
    if(w.inds.brain&&w._brain){var _bl=w._brain.score[w._brain.score.length-1];if(isFinite(_bl))w.legItems.push({raw:true,color:_bl>=55?'#2ebd85':_bl<=-55?'#ff6258':'#8fa3c4',label:'Brain '+(_bl>=0?'+':'')+Math.round(_bl)+' \u2014 '+(_bl>=55?'strong bullish lean':_bl<=-55?'strong bearish lean':'no strong edge'),title:'-100..+100: the learned 8-factor blend. Beyond \u00b155 the model sees a strongly one-sided market; between, it has no strong opinion.'});addS({color:'#c2f64a',lineWidth:1.6,priceScaleId:'brain'},mapVal(bars,w._brain.score),{label:'BRAIN',dec:0});addS({color:'rgba(120,130,140,.35)',lineStyle:2,priceScaleId:'brain'},bars.map(function(b){return {time:b.time,value:0};}));addS({color:'rgba(255,215,90,.3)',lineStyle:2,priceScaleId:'brain'},bars.map(function(b){return {time:b.time,value:55};}));addS({color:'rgba(255,215,90,.3)',lineStyle:2,priceScaleId:'brain'},bars.map(function(b){return {time:b.time,value:-55};}));pscale('brain','brain');
