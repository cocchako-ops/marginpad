...
(function(){
  ...
  var INDS=[['sig','Buy / Sell signals'],['sr','Support / Resistance'],['ema9','EMA 9'],['ema21','EMA 21'],['ema50','EMA 50'],['ema100','EMA 100'],['ema200','EMA 200'],['sma20','SMA 20'],['sma50','SMA 50'],['sma100','SMA 100'],['sma200','SMA 200'],['hma','Hull MA 21'],['vwap','VWAP'],['bb','Bollinger'],['vol','Volume'],['rsi','RSI'],['macd','MACD'],['stoch','Stoch'],['atr','ATR'],['wr','Williams %R'],['cci','CCI'],['casc','Cascade Radar'],['brain','Market Brain (adaptive AI)'],['memory','Market Memory (AI forecast)'],['magnet','Liquidation Magnet']];
  ...
  function applyInds(p){ if(!p.chart||!p.candle)return;
    ...
    if(p.inds.magnet&&p._mag&&S){var _mg=p._mag,_M=S.money;
      ...
      var _tg=_mg.side==='up'?_mg.up:(_mg.side==='down'?_mg.dn:null),_mp=[];if(_mg.up)_mp.push('above +'+(_mg.up.dist*100).toFixed(1)+'% ('+_M(_mg.up.w)+' short)');if(_mg.dn)_mp.push('below \u2212'+(_mg.dn.dist*100).toFixed(1)+'% ('+_M(_mg.dn.w)+' long)');if(_tg)p.legItems.push({raw:true,color:'#c2f64a',label:'Magnet: pulled '+(_mg.side==='up'?'UP':'DOWN')+' \u00b7 '+_mp.join(' \u00b7 ')});}
    mLeg(p);
  }
  // ---- trade import ----
  ...
  function buildAi(body,p){
    ...
    function ctx(){
      ...
      try{if(window.__mpAiContext&&p.bars&&p.bars.length>10){var rc=window.__mpAiContext({sym:p.sym,tf:p.tf,bars:p.bars,inds:p.inds,dr:p.w&&p.w.dr,emaList:[],smaList:[]});if(rc){var al=mAllowed(),on=[],lk=[];INDS.forEach(function(d){if(p.inds[d[0]])on.push(d[0]);if(mEx(d[0])&&!al)lk.push(d[0]);});
        rc.indicatorsShown=on;rc.chartTools={indicators:{ids:INDS.map(function(d){return d[0];}),on:on,locked:lk,note:'moving averages are separate ids here: ema9 ema21 ema50 ema100 ema200 sma20 sma50 sma100 sma200'},shapes:['trend','ray','channel','zone','hline','forecast','text','fib'],timeframes:TFS.map(function(t){return t[0];}),currentTf:p.tf,currentTfLabel:tfLabel(p.tf),canSwitchSymbol:true,barsAgoNote:'barsAgo 0 = the newest candle; '+p.bars.length+' candles are loaded; negative projects into the future (max -30)'};return rc;}}}catch(e){}
